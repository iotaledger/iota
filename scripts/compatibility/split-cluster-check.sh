...
# Track child PIDs so we can report liveness and tear the cluster down cleanly.
NODE_PIDS=()
NODE_NAMES=()
...
start_node node-0 "$WORKING_DIR/iota-node-release" "${CONFIGS[0]}"
start_node node-1 "$WORKING_DIR/iota-node-release" "${CONFIGS[1]}"
start_node node-2 "$WORKING_DIR/iota-node-candidate" "${CONFIGS[2]}"
start_node node-3 "$WORKING_DIR/iota-node-candidate" "${CONFIGS[3]}"
start_node fullnode "$WORKING_DIR/iota-node-release" "$IOTA_CONFIG_DIR/fullnode.yaml"
...
dump_diagnostics() {
  local i name marker status
  echo "===== DIAGNOSTICS (elapsed ${SECONDS}s) ====="
  for i in "${!NODE_NAMES[@]}"; do
    if kill -0 "${NODE_PIDS[$i]}" 2>/dev/null; then
      echo "  ${NODE_NAMES[$i]} (pid ${NODE_PIDS[$i]}): ALIVE"
    else
      echo "  ${NODE_NAMES[$i]} (pid ${NODE_PIDS[$i]}): DEAD"
    fi
  done
  echo "===== MARKER MATRIX ====="
  for i in "${!NODE_NAMES[@]}"; do
    ...
    for marker in "${DIAG_MARKERS[@]}"; do
      if grep -q "$marker" "$LOG_DIR/$name.log" 2>/dev/null; then status="present"; else status="ABSENT "; fi
      printf '  %-9s | %-45s | %s\n' "$name" "$marker" "$status"
    done
  done
  echo "===== LOG TAILS (last 50 lines) ====="
  for i in "${!NODE_NAMES[@]}"; do
    echo "----- ${NODE_NAMES[$i]}.log -----"
