...
IGNORED_EXTENSIONS = ('svg', 'mv', 'png', 'jpg', 'jpeg', 'gif', 'wasm', 'errmap', 'bcs',
                      'chk', 'pdf', 'ai', 'mp3', 'wav', 'ico', 'ttf', 'otf', 'woff', 'woff2', 'mvsm')
...
CASE_INSENSITIVE_IGNORE_REPLACE = ('mistakes', 'optimistic', 'optimist', 'optimistically', 'testsuite',
                                   'test_suite', 'test-suite', 'lawsuit', 'suitable', 'mistic', 'minimist', 'headlessui')
    ...
    if not skip_filemod:
        ...
        for fn in to_process:
                ...
                for kw, replacement in REPLACE_MAP:
                    ...
                    for word in content.split():
                        if '://' in word and kw in word:
                            suspicious.append(
                                (fn, line, 'PART_OF_URL', content.strip(), word, '://'))

                        for ignore_part in CASE_INSENSITIVE_IGNORE_REPLACE:
                            if ignore_part.lower() in word.lower():
                                suspicious.append(
                                    (fn, line, 'IGNORE_LIST', content.strip(), word, ignore_part))
                                ignored_word = True

                        for ignore_part in WARN_ABOUT:
                            if ignore_part in word:
                                suspicious.append(
                                    (fn, line, 'REPLACED_BUT_POSSIBLE_BREAKING_TESTS', content.strip(), word, ignore_part))
            ...
            for i, line in enumerate(new_content):
                if 'SPDX-License-Identifier' in line:
                    ...
                    if line.strip().startswith('#'):
                        ...
                        print('# Copyright addition edge case for %s: `%s` (line %s)' % (
                            fn, line, i))
                        suspicious.append(
                            (fn, i, 'COPYRIGHT_EDGE_CASE', line, '', ''))
                        fix_copyright = False
                        break
            ...
            if fix_copyright:
                ...
                new_content = new_content[:last_copyright_line+1] + \
                    to_add + new_content[last_copyright_line+1:]
                ...
                print('# Copyright added to %s: \n%s\n' % (fn, ''.join(
                    new_content[prefix_line:last_copyright_line+10])))
        ...
        if len(suspicious) > 0:
            print(
                '# IGNORED REPLACEMENTS (marked as suspicious), might need manual replacement!\n')

            for sus in suspicious:
                fn, line, reason, content, word, extra = sus
                print('# - %s (line: %s):  %s (%s: %s `%s`)' %
                      (fn, line, content.strip(), reason, word, extra))
                ...
                if kw in os.path.basename(filename):
                    ...
                    for orig, case in skip_replace_mapping:
                        if case in filename:
                            ...
                            newfn = newfn.replace(
                                case.replace(orig, replacement), case)
                            print('# Edge case: %s (o: %s, case: %s) -> %s -> %s' %
                                  (filename, orig, case, replacement, newfn))
...
def main():
    parser = argparse.ArgumentParser(
        description='Port a folder recursively to from SUI based naming to IOTA based naming.')

    parser.add_argument(
        '--path', '-p',  help='The path of the folder to process, defaults to the main folder of the project if left out')
    parser.add_argument('--execute', action='store_true', default=False,
                        help='Flag to execute the replacements, if omitted it will just be a dry-run')
    parser.add_argument('--respect-gitignore', action='store_true', default=True,
                        help='Respect what is set in .gitignore to be ignored, defaults to True')
    parser.add_argument('--skip-filemod', action='store_true', default=False,
                        help='Skip file modification and only do the rename part')
    parser.add_argument('--use-git-mv', action='store_true', default=False,
                        help='Use git mv instead of a normal mv, for same repo modifications')

    args = parser.parse_args()

    rename(path=args.path, dry_run=(not args.execute), respect_gitignore=args.respect_gitignore,
           skip_filemod=args.skip_filemod, use_git_mv=args.use_git_mv)
