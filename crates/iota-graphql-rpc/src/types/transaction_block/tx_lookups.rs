...
//!
//! This pattern allows us to offer a simple rule for users: If you are
//! filtering on a single value, you can do so without worrying. If you want to
//! additionally filter by the sender, that is also possible, but if you want to
//! combine any other set of filters, you need to use a "scan limit".
//!
//! ## Query construction
//!
...
pub(crate) struct TxBounds {
    ...
    /// the smallest transaction sequence number of the earliest checkpoint
...
impl TxBounds {
    ...
    /// further adjusted by cursors and scan limit. If the checkpoints cannot be
    /// found, or if there are any inconsistencies or invalid combinations,
    /// i.e. `after` cursor is greater than the upper bound, return None.
    pub(crate) async fn query(
        ...
        // Inclusive lowerbound in terms of checkpoint sequence number. If a lower bound
        // is not set, we will default to the smallest checkpoint available from
        // the database, retrieved from the watermark.
        //
        ...
        // Inclusive upper bound in terms of checkpoint sequence number. If no
        // upperbound is given, use `checkpoint_viewed_at`.
        //
        ...
        // Read from the `checkpoints` table rather than the `pruner_cp_watermark`
        // table, because the `checkpoints` table is pruned first.
        use checkpoints::dsl;
        // Inclusive lower bound and exclusive upper bound of the transaction sequence
        // number range.
        let (tx_lo, tx_hi) = {
...
/// we just query the `tx_affected_addresses` table. Other combinations, in
/// particular when kind is SystemTx and sender is specified and not 0x0, are
/// inconsistent and will not produce any results. These inconsistent cases are
/// expected to be checked for before this is called.
fn select_kind(
