use iota_proc_macros::{extend_trait_with_non_fallible, extend_impl_with_non_fallible};
...
impl ExtendedService for MockStore {
    ...
    fn try_put_async(&self, _key: &str, _value: String) -> impl Future<Output = IotaResult<()>> {
        async { Ok(()) }
    }
