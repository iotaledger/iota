...
pub trait TransactionBuilder {
    ...
    async fn transfer_object(
        ...
        object_id: ObjectID,
        /// gas object to be used in this transaction, node will pick one from
        /// the signer's possession if not provided
        gas: Option<ObjectID>,
        /// the gas budget, the transaction will fail if the gas cost exceed the
        /// budget
    ...
    /// Create an unsigned transaction to send SUI coin object to a Sui address.
    ...
    /// will be used to pay the gas fee for the transaction. The gas object
    /// can not appear in `input_coins`. If the gas object is not specified, the
    /// RPC server will auto-select one.
    #[method(name = "pay")]
    async fn pay(
        &self,
        /// the transaction signer's Sui address
        ...
        /// the recipients' addresses, the length of this vector must be the
        /// same as amounts.
        recipients: Vec<SuiAddress>,
        /// the amounts to be transferred to recipients, following the same
        /// order
    ...
    /// This is for SUI coin only and does not require a separate gas coin
    /// object. Specifically, what pay_sui does are:
    /// 1. debit each input_coin to create new coin following the order of
    /// amounts and assign it to the corresponding recipient.
    /// 2. accumulate all residual SUI from input coins left and deposit all SUI
    ///    to the first
    ...
    async fn pay_sui(
        ...
        /// the transaction signer's Sui address
        signer: SuiAddress,
        /// the Sui coins to be used in this transaction, including the coin for
        /// gas payment.
        input_coins: Vec<ObjectID>,
        /// the recipients' addresses, the length of this vector must be the
        /// same as amounts.
    ...
    /// This is for SUI coin only and does not require a separate gas coin
    /// object. Specifically, what pay_all_sui does are:
    /// 1. accumulate all SUI from input coins and deposit all SUI to the first
    ///    input coin
    /// 2. transfer the updated first coin to the recipient and also use this
    ///    first coin as gas coin object.
    /// 3. the balance of the first input coin after tx is sum(input_coins) -
    ///    actual_gas_cost.
    ...
    async fn move_call(
        ...
        /// budget
        gas_budget: BigInt<u64>,
        /// Whether this is a Normal transaction or a Dev Inspect Transaction.
        /// Default to be `SuiTransactionBlockBuilderMode::Commit` when it's
        /// None.
    ...
    async fn publish(
        ...
        /// a list of transitive dependency addresses that this set of modules
        /// depends on.
    ...
    async fn merge_coin(
        ...
        /// the transaction signer's Sui address
        signer: SuiAddress,
        /// the coin object to merge into, this coin will remain after the
        /// transaction
        primary_coin: ObjectID,
        /// the coin object to be merged, this coin will be destroyed, the
        /// balance will be added to `primary_coin`
    ...
    /// Add timelocked stake to a validator's staking pool using multiple
    /// balances and amount.
