...
To try this locally, refer to the [docs](https://docs.iota.org/developer/getting-started/local-network). Note: run `RUST_LOG="consensus=off" cargo run -p iota-localnet --features indexer start --with-faucet --force-regenesis --with-indexer` to rebuild.
