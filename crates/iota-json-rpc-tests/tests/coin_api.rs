...
async fn get_all_coins_with_a_cursor_whose_object_is_gone() {
    ...
    let all: CoinPage = http_client
        ...
        .await
        .unwrap();
    assert!(all.data.len() >= 4, "need coins either side of the cursor");
    let cursor_coin = all.data[0].coin_object_id;
    let next_coin = all.data[1].coin_object_id;
    // Merging into `next_coin` would move its own row too, so use a later one.
    let primary_coin = all.data[2].coin_object_id;
    ...
    let merge = http_client
        ...
        .await
        .unwrap();
    execute_tx(&cluster, http_client, merge).await.unwrap();
    ...
    assert!(
        ...
        "the destroyed cursor coin must not come back",
    );
    assert!(
...
async fn get_all_balances() {
    ...
    let iota_balance = balances
        ...
        .expect("the IOTA balance should exist");

    assert_eq!(
        (DEFAULT_NUMBER_OF_OBJECT_PER_ACCOUNT as u64 * DEFAULT_GAS_AMOUNT) as u128,
        iota_balance.total_balance,
        "the IOTA balance should match expected amount"
    );
