...
async fn get_checkpoint(
    ...
    let SignedCheckpointSummary {
        ...
        CheckpointId::Digest(d) => state.inner().try_get_checkpoint_by_digest(&d),
    }?
...
async fn list_full_checkpoints(
    ...
    accept: AcceptFormat,
    State(state): State<StateReader>,
) -> Result<Page<iota_types::full_checkpoint_content::CheckpointData, CheckpointSequenceNumber>> {
    match accept {
        AcceptFormat::Bcs => {}
        _ => {
            return Err(RestError::new(
    ...
    let cursor = checkpoints.last().and_then(|checkpoint| match direction {
        ...
        Direction::Descending => {
            ...
            // If we've exhausted our available object range then there are no more pages
            // left
            if cursor < Some(oldest_checkpoint) {
                None
            } else {
