...
impl<'a> BytecodeSourceVerifier<'a> {
    ...
    /// Iota network we are publishing to.
    ...
    /// matches the bytecode at the address specified on the Iota network we
    /// are publishing to.
    ...
    async fn pkg_for_address(
        &self,
        addr: AccountAddress,
    ) -> Result<IotaRawMovePackage, SourceVerificationError> {
        ...
        match obj {
            ...
            IotaRawData::MoveObject(move_obj) => Err(
