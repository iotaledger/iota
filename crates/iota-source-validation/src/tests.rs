...
async fn fail_verification_bad_address() -> anyhow::Result<()> {
    ...
    expected.assert_eq(
        &BytecodeSourceVerifier::new(client.read_api())
            .verify(&a_pkg, ValidationMode::root_and_deps_at(Address::ZERO))
            .await
            .unwrap_err()
            .to_string(),
    );
...
async fn package_not_found() -> anyhow::Result<()> {
    ...
    let package_root = Address::random();
...
async fn successful_verification_with_bytecode_dep() -> anyhow::Result<()> {
    ...
    let tmp_dir = iota_common::tempdir();
    {
        ...
        let b_src =
            copy_published_package(&tmp_dir.path().join("publish"), "b", Address::ZERO).await?;
    ...
    let (a_pkg, a_ref) = {
        let a_src = copy_published_package(&tmp_dir, "a", Address::ZERO).await?;
        (
            compile_package(a_src.clone()),
            publish_package(context, a_src).await.0,
        )
...
pub async fn upgrade_package_with_wallet(
    ...
    let grpc_client = context.get_grpc_client().await.unwrap();
    let transaction = {
