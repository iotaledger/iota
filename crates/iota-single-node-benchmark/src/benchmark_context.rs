    ...
    pub(crate) async fn benchmark_sustained_execution(
        ...
        while std::time::Instant::now() < deadline {
            ...
            let tasks: FuturesUnordered<_> = transactions
                ...
                .map(|tx| {
                    ...
                    tokio::spawn(async move {
                        validator
                            .execute_certificate(tx, &vec![], Component::Baseline)
                            .await
                    })
                })
                .collect();
            ...
            for effect in &effects {
                ...
                for mutated_obj in effect.mutated() {
                    let oref = *mutated_obj.reference();
    ...
    pub(crate) async fn preparing_owned_objects(
        ...
        for effects in results {
            ...
            for created in effects.created() {
                if let Some(owner) = created.owner().as_opt_address() {
                    owned_objects
                        .entry(*owner)
                        .or_default()
                        .push(*created.reference());
                }
            }
            ...
            let (owner, root_object) = effects
                ...
                .filter_map(|created| {
                    created
                        .owner()
                        .as_opt_address()
                        .map(|owner| (*owner, *created.reference()))
                })
                .next()
                .unwrap();
    ...
    pub(crate) async fn prepare_shared_objects(
        ...
        for effects in results {
            let shared_object = effects
                ...
                .into_iter()
                .filter_map(|created| {
                    let oref = created.reference();
                    if created.owner().is_shared() {
