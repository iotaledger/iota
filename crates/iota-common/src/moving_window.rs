...
impl<T: MovingWindowValue> MovingWindow<T> {
    ...
    /// Adds a new value to the window. If the window is at capacity, the oldest
    /// value is removed before adding the new value.
    ...
    /// Get the current average of all values in the window. Returns the value's
