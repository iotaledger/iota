    ...
    T: Copy + PartialOrd + rand::distributions::uniform::SampleUniform + TryFrom<usize>,
{
    ...
    // random: thrown in case there are weird behaviors at specific but arbitrary
    // eviction/miss rates.
    //
    // We don't simply use a uniform random choice because the most interesting
    // cases are probably a) the value that will be used in production and b) a
    // very tiny value so we want those two cases to get picked more often.
...
pub fn tempdir() -> TempDir {
    nondeterministic!(tempfile::tempdir()).expect("Failed to open temporary directory")
}
