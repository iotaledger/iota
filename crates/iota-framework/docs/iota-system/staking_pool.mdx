...
activation_epoch: <Link to="../move-stdlib/option#0x1_option_Option">option::Option</Link>&lt;u64&gt;
</dt>
<dd>
 The epoch at which this pool became active.
 The value is 
None if the pool is pre-active and 
Some(&lt;epoch_number&gt;) if active or inactive.
</dd>
<dt>

deactivation_epoch: <Link to="../move-stdlib/option#0x1_option_Option">option::Option</Link>&lt;u64&gt;
</dt>
<dd>
 The epoch at which this staking pool ceased to be active. 
None = \{pre-active, active},
 
Some(&lt;epoch_number&gt;) if in-active, and it was de-activated at epoch 
&lt;epoch_number&gt;.
</dd>
<dt>
...
<dd>
 ...
 in the 
<Link to="staking_pool#0x3_staking_pool_StakedIota">StakedIota</Link> object, updated at epoch boundaries.
</dd>
<dt>

rewards_pool: <Link to="../iota-framework/balance#0x2_balance_Balance">balance::Balance</Link>&lt;<Link to="../iota-framework/iota#0x2_iota_IOTA">iota::IOTA</Link>&gt;
</dt>
<dd>
...
exchange_rates: <Link to="../iota-framework/table#0x2_table_Table">table::Table</Link>&lt;u64, <Link to="staking_pool#0x3_staking_pool_PoolTokenExchangeRate">staking_pool::PoolTokenExchangeRate</Link>&gt;
</dt>
<dd>
 Exchange rate history of previous epochs. Key is the epoch number.
 The entries start from the 
activation_epoch of this pool and contains exchange rates at the beginning of each epoch,
...
principal: <Link to="../iota-framework/balance#0x2_balance_Balance">balance::Balance</Link>&lt;<Link to="../iota-framework/iota#0x2_iota_IOTA">iota::IOTA</Link>&gt;
</dt>
<dd>
...
2. Using the above number and the given 
principal_withdraw_amount, calculates the rewards portion of the
...
Called by 
<Link to="validator#0x3_validator">validator</Link> module to activate a staking pool.
...
Deactivate a staking pool by setting the 
deactivation_epoch. After
...
Split StakedIota 
self to two parts, one with principal 
split_amount,
and the remaining principal is left in 
self.
All the other parameters of the StakedIota like 
stake_activation_epoch or 
pool_id remain the same.
...
Split the given StakedIota to the two parts, one with principal 
split_amount,
...
Consume the staked iota 
other and add its value to 
self.
