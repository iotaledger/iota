...
at 
<Link to="voting_power#0x3_voting_power_MAX_VOTING_POWER">MAX_VOTING_POWER</Link>. If 
<Link to="voting_power#0x3_voting_power_MAX_VOTING_POWER">MAX_VOTING_POWER</Link> is not a feasible cap, we pick the lowest possible cap.
...
Insert 
new_info to 
info_list as part of insertion sort, such that 
info_list is always sorted
