...
validator_low_stake_threshold are considered to
...
validator_very_low_stake_threshold will be removed
...
iota_treasury_cap: <Link to="../iota-framework/iota#0x2_iota_IotaTreasuryCap">iota::IotaTreasuryCap</Link>
</dt>
<dd>
...
validators: <Link to="validator_set#0x3_validator_set_ValidatorSet">validator_set::ValidatorSet</Link>
</dt>
<dd>
...
<Link to="storage_fund#0x3_storage_fund">storage_fund</Link>: <Link to="storage_fund#0x3_storage_fund_StorageFund">storage_fund::StorageFund</Link>
</dt>
<dd>
 The storage fund.
</dd>
...
parameters: <Link to="iota_system_state_inner#0x3_iota_system_state_inner_SystemParameters">iota_system_state_inner::SystemParameters</Link>
</dt>
<dd>
 A list of system config parameters.
</dd>
...
validator_report_records: <Link to="../iota-framework/vec_map#0x2_vec_map_VecMap">vec_map::VecMap</Link>&lt;<b>address</b>, <Link to="../iota-framework/vec_set#0x2_vec_set_VecSet">vec_set::VecSet</Link>&lt;<b>address</b>&gt;&gt;
</dt>
<dd>
...
<Link to="stake_subsidy#0x3_stake_subsidy">stake_subsidy</Link>: <Link to="stake_subsidy#0x3_stake_subsidy_StakeSubsidy">stake_subsidy::StakeSubsidy</Link>
</dt>
<dd>
...
<dd>
 ...
 The rest of the fields starting with 
safe_mode_ are accmulated during safe mode
 when advance_epoch_safe_mode is executed. They will eventually be processed once we
 are out of safe mode.
</dd>
<dt>

safe_mode_storage_charges: <Link to="../iota-framework/balance#0x2_balance_Balance">balance::Balance</Link>&lt;<Link to="../iota-framework/iota#0x2_iota_IOTA">iota::IOTA</Link>&gt;
</dt>
<dd>
...
safe_mode_computation_rewards: <Link to="../iota-framework/balance#0x2_balance_Balance">balance::Balance</Link>&lt;<Link to="../iota-framework/iota#0x2_iota_IOTA">iota::IOTA</Link>&gt;
</dt>
<dd>
...
parameters: <Link to="iota_system_state_inner#0x3_iota_system_state_inner_SystemParametersV2">iota_system_state_inner::SystemParametersV2</Link>
</dt>
<dd>
 A list of system config parameters.
</dd>
...
stakes in their staking pool. Once they have at least 
MIN_VALIDATOR_JOINING_STAKE amount of stake they
can call 
request_add_validator to officially become an active validator at the next epoch.
Aborts if the caller is already a pending or active validator, or a validator candidate.
Note: 
proof_of_possession MUST be a valid signature using iota_address and protocol_pubkey_bytes.
...
We use the sender of 
ctx to look up the validator
(i.e. sender must match the iota_address in the validator).
At the end of the epoch, the 
<Link to="validator#0x3_validator">validator</Link> object will be returned to the iota_address
of the validator.
...
1. both the reporter in 
cap and the input 
reportee_addr are active validators.
...
Undo a 
report_validator action. Aborts if
1. the reportee is not a currently active validator or
2. the sender has not previously reported the 
reportee_addr, or
...
Create a new 
UnverifiedValidatorOperationCap, transfer it to the
...
<b>public</b>(package) <b>fun</b> <Link to="iota_system_state_inner#0x3_iota_system_state_inner_advance_epoch">advance_epoch</Link>(
    ...
    <b>let</b> burnt_leftover_amount = leftover_staking_rewards.value();
...
Returns the total amount staked with 
validator_addr.
Aborts if 
validator_addr is not an active validator.
...
Returns all the validators who are currently reporting 
addr
