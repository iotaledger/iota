...
<dl>
<dt>
<code>contents: <Link to="../move-stdlib/vector#0x1_vector">vector</Link>&lt;<Link to="../iota-framework/vec_map#0x2_vec_map_Entry">vec_map::Entry</Link>&lt;K, V&gt;&gt;</code>
</dt>
<dd>
...
Create an empty <code><Link to="../iota-framework/vec_map#0x2_vec_map_VecMap">VecMap</Link></code>
...
Only works for a "copyable" value as references cannot be stored in <code><Link to="../move-stdlib/vector#0x1_vector">vector</Link></code>.
...
Note that map entries are stored in insertion order, *not* sorted by key.
Aborts if <code>idx</code> is greater than or equal to <code><Link to="../iota-framework/vec_map#0x2_vec_map_size">size</Link>(self)</code>
