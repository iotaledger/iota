...
Similar to 
iota::table, an 
<Link to="object_table#0x2_object_table_ObjectTable">ObjectTable</Link>&lt;K, V&gt; is a map-like collection. But unlike
...
<Link to="table#0x2_table">table</Link>: &<b>mut</b> <Link to="object_table#0x2_object_table_ObjectTable">ObjectTable</Link>&lt;K, V&gt; and returns the value.
Aborts with 
iota::dynamic_field::EFieldDoesNotExist if the table does not have an entry with
that key 
k: K.
...
Aborts with 
<Link to="object_table#0x2_object_table_ETableNotEmpty">ETableNotEmpty</Link> if the table still contains values
...
Returns the ID of the object associated with the key if the table has an entry with key 
k: K
