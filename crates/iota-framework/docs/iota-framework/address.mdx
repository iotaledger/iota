...
Error from 
from_bytes when it is supplied too many or too few bytes.
...
Convert 
a into a u256 by interpreting 
a as the bytes of a big-endian integer
(e.g., 
<Link to="../iota-framework/address#0x2_address_to_u256">to_u256</Link>(0x1) == 1)
...
Convert 
n into an address by encoding it as a big-endian integer (e.g., 
<Link to="../iota-framework/address#0x2_address_from_u256">from_u256</Link>(1) = @0x1)
Aborts if 
n > 
MAX_ADDRESS
...
Aborts with 
<Link to="../iota-framework/address#0x2_address_EAddressParseError">EAddressParseError</Link> if the length of 
bytes is not 32
...
Convert 
a to a hex-encoded ASCII string
