...
issuer: <Link to="../move-stdlib/string#0x1_string_String">string::String</Link>
</dt>
<dd>
 The issuer
</dd>
...
Aborts with 
<Link to="zklogin_verified_issuer#0x2_zklogin_verified_issuer_EInvalidProof">EInvalidProof</Link> if the verification fails.
...
Returns true if 
<b>address</b> was created using zklogin with the given issuer and address seed.
...
Aborts with 
<Link to="zklogin_verified_issuer#0x2_zklogin_verified_issuer_EInvalidInput">EInvalidInput</Link> if the 
iss input is not a valid UTF-8 string.
