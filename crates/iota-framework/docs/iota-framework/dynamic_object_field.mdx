...
Similar to 
iota::dynamic_field, this module allows for the access of dynamic fields. But
unlike, 
...
name: Name.
Aborts with 
EFieldAlreadyExists if the object already has that field with that name.
...
<Link to="../iota-framework/object#0x2_object">object</Link>s dynamic object field with the name specified by 
name: Name.
Aborts with 
EFieldDoesNotExist if the object does not have a field with that name.
Aborts with 
...
<Link to="../iota-framework/object#0x2_object">object</Link> has a dynamic object field with the name specified by
