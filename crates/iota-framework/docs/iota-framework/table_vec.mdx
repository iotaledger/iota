...
A basic scalable vector library implemented using 
Table.
...
contents: <Link to="../iota-framework/table#0x2_table_Table">table::Table</Link>&lt;u64, Element&gt;
</dt>
<dd>
...
Return a TableVec of size one containing element 
e.
...
Pop an element from the end of TableVec 
t.
Aborts if 
t is empty.
...
Drop a possibly non-empty TableVec 
t.
...
Swap the 
ith element of the TableVec 
t with the last element and then pop the TableVec.
