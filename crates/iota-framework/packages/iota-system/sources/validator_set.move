...
module iota_system::validator_set {
    ...
    public(package) fun get_committee_validator_ref_inner(
        ...
        get_committee_validator_ref(&self.active_validators, &self.committee_members, validator_address)
    }
    ...
    public(package) fun sum_committee_voting_power_by_addresses(vs: &vector<ValidatorV1>, committee_members: &vector<u64>, addresses: &vector<address>): u64 {
        ...
        addresses.do_ref!(|addr| {
            let validator = get_committee_validator_ref(vs, committee_members, *addr);
            sum = sum + validator.voting_power();
        });
        sum
    }
