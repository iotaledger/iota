...
module iota_system::rewards_distribution_tests {
    ...
    #[test]
    fun test_validator_subsidy_no_supply_change_with_tips() {
        set_up_iota_system_state();
        let mut scenario_val = test_scenario::begin(VALIDATOR_ADDR_1);
        let scenario = &mut scenario_val;
        let prev_supply = total_supply(scenario);
    ...
    #[test]
    fun test_validator_subsidy_deflation_with_tips() {
        set_up_iota_system_state();
        let mut scenario_val = test_scenario::begin(VALIDATOR_ADDR_1);
        let scenario = &mut scenario_val;
        let prev_supply = total_supply(scenario);
        ...
        let new_supply = total_supply(scenario);
        // The difference between validator subsidy and computation charges burned should have been burned.
        assert_eq(prev_supply - (computation_charge_burned - validator_subsidy) * NANOS_PER_IOTA, new_supply);
    ...
    #[test]
    fun test_validator_subsidy_inflation_with_tips() {
        set_up_iota_system_state();
        let mut scenario_val = test_scenario::begin(VALIDATOR_ADDR_1);
        let scenario = &mut scenario_val;
        let prev_supply = total_supply(scenario);
        ...
        let validator_subsidy = 100;
        let computation_charge = 60;
        let computation_charge_burned = 50;
    ...
    fun test_validator_subsidy_higher_than_computation_charge_with_tips() {
        ...
        let validator_subsidy = 800;
        let computation_charge = 400;
        let computation_charge_burned = 300;

        advance_epoch_with_amounts(validator_subsidy, 0, computation_charge, computation_charge_burned, scenario);

        let expected_total_reward_amount = validator_subsidy + computation_charge - computation_charge_burned;
        ...
        assert_validator_total_stake_amounts(
            validator_addrs(),
            vector[
              (100 + expected_total_reward_amount/4) * NANOS_PER_IOTA,
              (200 + expected_total_reward_amount/4) * NANOS_PER_IOTA,
              (300 + expected_total_reward_amount/4) * NANOS_PER_IOTA,
              (400 + expected_total_reward_amount/4) * NANOS_PER_IOTA,
            ],
            scenario
        );
        ...
        // Validator should get the entire reward of 200 plus its initially staked 100 IOTA.
        assert_eq(total_iota_balance(VALIDATOR_ADDR_1, scenario), (100+expected_total_reward_amount/4) * NANOS_PER_IOTA);
    ...
    fun test_validator_subsidy_lower_than_computation_charge_with_tips() {
        ...
        let validator_subsidy = 800;
        let computation_charge = 1000;
        let computation_charge_burned = 900;
