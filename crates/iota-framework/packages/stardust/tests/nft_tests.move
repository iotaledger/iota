...
fun nft_assets_extraction() {
    ...
    assert!(
        native_tokens.borrow<ascii::String, Balance<TEST_A>>(type_name::get<TEST_A>().into_string()).value() == 100,
        1,
    );
    assert!(
        native_tokens.borrow<ascii::String, Balance<TEST_B>>(type_name::get<TEST_B>().into_string()).value() == 200,
        2,
    );
    ...
    assert!(
        nft.immutable_metadata().uri() == url::new_unsafe(ascii::string(b"www.best-nft.com/nft.png")),
        9,
    );
    ...
    assert!(
        nft.immutable_metadata().royalties().get(&sender) == fixed_point32::create_from_rational(1, 2),
        13,
    );
    ...
    let coin_a = coin::from_balance(
        native_tokens.remove<ascii::String, Balance<TEST_A>>(type_name::get<
            TEST_A,
        >().into_string()),
        scenario.ctx(),
    );
    let coin_b = coin::from_balance(
        native_tokens.remove<ascii::String, Balance<TEST_B>>(type_name::get<
            TEST_B,
        >().into_string()),
        scenario.ctx(),
    );
