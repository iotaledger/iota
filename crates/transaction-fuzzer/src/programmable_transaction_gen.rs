...
prop_compose! {
    pub fn gen_transfer()
        ...
        (args in vec(gen_argument(), x..=x), arg_to in gen_argument()) -> Command {
                Command::transfer_objects(args, arg_to)
    }
}
...
prop_compose! {
    pub fn gen_split_coins()
        (x in arg_len_strategy())
        (args in vec(gen_argument(), x..=x), arg_to in gen_argument()) -> Command {
                Command::split_coins(arg_to, args)
    }
}
...
prop_compose! {
    pub fn gen_merge_coins()
        ...
        (args in vec(gen_argument(), x..=x), arg_from in gen_argument()) -> Command {
                Command::merge_coins(arg_from, args)
    }
}
...
prop_compose! {
    pub fn gen_move_vec()
        ...
        (args in vec(gen_argument(), x..=x)) -> Command {
                Command::make_move_vector(None, args)
    }
}
...
pub fn gen_transfer_input(
    ...
    let next_cmd = Command::transfer_objects(coins, builder.pure(recipient).unwrap());
    (next_cmd, cmd_inc)
}
...
pub fn gen_split_coins_input(
    ...
    let next_cmd = Command::split_coins(coin_arg, split_args);
    (next_cmd, cmd_inc)
}
...
pub fn gen_merge_coins_input(
    ...
    let next_cmd = Command::merge_coins(output_coin, coins);
    (next_cmd, cmd_inc)
}
...
pub fn gen_move_vec_input(
    ...
    let next_cmd = Command::make_move_vector(None, coins);
    (next_cmd, cmd_inc)
}
