...
//! default, this is controlled by env_filter and logging levels.
...
impl PrometheusSpanLatencyLayer {
    ...
    /// The num_buckets controls how many buckets thus how much memory and time
    /// series one uses up in Prometheus (and in the application).  10 is
    /// probably a minimum.
    pub fn try_new(registry: &Registry, num_buckets: usize) -> Result<Self, PrometheusSpanError> {
        ...
        // Histogram for span latencies must accommodate a wide range of possible
        // latencies, so don't use the default Prometheus buckets.  Latencies in
        // NS.  Calculate the multiplier to go from LOWEST to TOP in num_bucket
        // steps, step n+1 = step n * factor.
    ...
    S: Subscriber + for<'span> tracing_subscriber::registry::LookupSpan<'span>,
{
    fn on_new_span(
        ...
        // tracing_subscriber's with_span_events() inserts events at open and close that
        // contain timings. However, we cannot be guaranteed that those events
        // would be turned on.
        span.extensions_mut()
