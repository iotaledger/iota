...
async fn test_get_owned_objects() -> Result<(), anyhow::Error> {
    ...
    // TODO: right now we compare the results from indexer and fullnode, but as we
    // deprecate fullnode rpc, we should change this to compare the results with
    // the object id/digest from genesis potentially.
