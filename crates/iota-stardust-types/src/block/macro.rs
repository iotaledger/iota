...
macro_rules! impl_id {
    ($vis:vis $name:ident, $length:literal, $doc:literal) => {
        ...
        impl $crate::block::ConvertTo<$name> for &alloc::string::String {
            fn convert(self) -> Result<$name, $crate::block::Error> {
                self.try_into()
            }
        }

        impl $crate::block::ConvertTo<$name> for &str {
            fn convert(self) -> Result<$name, $crate::block::Error> {
                self.try_into()
            }
        }
