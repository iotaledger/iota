...
pub use self::{
    migrated_funds_entry::MigratedFundsEntry, tail_transaction_hash::TailTransactionHash,
};
...
impl ReceiptMilestoneOption {
    ...
    /// Returns the milestone index at which the funds of a
    /// [`ReceiptMilestoneOption`] were migrated at in the legacy network.
    pub fn migrated_at(&self) -> MilestoneIndex {
        self.migrated_at
    }

    /// Returns whether a [`ReceiptMilestoneOption`] is the final one for a
    /// given migrated at index.
    ...
    pub fn transaction(&self) -> &TreasuryTransactionPayload {
        if let Payload::TreasuryTransaction(ref transaction) = self.transaction {
            ...
            // It has already been validated at construction that `transaction` is a
            // `TreasuryTransactionPayload`.
            unreachable!()
        }
    }

    /// Returns the sum of all [`MigratedFundsEntry`] items within a
...
fn verify_funds<const VERIFY: bool>(
    ...
    if VERIFY {
        ...
        for (index, funds) in funds.iter().enumerate() {
            if let Some(previous) =
                tail_transaction_hashes.insert(funds.tail_transaction_hash().as_ref(), index)
            {
            ...
            funds_sum = funds_sum.checked_add(funds.amount()).ok_or_else(|| {
                Error::InvalidReceiptFundsSum(funds_sum as u128 + funds.amount() as u128)
            })?;
            ...
            Ok(if let Some(token_supply) = params.token_supply() {
                ...
                Self {
                    ...
                    funds: VecPrefix::<MigratedFundsEntry, ReceiptFundsCount>::try_from(funds)
                        .map_err(Error::InvalidReceiptFundsCount)?,
                    transaction: if let PayloadDto::TreasuryTransaction(transaction) =
                        dto.transaction
                    {
                        TreasuryTransactionPayload::try_from_dto_with_params(*transaction, &params)?
                            .into()
