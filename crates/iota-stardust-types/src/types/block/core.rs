...
impl BlockBuilder {
    ...
    /// Finishes the [`BlockBuilder`] into a [`Block`], computing the nonce with
    /// a given provider.
    pub fn finish_nonce<F: Fn(&[u8]) -> Option<u64>>(
        self,
        nonce_provider: F,
    ) -> Result<Block, Error> {
        ...
        block.nonce =
            nonce_provider(&block_bytes[..block_bytes.len() - core::mem::size_of::<u64>()])
...
pub struct Block {
    ...
    payload: OptionalPayload,
    /// The result of the Proof of Work in order for the block to be accepted
    /// into the tangle.
    nonce: u64,
}
...
impl Block {
    ...
    /// Unpacks a [`Block`] from a sequence of bytes doing syntactical checks
    /// and verifying that there are no trailing bytes in the sequence.
    pub fn unpack_strict<T: AsRef<[u8]>>(
        ...
        // When parsing the block is complete, there should not be any trailing bytes
        // left that were not parsed.
        ...
        fn try_from_dto_with_params_inner(
            ...
            let mut builder = BlockBuilder::new(parents)
                ...
                .with_nonce(
                    dto.nonce
                        .parse::<u64>()
                        .map_err(|_| Error::InvalidField("nonce"))?,
                );
