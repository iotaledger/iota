...
//! The parents module defines the core data type for storing the blocks
//! directly approved by a block.
...
pub(crate) type ParentCount =
    BoundedU8<{ *Parents::COUNT_RANGE.start() }, { *Parents::COUNT_RANGE.end() }>;

/// A [`Block`](crate::types::block::Block)'s [`Parents`] are the [`BlockId`]s
/// of the blocks it directly approves.
///
/// Parents must be:
...
pub struct Parents(
    #[packable(verify_with = verify_parents)] BoxedSlicePrefix<BlockId, ParentCount>,
);
