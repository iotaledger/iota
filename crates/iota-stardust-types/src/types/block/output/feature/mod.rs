...
pub use self::{
    issuer::IssuerFeature, metadata::MetadataFeature, sender::SenderFeature, tag::TagFeature,
};
pub(crate) use self::{metadata::MetadataFeatureLength, tag::TagFeatureLength};
use crate::types::block::{Error, create_bitflags};
...
pub struct Features(
    #[packable(verify_with = verify_unique_sorted)] BoxedSlicePrefix<Feature, FeatureCount>,
);
...
impl Features {
    ...
    pub fn from_vec(features: Vec<Feature>) -> Result<Self, Error> {
        let mut features =
            BoxedSlicePrefix::<Feature, FeatureCount>::try_from(features.into_boxed_slice())
...
pub(crate) fn verify_allowed_features(
    features: &Features,
...
pub mod dto {
    ...
    pub use self::{
        issuer::dto::IssuerFeatureDto, metadata::dto::MetadataFeatureDto,
        sender::dto::SenderFeatureDto, tag::dto::TagFeatureDto,
    };
    use super::*;
        ...
        fn deserialize<D: serde::Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
            ...
            Ok(
                match value
                    ...
                    .ok_or_else(|| serde::de::Error::custom("invalid feature type"))?
                    as u8
                {
                    SenderFeature::KIND => {
                        Self::Sender(SenderFeatureDto::deserialize(value).map_err(|e| {
                            serde::de::Error::custom(format!(
                                "cannot deserialize sender feature: {e}"
                            ))
                        })?)
                    }
                    IssuerFeature::KIND => {
                        Self::Issuer(IssuerFeatureDto::deserialize(value).map_err(|e| {
                            serde::de::Error::custom(format!(
                                "cannot deserialize issuer feature: {e}"
                            ))
                        })?)
                    }
                    MetadataFeature::KIND => {
                        Self::Metadata(MetadataFeatureDto::deserialize(value).map_err(|e| {
                            serde::de::Error::custom(format!(
                                "cannot deserialize metadata feature: {e}"
                            ))
                        })?)
                    }
                    TagFeature::KIND => {
                        Self::Tag(TagFeatureDto::deserialize(value).map_err(|e| {
                            serde::de::Error::custom(format!("cannot deserialize tag feature: {e}"))
                        })?)
                    }
    ...
    impl TryFrom<FeatureDto> for Feature {
        ...
        fn try_from(value: FeatureDto) -> Result<Self, Self::Error> {
            Ok(match value {
                FeatureDto::Sender(v) => {
                    Self::Sender(SenderFeature::new(Address::try_from(v.address)?))
                }
                FeatureDto::Issuer(v) => {
                    Self::Issuer(IssuerFeature::new(Address::try_from(v.address)?))
                }
