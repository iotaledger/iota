...
use iota_stardust_types::types::block::{Error, payload::milestone::option::TailTransactionHash};
use packable::{PackableExt, error::UnpackError};
use pretty_assertions::assert_eq;

const HASH_TRYTES: &str =
    "TZTXLMTAURX9DYQICXZEUMCDBPNXVOHNBBZDSSVCNCTWKSMUALAYPMHUCKGOGSTBUHSQIMSY9HQEP9AXJ";
const HASH_BYTES: [u8; 49] = [
    222, 235, 107, 67, 2, 173, 253, 93, 165, 90, 166, 45, 102, 91, 19, 137, 71, 146, 156, 180, 248,
    31, 56, 25, 68, 154, 98, 100, 64, 108, 203, 48, 76, 75, 114, 150, 34, 153, 203, 35, 225, 120,
    194, 175, 169, 207, 80, 229, 10,
];
...
fn as_ref_valid() {
    assert_eq!(
        TailTransactionHash::try_from(HASH_BYTES).unwrap().as_ref(),
        HASH_BYTES
    );
}
...
fn to_str_valid() {
    assert_eq!(
        TailTransactionHash::try_from(HASH_BYTES)
            .unwrap()
            .to_string(),
        HASH_TRYTES
    );
