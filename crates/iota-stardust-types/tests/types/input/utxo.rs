...
const TRANSACTION_ID_INVALID: &str =
    "0x52fdfc072182654f163f5f0f9a621d729566c74d10037c4d7bbb0407d1e2c6";
...
fn from_str_to_str() {
    assert_eq!(
        UtxoInput::from_str(OUTPUT_ID).unwrap().to_string(),
        OUTPUT_ID
    );
}
...
fn pack_unpack() {
    ...
    assert_eq!(
        input,
        Input::unpack_verified(packed_input.as_slice(), &()).unwrap()
    );
}
