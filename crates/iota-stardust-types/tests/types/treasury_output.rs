...
use iota_stardust_types::types::block::{
    Error, output::TreasuryOutput, protocol::protocol_parameters,
};
...
fn invalid_more_than_max_amount() {
    assert!(matches!(
        ...
        Err(Error::InvalidTreasuryOutputAmount(
            3_038_287_259_199_220_266
        ))
    ));
}
...
fn pack_unpack_valid() {
    ...
    let output_2 =
        TreasuryOutput::unpack_verified(output_1.pack_to_vec().as_slice(), &protocol_parameters())
            .unwrap();
