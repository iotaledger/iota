...
use iota_stardust_types::types::block::{
    ...
    payload::{
        ...
        milestone::{
            MilestoneEssence, MilestoneIndex, MilestoneOption, MilestoneOptions,
...
fn getters() {
    ...
    let binary_parameters = rand_bytes(rand_number_range(
        ParametersMilestoneOption::BINARY_PARAMETERS_LENGTH_RANGE,
    ) as usize);
    ...
    assert_eq!(
        milestone_payload.previous_milestone_id(),
        &previous_milestone_id
    );
    assert_eq!(*milestone_payload.parents(), parents);
    assert_eq!(
        *milestone_payload.inclusion_merkle_root(),
        inclusion_merkle_root
    );
    assert_eq!(
        *milestone_payload.applied_merkle_root(),
        applied_merkle_root
    );
    assert_eq!(
        milestone_payload
