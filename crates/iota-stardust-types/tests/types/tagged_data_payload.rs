...
fn new_valid_tag_length_min() {
    let payload =
        TaggedDataPayload::new(Vec::new(), [0x42, 0xff, 0x84, 0xa2, 0x42, 0xff, 0x84, 0xa2])
            .unwrap();
...
#[test]
fn packed_len() {
    let tagged_data = TaggedDataPayload::new(
        rand_bytes(10),
        [0x42, 0xff, 0x84, 0xa2, 0x42, 0xff, 0x84, 0xa2],
    )
    .unwrap();
...
fn pack_unpack_valid() {
    let tagged_data_1 = TaggedDataPayload::new(
        rand_bytes(32),
        [0x42, 0xff, 0x84, 0xa2, 0x42, 0xff, 0x84, 0xa2],
    )
    .unwrap();
    let tagged_data_2 =
        TaggedDataPayload::unpack_verified(tagged_data_1.pack_to_vec().as_slice(), &()).unwrap();
...
fn unpack_valid_tag_length_min() {
    let payload =
        TaggedDataPayload::unpack_verified([0x00, 0x00, 0x00, 0x00, 0x00, 0x00].as_slice(), &())
            .unwrap();
