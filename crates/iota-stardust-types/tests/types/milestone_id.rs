...
use iota_stardust_types::types::block::payload::milestone::MilestoneId;
...
fn from_to_str() {
    assert_eq!(
        MILESTONE_ID,
        MilestoneId::from_str(MILESTONE_ID).unwrap().to_string()
    );
}

// Validate that the length of a packed `MilestoneId` matches the declared
// `packed_len()`.
#[test]
fn packed_len() {
...
// Validate that a `unpack` ∘ `pack` round-trip results in the original
// milestone id.
#[test]
