...
use iota_stardust_types::types::block::{
    input::TreasuryInput, output::TreasuryOutput, payload::TreasuryTransactionPayload,
...
fn pack_unpack_valid() {
    ...
    let transaction_2 = TreasuryTransactionPayload::unpack_verified(
        transaction_1.pack_to_vec().as_slice(),
        &protocol_parameters,
    )
    .unwrap();
...
fn pack_unpack_invalid() {
    let bytes = [
        1, 82, 253, 252, 7, 33, 130, 101, 79, 22, 63, 95, 15, 154, 98, 29, 114, 149, 102, 199, 77,
        16, 3, 124, 77, 123, 187, 4, 7, 209, 226, 198, 73, // Faulty byte here ->
        0, 232, 3, 0, 0, 0, 0, 0, 0,
    ];

    // Actual error is not checked because the output type check is done after the
    // output is parsed so the error is not `InvalidOutputKind` but something
    // related to an invalid address, so not really relevant for this test.
    assert!(
        TreasuryTransactionPayload::unpack_verified(bytes.as_slice(), &protocol_parameters())
            .is_err()
    );
}
