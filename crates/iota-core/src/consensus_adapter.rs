...
impl ConsensusAdapter {
    ...
    pub fn submit_batch(
        ...
        if transactions.len() > 1 {
            // In soft bundle, we need to check if all transactions are of
            // CertifiedTransaction kind. The check is required because we
            // assume this in submit_and_wait_inner.
    ...
    /// executed via a checkpoint (state sync). Returns the processed
    /// method, whether the transactions have been processed via consensus, or
    /// have been synced via checkpoint.
