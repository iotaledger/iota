...
/// floor.
///
/// A column family left for auto-discovery would be reopened with default
/// options and a block cache of its own, so every one already on disk under
/// `db_path` is named here. A path with no database yet, or one whose column
/// families cannot be listed, yields the retention floor alone — the store's
/// own open then either creates the database fresh or fails on the same
/// listing problem.
...
/// Reads a bucket whose column family may have been dropped since the walk
/// took its handle.
///
...
/// A dropped bucket is an epoch this node no longer serves, so its rows are
/// absent rather than an error — and the infallible read wrappers above these
/// walks would otherwise abort the node.
