...
// Pipeline system for coordinating concurrent, pipelined execution without
// having to break code down into a one-stage-per-function style.
//
// Pipeline concurrency can be achieved when you have the following sort of
...
// checkpoint sequence number. But there is no requirement that we commit
// outputs for seq i before we begin executing transactions for seq i+1.
//
// The code here allows you to write pipelined executions without breaking up
// every stage into its own function. Instead you can write monolithic functions
// like:
//
...
//       }
//
// Then, simply run many instances of this function concurrently, and the
// pipeline ordering will be maintained.
//
// Currently this code is not generic with respect to the names of the stages.
// We can make the stage enum a generic param when we need to use this code in
// other places.
...
/// PipelineHandle is used to coordinate one iteration (sequence number) of the
/// pipeline, starting from the first stage and ending at the last stage.
...
/// Cannot use this check in the simulator because it drops tasks to simulate
/// killing nodes.
#[cfg(not(msim))]
