...
impl HistoryBucket {
    fn reopen(db: &Arc<Database>, epoch: EpochId) -> Result<Self, TypedStoreError> {
        ...
        Ok(Self {
            transaction_order: map(db, &cf, 0)?,
            transactions_seq: map(db, &cf, 1)?,
            transactions_from_addr: map(db, &cf, 2)?,
            transactions_to_addr: map(db, &cf, 3)?,
            transactions_by_input_object_id: map(db, &cf, 4)?,
            transactions_by_mutated_object_id: map(db, &cf, 5)?,
            transactions_by_move_function: map(db, &cf, 6)?,
            event_order: map(db, &cf, 7)?,
            event_by_move_module: map(db, &cf, 8)?,
            event_by_move_event: map(db, &cf, 9)?,
            event_by_event_module: map(db, &cf, 10)?,
            event_by_sender: map(db, &cf, 11)?,
            event_by_time: map(db, &cf, 12)?,
        })
    }
