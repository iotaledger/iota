...
mod tests {
    ...
    use iota_sdk_types::{
        Address, CheckpointSummary, GasCostSummary, ObjectDigest, ObjectId, Owner, StructTag,
    ...
    /// Writes one transaction's owner and dynamic-field rows the way
    /// `index_checkpoint` does, so the cursor tests below page over rows the
    /// indexing path itself produced.
    fn index_object_changes(index_store: &IndexStore, changes: ObjectIndexChanges) {
        let mut batch = index_store.tables.owner_index.batch();
        let mut coin_changes = BTreeMap::new();
        index_store
            .tables
            .index_object_changes(
                &mut batch,
                &mut coin_changes,
    ...
    /// The `cursor` of `get_dynamic_field_ids_iterator` is exclusive: the
    /// cursor row itself is never returned again, and when the cursor row
    /// no longer exists (the field was removed from its parent or
    /// transferred between pages) the scan resumes at the next live row
    /// without skipping it.
    #[tokio::test]
    async fn get_dynamic_field_ids_iterator_cursor_is_exclusive() {
        let tmp_dir = iota_common::tempdir();
        let index_store = IndexStore::new_without_init(
            tmp_dir.path().to_path_buf(),
            &Registry::default(),
        ...
        let new_dynamic_fields = field_ids.iter().map(|&id| (parent, id)).collect();
        index_object_changes(
            &index_store,
            ObjectIndexChanges {
                deleted_owners: vec![],
    ...
    /// Walking `get_dynamic_field_ids_iterator` one field at a time, feeding
    ...
    #[tokio::test]
    async fn get_dynamic_field_ids_iterator_walks_every_field() {
        let tmp_dir = iota_common::tempdir();
        let index_store = IndexStore::new_without_init(
            tmp_dir.path().to_path_buf(),
            &Registry::default(),
        ...
        let mut new_dynamic_fields: Vec<_> = field_ids.iter().map(|&id| (parent, id)).collect();
        new_dynamic_fields.extend(other_field_ids.iter().map(|&id| (other_parent, id)));
