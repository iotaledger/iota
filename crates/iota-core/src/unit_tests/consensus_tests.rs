...
async fn submit_transaction_to_consensus_adapter() {
    ...
    let block_status_receivers = vec![
        with_block_status(BlockStatus::GarbageCollected(starfish_core::GenericTransactionRef::BlockRef(BlockRef::MIN))),
        with_block_status(BlockStatus::GarbageCollected(starfish_core::GenericTransactionRef::BlockRef(BlockRef::MIN))),
        with_block_status(BlockStatus::GarbageCollected(starfish_core::GenericTransactionRef::BlockRef(BlockRef::MIN))),
        with_block_status(BlockStatus::Sequenced(starfish_core::GenericTransactionRef::BlockRef(BlockRef::MIN))),
    ];
...
async fn submit_recovered_end_of_publish_crash_recovery() {
    ...
    impl ConsensusClient for RecordingClient {
        ...
        async fn submit(
            ...
            Ok(with_block_status(BlockStatus::Sequenced(starfish_core::GenericTransactionRef::BlockRef(BlockRef::MIN))))
        }
    }
