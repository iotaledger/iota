...
fn get_parent_and_child(
    ...
    // of another object.
    let created_addrs: HashSet<_> = created.iter().map(|(oref, _)| oref.object_id).collect();
    ...
    transfer_test_runner! { |mut runner: TestRunner| async move {
        ...
        for (obj_ref, owner) in effects.mutated().iter() {
            if obj_ref.object_id == child.0.object_id {
                ...
                assert_eq!(owner, &Owner::AddressOwner(Address::ZERO));
...
async fn test_tto_intersection_input_and_receiving_objects() {
    transfer_test_runner! { |mut runner: TestRunner| async move {
        ...
        assert!(matches!(error, UserInputError::DuplicateObjectReferenceInput));
    ...
    transfer_test_runner! { |mut runner: TestRunner| async move {
        ...
        let mutations: Vec<(
            ...
            (
                ...
                Box::new(|err| matches!(err, UserInputError::InvalidVersion)),
            ),
            (
            ...
            (
                Box::new(|x: ObjectReference| ObjectReference::new(x.object_id, x.version + 1, x.digest)),
                Box::new(|err| {
                    matches!(
                        err,
            ...
            (
                Box::new(|x: ObjectReference| ObjectReference::new(x.object_id, x.version, ObjectDigest::new(rand::random()))),
            ...
            .run_with_gas_object(
                {
                    let mut builder = ProgrammableTransactionBuilder::new();
                    builder
                        .transfer_object(Address::from(parent.0.object_id), child.0)
                        .unwrap();
                    builder.finish()
                },
                1,
            ...
            .run_with_gas_object(
                {
                    let mut builder = ProgrammableTransactionBuilder::new();
                    builder
                        .transfer_object(Address::from(parent.0.object_id), old_child.0)
                        .unwrap();
                    builder.finish()
                },
                1,
...
async fn test_have_deleted_owned_object() {
    transfer_test_runner! { |mut runner: TestRunner| async move {
        ...
        let deleted_child = effects.deleted().into_iter().find(|oref| oref.object_id == new_child.0.object_id).unwrap();
        ...
        assert!(!cache.have_deleted_owned_object_at_version_or_after(&deleted_child.object_id, deleted_child.version + 1, 0));
