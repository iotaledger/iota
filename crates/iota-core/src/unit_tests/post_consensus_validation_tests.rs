...
async fn test_v2_attestor_mismatch() {
    ...
    let min_units = protocol_config
        .base_tx_cost_fixed()
        .min(protocol_config.gas_rounding_step());
    let mut transactions = vec![make_user_tx_v2(tx, 1, min_units - 1)];
...
async fn test_v2_cost_out_of_bounds() {
    ...
    let min_units = protocol_config
        .base_tx_cost_fixed()
        .min(protocol_config.gas_rounding_step());
    let ref_data = reference_tx.data().transaction();
    ...
    for (attested_units, expect_below) in [(min_units - 1, true), (max_units + 1, false)] {
        ...
        let digest = *tx.digest();
        let mut transactions = vec![make_user_tx_v2(tx, 0, attested_units)];
    ...
    let _guard = ProtocolConfig::apply_overrides_for_testing(move |_, mut config| {
        ...
        if let Some(bandwidth) = memory_bandwidth {
            config.set_gas_vector_coefficients_for_testing(GasVectorCoefficientsV1 {
                memory_bandwidth_bytes_per_sec: bandwidth,
                ..Default::default()
            });
        }
        config
