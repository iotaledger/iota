...
use iota_sdk_types::{
    Address, Command, ExecutionError, ExecutionStatus, MoveAuthenticator, MoveAuthenticatorV1,
    ObjectId, Transaction, UserSignature,
};
use iota_types::{
...
async fn structural_move_auth_failure_resolves_to_failure_effect() {
    ...
    let tx = TransactionEnvelope::from_user_sig_data(tx_data, vec![authenticator]);
