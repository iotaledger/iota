...
// End-to-End White Flag Flow Tests

#[sim_test]
async fn test_white_flag_flow_end_to_end_single_transaction() {
    telemetry_subscribers::init_for_testing();
    ...
    let verified_tx = init_transfer_transaction(
        &authority,
        sender,
        &sender_key,
        recipient,
    ...
    // Wrap in UserTransactionV1 and process through consensus
    let consensus_tx = ConsensusTransaction {
        kind: ConsensusTransactionKind::UserTransactionV1(Box::new(verified_tx.into())),
        tracking_id: Default::default(),
    };

    let sequenced_txs = vec![SequencedConsensusTransaction::new_test(consensus_tx)];
    ...
    // Verify transaction is executable
    assert_eq!(
        executable_txs.len(),
        1,
        "Should have one executable transaction"
    );
    assert_eq!(
        executable_txs[0].inner().transaction_data().digest(),
        tx_digest,
        "Executable transaction digest should match"
    );

    // The transaction went through consensus and is now executable
    // We could execute it here using execute_certificate or
    // send_and_confirm_transaction but the key point is that it made it
    // through the white flag flow
}

#[sim_test]
async fn test_white_flag_flow_end_to_end_conflict_resolution() {
    telemetry_subscribers::init_for_testing();
    ...
    let tx1_digest = *verified_tx1.digest();
    ...
    let verified_tx2 = init_transfer_transaction(
        ...
        &sender_key,
        recipient2,
        object.compute_object_reference(), // Same object - conflict!
        gas2.compute_object_reference(),
        rgp * TEST_ONLY_GAS_UNIT_FOR_TRANSFER,
        rgp,
    );
    let _tx2_digest = *verified_tx2.digest();

    // Wrap both in UserTransactionV1 and process through consensus
    let consensus_tx1 = ConsensusTransaction {
        kind: ConsensusTransactionKind::UserTransactionV1(Box::new(verified_tx1.into())),
        tracking_id: Default::default(),
    };
    let consensus_tx2 = ConsensusTransaction {
        kind: ConsensusTransactionKind::UserTransactionV1(Box::new(verified_tx2.into())),
        tracking_id: Default::default(),
    };
    ...
    // Verify: Only first transaction is executable (second was dropped due to
    // conflict)
    assert_eq!(
        executable_txs.len(),
        1,
        "Only first transaction should be executable"
    );
    assert_eq!(
        executable_txs[0].inner().transaction_data().digest(),
        tx1_digest,
        "First transaction should be executable"
    );

    // Verify the object is locked by tx1 (tx2 was dropped)
    let lock = get_object_lock(&epoch_store, &object.compute_object_reference());
    assert_eq!(
        lock,
        Some(tx1_digest),
        "Object should be locked by first transaction"
    );

    // The key point of this test is that:
    // 1. Both transactions went through consensus (in order: tx1, tx2)
    // 2. White flag conflict resolution detected the conflict
    // 3. Only tx1 is executable, tx2 was dropped
}

#[sim_test]
async fn test_white_flag_consensus_signature_verification() {
    telemetry_subscribers::init_for_testing();
    ...
    // Sign with wrong key
    let invalid_tx = to_sender_signed_transaction(tx_data, &wrong_key);

    // Try to verify the transaction - should fail
    let verify_result = epoch_store.verify_transaction(invalid_tx);
