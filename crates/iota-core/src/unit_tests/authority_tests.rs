...
async fn test_dry_run_transaction_block() {
    ...
    let txn_data = Transaction::new_with_gas_coins(
        tx.kind().clone(),
        tx.sender(),
        vec![],
        tx.gas_budget(),
...
async fn test_handle_sponsored_transaction() {
    ...
    let data = Transaction::new_with_gas_data(
        tx_kind.clone(),
        sender,
        GasPayment {
...
async fn test_move_call_insufficient_gas() {
    ...
    // required.
    let data = Transaction::new_transfer(sender, obj_ref, recipient, gas_ref, gas_used - 5, rgp);
