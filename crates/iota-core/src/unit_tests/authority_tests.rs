...
async fn test_dry_run_dev_inspect_dynamic_field_too_new() {
    ...
    let kind = TransactionKind::new_programmable_transaction(pt.clone());
...
async fn test_invalid_authenticator_state_parameter() {
    ...
    let tx_data = TransactionData::new_move_call(
        ...
        vec![],
        gas_ref,
        vec![CallArg::Shared(iota_sdk_types::SharedObjectReference::new(
            ObjectID::AUTHENTICATOR_STATE,
            iota_sdk_types::Version::INITIAL_SHARED_VERSION,
            true,
        ))],
        TEST_ONLY_GAS_UNIT_FOR_OBJECT_BASICS * rgp,
        rgp,
...
pub async fn call_dev_inspect(
    ...
    let kind = TransactionKind::new_programmable_transaction(builder.finish());
    let rgp = authority.reference_gas_price_for_testing().unwrap();
    authority
        .dev_inspect_transaction_block(*sender, kind, Some(rgp), None, None, None, None, None)
        .await
