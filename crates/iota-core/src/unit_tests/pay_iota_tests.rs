...
async fn test_pay_iota_failure_empty_recipients() {
    ...
    let coin_id = ObjectId::new(rand::random());
...
async fn test_pay_iota_failure_insufficient_gas_balance_one_input_coin() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let coin1 = Object::with_id_owner_gas_for_testing(ObjectId::new(rand::random()), sender, 2000);
...
async fn test_pay_iota_failure_insufficient_total_balance_one_input_coin() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let coin1 =
        Object::with_id_owner_gas_for_testing(ObjectId::new(rand::random()), sender, 1000100);
    let recipient1 = dbg_addr(1);
    let recipient2 = dbg_addr(2);
...
async fn test_pay_iota_failure_insufficient_gas_balance_multiple_input_coins() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let coin1 = Object::with_id_owner_gas_for_testing(ObjectId::new(rand::random()), sender, 800);
    let coin2 = Object::with_id_owner_gas_for_testing(ObjectId::new(rand::random()), sender, 700);
    let recipient1 = dbg_addr(1);
    let recipient2 = dbg_addr(2);
...
async fn test_pay_iota_failure_insufficient_total_balance_multiple_input_coins() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let coin1 =
        Object::with_id_owner_gas_for_testing(ObjectId::new(rand::random()), sender, 404000);
    let coin2 =
        Object::with_id_owner_gas_for_testing(ObjectId::new(rand::random()), sender, 603000);
    let recipient1 = dbg_addr(1);
    let recipient2 = dbg_addr(2);
...
async fn test_pay_iota_success_multiple_input_coins() -> anyhow::Result<()> {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let object_id1 = ObjectId::new(rand::random());
    let object_id2 = ObjectId::new(rand::random());
    let object_id3 = ObjectId::new(rand::random());
...
async fn test_pay_all_iota_failure_insufficient_gas_one_input_coin() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let coin1 = Object::with_id_owner_gas_for_testing(ObjectId::new(rand::random()), sender, 1800);
...
async fn test_pay_all_iota_failure_insufficient_gas_budget_multiple_input_coins() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let coin1 = Object::with_id_owner_gas_for_testing(ObjectId::new(rand::random()), sender, 1000);
    let coin2 = Object::with_id_owner_gas_for_testing(ObjectId::new(rand::random()), sender, 1000);
    let recipient = dbg_addr(2);
    let res =
...
async fn test_pay_all_iota_success_multiple_input_coins() -> anyhow::Result<()> {
    ...
    let coin_obj2 =
        Object::with_id_owner_gas_for_testing(ObjectId::new(rand::random()), sender, 1000);
    let coin_obj3 =
        Object::with_id_owner_gas_for_testing(ObjectId::new(rand::random()), sender, 1000);
    let recipient = dbg_addr(2);
