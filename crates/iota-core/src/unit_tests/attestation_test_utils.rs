...
use iota_sdk_types::{ObjectReference, TransactionDigest, TransactionEffects};
use iota_types::{
...
impl AbstractAccountTestEnv {
    ...
    pub fn attest(&self, tx: &TransactionEnvelope) -> Attestation {
        ...
        let (payload, _) = self
            ...
            .attest_transaction(
                &verified,
                &epoch_store,
                &self.authority.config.transaction_deny_config,
            )
