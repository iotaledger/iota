...
/// Reopens `store` with the deny-rules object present, as the epoch after its
/// creation starts. The caller keeps a protocol config override alive for it.
fn reopen_with_deny_rules_object(
