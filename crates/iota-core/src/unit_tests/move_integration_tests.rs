...
async fn test_entry_point_vector_empty() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        let empty_vec = builder.command(Command::make_move_vector(None, vec![]));
        builder.programmable_move_call(
            package.object_id,
            Identifier::from_static("entry_point_vector"),
            Identifier::from_static("obj_vec_empty"),
...
async fn error_test_make_move_vec_for_type<T: Clone + Serialize>(
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    let arg = builder.pure(value.clone()).unwrap();
    builder.command(Command::make_move_vector(None, vec![arg]));
    let pt = builder.finish();
    let effects = execute_programmable_transaction(
        authority,
        gas,
...
async fn test_make_move_vec_empty() {
    ...
    let rgp = authority.reference_gas_price_for_testing().unwrap();
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.command(Command::make_move_vector(None, vec![]));
    let pt = builder.finish();
