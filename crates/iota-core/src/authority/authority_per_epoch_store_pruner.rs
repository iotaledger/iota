...
mod tests {
    ...
    async fn test_basic_epoch_pruner() {
        let parent_directory = iota_common::tempdir();
        let directories: Vec<_> = vec!["epoch_0", "epoch_1", "epoch_3", "epoch_4"]
            .into_iter()
            .map(|name| parent_directory.path().join(name))
            .collect();
        for directory in &directories {
            fs::create_dir(directory).expect("failed to create directory");
        }

        let pruned = AuthorityPerEpochStorePruner::prune_old_directories(
            &parent_directory.path().to_path_buf(),
            2,
        )
        .await
        .unwrap();
