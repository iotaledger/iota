...
impl AuthorityStorePruner {
    ...
    pub fn compact(perpetual_db: &Arc<AuthorityPerpetualTables>) -> Result<(), TypedStoreError> {
        perpetual_db.objects.compact_range(
            &ObjectKey(ObjectID::ZERO, SequenceNumber::MIN_VALID),
            &ObjectKey(ObjectID::MAX, SequenceNumber::MAX_VALID),
        )
    }
}
...
mod tests {
    ...
    async fn test_db_size_after_compaction() -> Result<(), anyhow::Error> {
        ...
        let start = ObjectKey(ObjectID::ZERO, SequenceNumber::MIN_VALID);
        let end = ObjectKey(ObjectID::MAX, SequenceNumber::MAX_VALID);
...
mod pprof_tests {
    ...
    fn read_keys(
        ...
        let mut i = 0;
        while i < num_reads {
            let _res = objects.get(&ObjectKey(ObjectID::random(), VersionNumber::MAX_VALID))?;
            i += 1;
        }
        Ok(())
    }
