...
mod tests {
    ...
    async fn test_pruning_objects() {
        ...
        let to_keep = run_pruner(path.path(), 3, 2, 1000).await;
        assert_eq!(
            HashSet::from_iter(to_keep),
            get_keys_after_pruning(path.path()).unwrap()
        );
    }
    ...
    async fn test_pruning_tombstones() {
        let path = iota_common::tempdir();
        let to_keep = run_pruner(path.path(), 0, 0, 1000).await;
        assert_eq!(to_keep.len(), 0);
        assert_eq!(get_keys_after_pruning(path.path()).unwrap().len(), 0);

        let path2 = iota_common::tempdir();
        let to_keep = run_pruner(path2.path(), 3, 0, 1000).await;
        assert_eq!(to_keep.len(), 0);
        assert_eq!(get_keys_after_pruning(path2.path()).unwrap().len(), 0);
    }

    #[cfg(not(target_env = "msvc"))]
    #[tokio::test]
    async fn test_db_size_after_compaction() -> Result<(), anyhow::Error> {
        let primary_path = iota_common::tempdir();
        let perpetual_db = Arc::new(AuthorityPerpetualTables::open(primary_path.path(), None));
        ...
        let db_path = primary_path.path().join("perpetual");
