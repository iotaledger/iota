...
mod object_cost_tests {
    ...
    #[test]
    fn congested_objects_version_assignments() {
        ...
        let obj_1 = ObjectID::random();
        let obj_2 = ObjectID::random();

        let max_execution_duration_per_commit = 1;

        let tx_1 = build_transaction(&[(obj_1, true)], 1, 1_000);
        ...
        let mut shared_input_next_versions = HashMap::from([(obj_1, 1.into())]);
        let cancelled_txns = BTreeMap::new();
        ...
        let congested_objects =
            if let SequencingResult::Defer(_, congested_objects) = sequencing_result {
                assert_eq!(congested_objects, vec![obj_1]);

                congested_objects
            } else {
                panic!("tx 2 must be deferred");
            };
        ...
        assert_eq!(
            assigned_versions,
            vec![
                (obj_1, SequenceNumber::CONGESTED),
                (obj_2, SequenceNumber::CANCELLED_READ)
            ]
        );
    }
}
