...
| **Requirement****                                                      | **Metrics**                                                                                                                                        | **Thresholds**                                                   |
| ---------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
...
| **2. Checkpoints batch commit latency**                                | Time in `min` where `latency > 0`<br><br>`latency` is defined by `checkpoint_db_commit_latency`                                                    | - **Genesis**: `> 35 min` <br><br>- **Post Genesis**: `> 1 min`  |
| **3. Checkpoint chunked transactions commit latency**                  | Time in `sec` where `latency > 0`<br><br>`latency` is defined by `checkpoint_db_commit_latency_transactions_chunks`                                | - **Genesis**: `> 5 min` <br><br>- **Post Genesis**: `> 5 sec`   |
| **4. Checkpoint chunked transactions insertion orders commit latency** | Time in `sec` where `latency > 0`<br><br>`latency` is defined by `checkpoint_db_commit_latency_tx_insertion_order_chunks`                          | `> 10 sec`                                                       |
| **5. Checkpoint chunked transaction indices commit latency**           | Time in `sec` where `latency > 0`<br><br>`latency` is defined by `checkpoint_db_commit_latency_tx_insertion_order_chunks`                          | - **Genesis**: `> 10 min` <br><br>- **Post Genesis**: `> 10 sec` |
| **6. Checkpoint chunked events commit latency**                        | Time in `sec` where `latency > 0`<br><br>`latency` is defined by `checkpoint_db_commit_latency_events_chunks`                                      | `> 5 sec`                                                        |
| **7. Checkpoint chunked events indices commit latency**                | Time in `sec` where `latency > 0`<br><br>`latency` is defined by `checkpoint_db_commit_latency_event_indices_chunks`                               | `> 5 sec`                                                        |
| **8. Checkpoint chunked packages commit latency**                      | Time in `sec` where `latency > 0`<br><br>`latency` is defined by `checkpoint_db_commit_latency_packages`                                           | `> 5 sec`                                                        |
| **9. Checkpoint chunked objects commit latency**                       | Time in `sec` where `latency > 0`<br><br>`latency` is defined by `checkpoint_db_commit_latency_objects_chunks`                                     | - **Genesis**: `> 1 min` <br><br>- **Post Genesis**: `> 5 sec`   |
| **10. Checkpoint chunked objects history commit latency**              | Time in `sec` where `latency > 0`<br><br>`latency` is defined by `checkpoint_db_commit_latency_objects_history_chunks`                             | - **Genesis**: `> 1 min` <br><br>- **Post Genesis**: `> 5 sec`   |
| **11. Epoch commit latency**                                           | Time in `sec` where `latency > 0`<br><br>`latency` is defined by `checkpoint_db_commit_latency_epoch`                                              | `> 5 sec`                                                        |
| **12. Object snapshot chunks commit latency**                          | Time in `sec` where `latency > 0`<br><br>`latency` is defined by `checkpoint_db_commit_latency_objects_snapshot_chunks`                            | - **Genesis**: `> 3 min` <br><br>- **Post Genesis**: `> 5 sec`   |
| **13. Object snapshot commit latency**                                 | Time in `sec` where `latency > 0`<br><br>`latency` is defined by `checkpoint_db_commit_latency_objects_snapshot`                                   | - **Genesis**: `> 60 min` <br><br>- **Post Genesis**: `> 5 sec`  |
| **14. DB Connection Pool Size**                                        | Time in `sec` where `pool_size = 0`.<br><br>`pool_size` is defined as `db_conn_pool_size`                                                          | - **Genesis**: `> 10 sec` <br><br>- **Post Genesis**: `> 5 sec`  |
| **15. JSON RPC latency**                                               | Time in `sec` where `latency > 0`<br><br>`latency` is defined by `req_latency_by_route`                                                            | `> 1 second`<br>                                                 |
...
| `checkpoint_db_commit_latency`                           | Total latency to commit a batch of checkpoints (including all transactions, objects, events, etc.) to the database. |
