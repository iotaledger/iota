...
async fn main() -> Result<(), IndexerError> {
    ...
    let old_conf = OldIndexerConfig::command()
        .version(VERSION)
        .try_get_matches()
        .and_then(|mut arg_matches| OldIndexerConfig::from_arg_matches_mut(&mut arg_matches));
    ...
    if matches!(opts.command, Command::HelpDeprecated) {
