...
mod integration_tests {
    ...
    use crate::common::pg_integration::{
        get_global_indexer_rpc_client, get_global_node_rpc_client, rpc_call_error_msg_matches,
    };

    static ADDRESS: OnceCell<IotaAddress> = OnceCell::const_new();
    ...
    pub async fn indexer_wait_for_checkpoint(checkpoint_sequence_number: u64) {
        tokio::time::timeout(Duration::from_secs(30), async {
            while {
                let cp_opt = get_global_indexer_rpc_client()
                    .get_latest_checkpoint_sequence_number()
                    .await
                    .unwrap();
                cp_opt.into_inner() < checkpoint_sequence_number
            } {
    ...
    async fn get_address() -> &'static IotaAddress {
        ADDRESS
            .get_or_init(|| async {
                let cluster = get_global_node_rpc_client();
                ...
                let address = tx_response
                    .object_changes
                    .map(|a| {
                        a.into_iter()
                            .filter_map(|a| match a {
                                iota_json_rpc_types::ObjectChange::Created {
                                    owner: iota_types::object::Owner::AddressOwner(iota_address),
                                    ..
                                } => Some(iota_address),
                                _ => None,
                            })
    ...
    async fn get_object_with_options(options: IotaObjectDataOptions) {
        indexer_wait_for_checkpoint(1).await;

        let fullnode_objects = get_global_node_rpc_client()
            .get_owned_objects(
                *get_address().await,
        ...
        println!("{:?}", fullnode_objects.data);

        for obj in fullnode_objects.data {
            let indexer_obj = get_global_indexer_rpc_client()
    ...
    async fn multi_get_objects_with_options(options: IotaObjectDataOptions) {
        ...
        let indexer_objects = get_global_indexer_rpc_client()
    ...
    async fn get_transaction_block_with_options(options: IotaTransactionBlockResponseOptions) {
        ...
        let tx = get_global_indexer_rpc_client()
            .get_transaction_block(tx_digest, Some(options.clone()))
            .await
            .unwrap();
    ...
    async fn multi_get_transaction_blocks_with_options(
        options: IotaTransactionBlockResponseOptions,
    ) {
        indexer_wait_for_checkpoint(3).await;

        let cluster = get_global_node_rpc_client();

        let fullnode_checkpoints = cluster.get_checkpoints(None, Some(3), false).await.unwrap();
        ...
        let indexer_txs = get_global_indexer_rpc_client()
    ...
    async fn get_checkpoint_by_seq_num() {
        ...
        let fullnode_checkpoint = get_global_node_rpc_client()
            .get_checkpoint(CheckpointId::SequenceNumber(0))
            .await
            .unwrap();

        let indexer_checkpoint = get_global_indexer_rpc_client()
            .get_checkpoint(CheckpointId::SequenceNumber(0))
            .await
            .unwrap();
    ...
    async fn get_checkpoint_by_seq_num_not_found() {
        ...
        let result = get_global_indexer_rpc_client()
    ...
    async fn get_checkpoints_by_cursor_and_limit_ascending() {
        indexer_wait_for_checkpoint(6).await;
    ...
    async fn get_object() {
        ...
        let fullnode_objects = get_global_node_rpc_client()
            .get_owned_objects(*get_address().await, None, None, None)
            .await
            .unwrap();
    ...
    async fn multi_get_objects_found_and_not_found() {
        ...
        let fullnode_objects = get_global_node_rpc_client()
            .get_owned_objects(*get_address().await, None, None, Some(5))
            .await
            .unwrap();
    ...
    async fn get_events() {
        ...
        let events = get_global_indexer_rpc_client()
    ...
    async fn get_protocol_config() {
        ...
        let fullnode_protocol_config = get_global_node_rpc_client()
            .get_protocol_config(None)
            .await
            .unwrap();

        let indexer_client = get_global_indexer_rpc_client();
    ...
    async fn get_chain_identifier() {
        ...
        let fullnode_chain_identifier = get_global_node_rpc_client()
            .get_chain_identifier()
            .await
            .unwrap();

        let indexer_chain_identifier = get_global_indexer_rpc_client()
            .get_chain_identifier()
            .await
            .unwrap();
    ...
    //     indexer_wait_for_checkpoint( stop_after_checkpoint_seq).await;
