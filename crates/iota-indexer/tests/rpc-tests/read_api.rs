...
mod integration_tests {
    ...
    fn get_total_transaction_blocks() {
        ...
        api_test_setup.runtime.block_on(async move {
            indexer_wait_for_checkpoint(&api_test_setup.store, checkpoint).await;
            let total_transaction_blocks = api_test_setup
                .client
            ...
            assert!(
                total_transaction_blocks.into_inner()
                    >= fullnode_checkpoint.network_total_transactions
            );
            assert!(
                total_transaction_blocks.into_inner()
                    >= indexer_checkpoint.network_total_transactions,
            );
        });
    }
    ...
    fn get_latest_checkpoint_sequence_number() {
        ...
        api_test_setup.runtime.block_on(async move {
            ...
            let latest_checkpoint_seq_number = api_test_setup
                .client
            ...
            indexer_wait_for_checkpoint(&api_test_setup.store, checkpoint + 5).await;
