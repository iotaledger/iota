...
struct Args {
    ...
    /// keystore iota genesis -f --with-faucet generates a genesis config
    /// that can be used to start this process. Example: iota-test-validator
    /// --config-dir ~/.iota/iota_config We can use any config dir that is
    /// generated by the iota genesis.
    #[clap(short, long)]
    config_dir: Option<std::path::PathBuf>,

    /// Host to start the Fullnode RPC server on
    ...
    /// Host to start the Iota faucet on
    #[clap(long, default_value = "127.0.0.1")]
    faucet_host: String,
...
async fn main() -> Result<()> {
    ...
    let cluster_config = ClusterTestOpt {
        ...
        faucet_address: Some(format!("{}:{}", faucet_host, faucet_port)),
        epoch_duration_ms,
        config_dir,
