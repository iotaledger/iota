...
use iota_grpc_client::read_mask_fields::ObjectReadMask;
...
async fn get_objects_scenarios() {
    ...
    let object_ids: Vec<ObjectId> = SYSTEM_PACKAGE_IDS
        ...
        .collect();
    let objects = client
        .get_objects(object_ids.iter().copied(), ObjectReadMask::default())
        .await
    ...
    let err = client
        .get_objects(Vec::<ObjectId>::new(), ObjectReadMask::default())
        .await
