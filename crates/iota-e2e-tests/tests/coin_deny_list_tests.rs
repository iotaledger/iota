...
use iota_json_rpc_types::{
    IotaTransactionBlockDataAPI, IotaTransactionBlockEffectsAPI, IotaTransactionBlockKind,
...
async fn test_coin_deny_list_creation() {
    ...
    for handle in test_cluster.all_node_handles() {
        handle.with(|node| {
            ...
            assert_eq!(deny_list.id, UID::new(IOTA_DENY_LIST_OBJECT_ID));
    ...
    let tx = test_cluster
        ...
        .iota_client
        .read_api()
        .get_transaction_with_options(prev_tx, IotaTransactionBlockResponseOptions::full_content())
        .await
        .unwrap()
        .transaction
        .unwrap();
