...
//         // Use a long epoch duration such that it won't change epoch on its
// own.         .with_epoch_duration_ms(10000000)
//         .with_default_jwks()
//         .build()
//         .await;
...
//     // construct a multisig address with 4 pks (ed25519, secp256k1,
// secp256r1,     // zklogin) with threshold = 1.
//     let (eph_kp, _eph_pk, zklogin_inputs) =
//         &load_test_vectors("../iota-types/src/unit_tests/
// zklogin_test_vectors.json").unwrap()[1];     let (eph_kp_1, _, _) =
//         &load_test_vectors("../iota-types/src/unit_tests/
// zklogin_test_vectors.json").unwrap()[2];     let zklogin_pk =
// PublicKey::ZkLogin(         ZkLoginPublicIdentifier::new(zklogin_inputs.
// get_iss(), zklogin_inputs.get_address_seed())             .unwrap(),
//     );
...
//         .build();
//     let intent_msg = IntentMessage::new(Intent::iota_transaction(),
// tx_data.clone());     let wrong_intent_msg =
// IntentMessage::new(Intent::personal_message(), tx_data.clone());

//     // 1. a multisig with a bad ed25519 sig fails to execute.
//     let wrong_sig: GenericSignature =
// Signature::new_secure(&wrong_intent_msg, &keys[0]).into();     let multisig =
// GenericSignature::MultiSig(         MultiSig::combine(vec![wrong_sig],
// multisig_pk.clone()).unwrap(),     );
//     let tx_1 = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![multisig]);     let res =
// context.execute_transaction_may_fail(tx_1).await;     assert!(
//         res.unwrap_err()
//             .to_string()
//             .contains(format!("Invalid sig for pk={}",
// pk0.encode_base64()).as_str())     );

//     // 2. a multisig with a bad secp256k1 sig fails to execute.
//     let wrong_sig_2: GenericSignature =
// Signature::new_secure(&wrong_intent_msg, &keys[1]).into();     let multisig =
// GenericSignature::MultiSig(         MultiSig::combine(vec![wrong_sig_2],
// multisig_pk.clone()).unwrap(),     );
//     let tx_2 = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![multisig]);     let res =
// context.execute_transaction_may_fail(tx_2).await;     assert!(
//         res.unwrap_err()
//             .to_string()
//             .contains(format!("Invalid sig for pk={}",
// pk1.encode_base64()).as_str())     );

//     // 3. a multisig with a bad secp256r1 sig fails to execute.
//     let wrong_sig_3: GenericSignature =
// Signature::new_secure(&wrong_intent_msg, &keys[2]).into();     let multisig =
// GenericSignature::MultiSig(         MultiSig::combine(vec![wrong_sig_3],
// multisig_pk.clone()).unwrap(),     );
//     let tx_3 = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![multisig]);     let res =
// context.execute_transaction_may_fail(tx_3).await;     assert!(
//         res.unwrap_err()
//             .to_string()
//             .contains(format!("Invalid sig for pk={}",
// pk2.encode_base64()).as_str())     );

//     // 4. a multisig with a bad ephemeral sig inside zklogin sig fails to
// execute.     let wrong_eph_sig = Signature::new_secure(&wrong_intent_msg,
// eph_kp);     let wrong_zklogin_sig =
// GenericSignature::ZkLoginAuthenticator(ZkLoginAuthenticator::new(
//         zklogin_inputs.clone(),
//         2,
//         wrong_eph_sig,
//     ));
//     let multisig = GenericSignature::MultiSig(
//         MultiSig::combine(vec![wrong_zklogin_sig],
// multisig_pk.clone()).unwrap(),     );
//     let tx_4 = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![multisig]);     let res =
// context.execute_transaction_may_fail(tx_4).await;     let pk3 =
// PublicKey::ZkLogin(         ZkLoginPublicIdentifier::new(zklogin_inputs.
// get_iss(), zklogin_inputs.get_address_seed())             .unwrap(),
//     );
//     assert!(
...
//             .to_string()
//             .contains(format!("Invalid sig for pk={}",
// pk3.encode_base64()).as_str())     );

//     // 5. a multisig with a mismatch ephermeal sig and zklogin inputs fails
// to     //    execute.
//     let eph_sig = Signature::new_secure(&intent_msg, eph_kp_1);
//     let zklogin_sig_mismatch =
// GenericSignature::ZkLoginAuthenticator(ZkLoginAuthenticator::new(
//         zklogin_inputs.clone(),
//         2,
//         eph_sig,
//     ));
//     let multisig = GenericSignature::MultiSig(
//         MultiSig::combine(vec![zklogin_sig_mismatch],
// multisig_pk.clone()).unwrap(),     );
//     let tx_5 = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![multisig]);     let res =
// context.execute_transaction_may_fail(tx_5).await;     assert!(
//         res.unwrap_err()
//             .to_string()
//             .contains(format!("Invalid sig for pk={}",
// pk3.encode_base64()).as_str())     );

//     // 6. a multisig with an inconsistent max_epoch with zk proof itself
// fails to     //    execute.
//     let eph_sig = Signature::new_secure(&intent_msg, eph_kp);
//     let zklogin_sig_wrong_zklogin_inputs =
// GenericSignature::ZkLoginAuthenticator(
//         ZkLoginAuthenticator::new(zklogin_inputs.clone(), 1, eph_sig), /*
// max_epoch set to 1
//                                                                         * instead of 2 */
//     );
//     let multisig = GenericSignature::MultiSig(
//         MultiSig::combine(vec![zklogin_sig_wrong_zklogin_inputs],
// multisig_pk.clone()).unwrap(),     );
//     let tx_7 = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![multisig]);     let res =
// context.execute_transaction_may_fail(tx_7).await;     assert!(
//         res.unwrap_err()
//             .to_string()
...
//     );
//     let gas = test_cluster
//         .fund_address_and_return_gas(rgp, Some(20000000000),
// wrong_multisig_addr)         .await;
//     let tx_data = TestTransactionBuilder::new(wrong_multisig_addr, gas, rgp)
//         .transfer_iota(None, IotaAddress::ZERO)
//         .build();
//     let intent_msg = IntentMessage::new(Intent::iota_transaction(),
// tx_data.clone());     let sig_4: GenericSignature =
// ZkLoginAuthenticator::new(         zklogin_inputs.clone(),
//         2,
//         Signature::new_secure(&intent_msg, eph_kp),
//     )
//     .into();
//     let multisig =
//         GenericSignature::MultiSig(MultiSig::combine(vec![sig_4],
// multisig_pk.clone()).unwrap());     let tx_8 =
...
//     // 8. a multisig with zklogin sig of invalid compact signature bytes
// fails to     //    execute.
...
//         .build();
//     let intent_msg = IntentMessage::new(Intent::iota_transaction(),
// tx_data.clone());     let sig_0: GenericSignature =
// Signature::new_secure(&intent_msg, &keys[0]).into();     let multisig =
//         GenericSignature::MultiSig(MultiSig::combine(vec![sig_0],
// multisig_pk.clone()).unwrap());     let tx_8 =
// Transaction::from_generic_sig_data(tx_data, vec![multisig]);     let _ =
// context.execute_transaction_must_succeed(tx_8).await;
...
//         .build();
//     let intent_msg = IntentMessage::new(Intent::iota_transaction(),
// tx_data.clone());     let sig_1: GenericSignature =
// Signature::new_secure(&intent_msg, &keys[1]).into();     let multisig =
//         GenericSignature::MultiSig(MultiSig::combine(vec![sig_1],
// multisig_pk.clone()).unwrap());     let tx_9 =
// Transaction::from_generic_sig_data(tx_data, vec![multisig]);     let _ =
// context.execute_transaction_must_succeed(tx_9).await;
...
//         .build();
//     let intent_msg = IntentMessage::new(Intent::iota_transaction(),
// tx_data.clone());     let sig_2: GenericSignature =
// Signature::new_secure(&intent_msg, &keys[2]).into();     let multisig =
//         GenericSignature::MultiSig(MultiSig::combine(vec![sig_2],
// multisig_pk.clone()).unwrap());     let tx_9 =
// Transaction::from_generic_sig_data(tx_data, vec![multisig]);     let _ =
// context.execute_transaction_must_succeed(tx_9).await;
...
//     let multisig =
//         GenericSignature::MultiSig(MultiSig::combine(vec![sig_4],
// multisig_pk.clone()).unwrap());     let tx_10 =
// Transaction::from_generic_sig_data(tx_data.clone(), vec![multisig]);
//     let _ = context.execute_transaction_must_succeed(tx_10).await;

//     // 4c. good zklogin sig AND good ed25519 combined used in multisig
...
//         .build();
//     let intent_msg = IntentMessage::new(Intent::iota_transaction(),
// tx_data.clone());     let sig: GenericSignature =
// Signature::new_secure(&intent_msg, &keys[0]).into();     let sig_1:
// GenericSignature = ZkLoginAuthenticator::new(         zklogin_inputs.clone(),
//         2,
//         Signature::new_secure(&intent_msg, eph_kp),
//     )
//     .into();
...
//     );
//     let tx_11 = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![multisig]);     let _ =
// context.execute_transaction_must_succeed(tx_11).await;
...
// tx_data.clone());     let sig: GenericSignature =
// Signature::new_secure(&intent_msg, &keys[0]).into();     let multisig =
// GenericSignature::MultiSig(MultiSig::insecure_new(         vec![sig.
// to_compressed().unwrap()],         0b0010,
//         multisig_pk.clone(),
//     ));
//     let tx_11 = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![multisig]);     let res =
// context.execute_transaction_may_fail(tx_11).await;     assert!(
//         res.as_ref()
//             .unwrap_err()
//             .to_string()
//             .contains("Invalid sig for pk")
...
//     // 10. invalid bitmap b10000 when the max bitmap for 4 pks is b1111,
// fails to     //     execute.
//     let multisig = GenericSignature::MultiSig(MultiSig::insecure_new(
...
//         multisig_pk.clone(),
//     ));
//     let tx_10 = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![multisig]);     let res =
// context.execute_transaction_may_fail(tx_10).await;     assert!(
//         res.unwrap_err()
//             .to_string()
...
//     let gas = test_cluster
//         .fund_address_and_return_gas(rgp, Some(20000000000),
// bad_multisig_addr_2)         .await;
//     let tx_data = TestTransactionBuilder::new(bad_multisig_addr_2, gas, rgp)
//         .transfer_iota(None, IotaAddress::ZERO)
//         .build();
//     let sig: GenericSignature = Signature::new_secure(&intent_msg,
// &keys[0]).into();     let multisig =
// GenericSignature::MultiSig(MultiSig::insecure_new(         vec![sig.
// to_compressed().unwrap()],         2,
//         bad_multisig_pk,
//     ));
//     let tx_14 = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![multisig]);     let res =
// context.execute_transaction_may_fail(tx_14).await;     assert!(
//         res.unwrap_err()
//             .to_string()
//             .contains("Invalid value was given to the function")
//     );
...
//     let small_multisig_pk =
// MultiSigPublicKey::insecure_new(vec![(pk0.clone(), 1)], 1);
//     let bad_multisig_addr_3 = IotaAddress::from(&small_multisig_pk);
//     let gas = test_cluster
//         .fund_address_and_return_gas(rgp, Some(20000000000),
// bad_multisig_addr_3)         .await;
...
// Signature::new_secure(&intent_msg, &keys[0]).into();     let multisig =
// GenericSignature::MultiSig(MultiSig::insecure_new(         vec![sig.
// to_compressed().unwrap(), sig.to_compressed().unwrap()],         0b1,
//         small_multisig_pk,
//     ));
//     let tx_13 = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![multisig]);     let res =
// context.execute_transaction_may_fail(tx_13).await;     assert!(
//         res.unwrap_err()
//             .to_string()
//             .contains("Invalid value was given to the function")
//     );

//     // 14. pass a multisig where there is dup pk in multisig_pk, fails to
// execute.     let multisig_pk_with_dup =
//         MultiSigPublicKey::insecure_new(vec![(pk0.clone(), 1), (pk0.clone(),
// 1)], 1);     let bad_multisig_addr_4 =
// IotaAddress::from(&multisig_pk_with_dup);     let gas = test_cluster
//         .fund_address_and_return_gas(rgp, Some(20000000000),
// bad_multisig_addr_4)         .await;
...
//     let multisig_pk_11 = MultiSigPublicKey::insecure_new(vec![(pk0.clone(),
// 1); 11], 1);     let bad_multisig_addr_11 =
// IotaAddress::from(&multisig_pk_11);     let gas = test_cluster
//         .fund_address_and_return_gas(rgp, Some(20000000000),
// bad_multisig_addr_11)         .await;
...
// Signature::new_secure(&intent_msg, &keys[0]).into();     let multisig =
// GenericSignature::MultiSig(MultiSig::insecure_new(         vec![sig.
// to_compressed().unwrap()],         0b00000000001,
//         multisig_pk_11,
//     ));
//     let tx_15 = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![multisig]);     let res =
// context.execute_transaction_may_fail(tx_15).await;     assert!(
//         res.unwrap_err()
//             .to_string()
//             .contains("Invalid value was given to the function")
//     );
...
//     let multisig_pk_12 =
//         MultiSigPublicKey::insecure_new(vec![(pk0.clone(), 1), (pk0.clone(),
// 1)], 3);     let bad_multisig_addr = IotaAddress::from(&multisig_pk_12);
//     let gas = test_cluster
//         .fund_address_and_return_gas(rgp, Some(20000000000),
// bad_multisig_addr)         .await;
//     let tx_data = TestTransactionBuilder::new(bad_multisig_addr, gas, rgp)
...
//         multisig_pk_12,
//     ));
//     let tx_16 = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![multisig]);     let res =
// context.execute_transaction_may_fail(tx_16).await;     assert!(
//         res.unwrap_err()
//             .to_string()
//             .contains("Invalid value was given to the function")
//     );
...
//     ));
//     let tx_17 = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![multisig]);     let res =
// context.execute_transaction_may_fail(tx_17).await;     assert!(
//         res.unwrap_err()
//             .to_string()
//             .contains("Invalid value was given to the function")
//     );
...
// {         config.
// set_zklogin_max_epoch_upper_bound_delta_for_testing(Some(1));         config
//     });
...
//     // max epoch at 2 is larger than current epoch (0) + upper bound (1), tx
// fails.     let res =
// test_cluster.wallet.execute_transaction_may_fail(tx).await;     assert!(
//         res.unwrap_err()
//             .to_string()
...
//     let test_vectors =
// &load_test_vectors("../iota-types/src/unit_tests/zklogin_test_vectors.json")
//         .unwrap()[1..11];
//     let test_cluster = TestClusterBuilder::new()
//         .with_epoch_duration_ms(15000)
//         .with_default_jwks()
...
//         .build();
//     let intent_msg = IntentMessage::new(Intent::iota_transaction(),
// tx_data.clone());     let mut zklogin_sigs = vec![];
...
//         MultiSig::combine(zklogin_sigs[..9].to_vec(),
// multisig_pk.clone()).unwrap(),     );
//     let bad_tx = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![short_multisig]);     let res =
// context.execute_transaction_may_fail(bad_tx).await;     assert!(
//         res.unwrap_err()
//             .to_string()
...
//     let multisig = GenericSignature::MultiSig(
//         MultiSig::combine(zklogin_sigs.clone(),
// multisig_pk.clone()).unwrap(),     );
//     let tx = Transaction::from_generic_sig_data(tx_data.clone(),
// vec![multisig]);     let _ =
