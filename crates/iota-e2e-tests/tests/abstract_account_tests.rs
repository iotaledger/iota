...
/// FAIL: try to "receive" in authenticate (by supplying a Receiving<Gift> to the authenticator).
#[sim_test]
async fn test_authenticate_receiving_object_fails() -> Result<(), anyhow::Error> {
    telemetry_subscribers::init_for_testing();
    ...
    // Any simple PTB; it won't run if auth fails
    let pt = test_env.craft_aa_simple_ptb()?;
    let tx_data = test_env.craft_tx_from_pt(pt, aa_gas, aa_sender, None).await?;
...
/// SUCCESS: receive in the main PT using abstract_account::receive_object<T>(...).
#[sim_test]
async fn test_receive_object_in_main_tx_succeeds() -> Result<(), anyhow::Error> {
    telemetry_subscribers::init_for_testing();
...
impl TestEnvironment {
    ...
    fn create_move_authenticator_for_receive_coin(
        ...
        let self_arg = CallArg::Object(ObjectArg::SharedObject {
            id: aa_ref.0,
            initial_shared_version: aa_ref.1,
            mutable: false,
        });
