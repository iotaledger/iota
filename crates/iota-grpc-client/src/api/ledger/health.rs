...
impl Client {
    ...
    ///
    /// Returns a [`GetHealthResponse`] with the latest checkpoint sequence
    /// number and an estimated validator latency field (reserved for future
    /// use).
    ///
    ...
    pub async fn get_health(&self, threshold_ms: Option<u64>) -> Result<GetHealthResponse> {
        ...
        let response = client.get_health(request).await?.into_inner();
