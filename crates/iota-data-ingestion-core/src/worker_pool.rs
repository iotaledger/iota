...
use crate::{
    Worker,
    executor::{MAX_CHECKPOINTS_IN_PROGRESS, WorkerPoolMsg},
};
...
enum WorkerMsg {
    /// Send Worker progress status to WorkerPool main loop
    Progress((usize, u64, Option<u64>)),
    /// Signal Worker graceful shutdown to WorkerPool main loop
    ShutDown(usize),
}
...
impl<W: Worker + 'static> WorkerPool<W> {
    ...
    pub async fn run(
        ...
        executor_progress_sender: mpsc::Sender<WorkerPoolMsg>,
            ...
            let join_handle = spawn_monitored_task!(async move {
                loop {
                    tokio::select! {
                        ...
                        _ = token.cancelled() => {
                            _ = cloned_progress_sender.send(WorkerMsg::ShutDown(worker_id)).await;
                            break
                        },
                        Some(checkpoint) = worker_recv.recv() => {
                            ...
                            if cloned_progress_sender.send(WorkerMsg::Progress((worker_id, sequence_number, worker.save_progress(sequence_number).await))).await.is_err() {
            ...
            tokio::select! {
                Some(worker_progress_msg) = progress_receiver.recv() => {
                    match worker_progress_msg {
                        WorkerMsg::Progress((worker_id, status_update, progress_watermark)) => {
                            ...
                            if status_update == current_checkpoint_number {
                                ...
                                if let Some(update) = executor_status_update {
                                    if executor_progress_sender
                                        .send(WorkerPoolMsg::Progress((self.task_name.clone(), update)))
                                        .await.is_err() {
                        ...
                        // Manages the graceful shutdown sequence of the worker pool.
                        //
                        // The shutdown process follows these steps:
                        // 1. Token cancellation triggers workers to:
                        //    - Complete their current checkpoint processing
                        //    - Send a final ShutDown message
                        //    - Exit their processing loop
                        //
                        // 2. Main loop continues running to:
                        //    - Process any remaining Progress messages
                        //    - Track worker shutdowns via ShutDown messages
                        //    - Once all workers have reported shutdown, await their join handles
                        //
                        // This ensures:
                        // - No work is interrupted mid-processing
                        // - All progress messages are processed
                        // - All resources are properly cleaned up
                        // - Workers terminate in an orderly fashion
                        WorkerMsg::ShutDown(worker_id) => {
                            // Track workers that have initiated shutdown
                            workers_shutdown_signals.push(worker_id);
                            // Once all workers have signaled completion, await their handles
                            // This ensures all workers have finished their final tasks
                            if workers_shutdown_signals.len() == self.concurrency {
                                for join_handle in workers_join_handles {
                                    join_handle.await.expect("worker thread panicked");
                                }
                                _ = executor_progress_sender
                                    .send(WorkerPoolMsg::ShutDown(self.task_name.clone()))
                                    .await;
