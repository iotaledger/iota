...
impl IotaTestAdapter {
    ...
    fn resolve_aa_params(&self, aa_type: String) -> anyhow::Result<(ObjectID, String, TypeTag)> {
        let mut account_type_parts: Vec<String> =
            aa_type.split("::").map(|s| s.to_string()).collect();

        let aa_package_addr = self
            .compiled_state
            .named_address_mapping
            .get(&account_type_parts[0])
    ...
    fn build_abstract_account_transaction(
        ...
        let auth_info_arg = builder.programmable_move_call(
            ...
            vec![pkg_metadata_arg, mod_arg, fn_arg],
        );
