...
use iota_types::{base_types::ObjectID, full_checkpoint_content::CheckpointData, object::Owner};
    ...
    fn process(&self, checkpoint: &Arc<CheckpointData>) -> Result<Vec<Self::Value>> {
        ...
        for object_id in checkpoint_input_objects.keys() {
            if !latest_live_output_objects.contains_key(object_id) {
                // If an input object is not in the latest live output objects, it must have
                // been deleted or wrapped in this checkpoint. We keep an entry
                // for it in the table. This is necessary when we query objects
                // and iterating over them, so that we don't include the object
                // in the result if it was deleted.
                values.insert(*object_id, StoredObjInfo {
        ...
        for (object_id, object) in latest_live_output_objects.iter() {
            // If an object is newly created/unwrapped in this checkpoint, or if the owner
            // changed, we need to insert an entry for it in the table.
