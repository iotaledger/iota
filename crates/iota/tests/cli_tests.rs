...
async fn test_simple_cli_commands() -> Result<(), anyhow::Error> {
    ...
    // Get wallet config path for creating separate contexts
    let wallet_config = test_cluster.swarm.dir().join(IOTA_CLIENT_CONFIG);

    // Get addresses for all tests
...
/// Helper for test_objects_command - tests listing objects owned by an address
async fn objects_command_helper(
    wallet_config: &std::path::Path,
    address: IotaAddress,
) -> Result<(), anyhow::Error> {
    // ===== test_objects_command =====
    // Tests listing objects owned by an address using both address and alias
...
/// Helper for test_object_info_get_command - tests getting detailed object info
/// by ID
async fn object_info_command_helper(
    wallet_config: &std::path::Path,
    address: IotaAddress,
) -> Result<(), anyhow::Error> {
    // ===== test_object_info_get_command =====
    // Tests getting detailed object info by ID
    let mut context = WalletContext::new(wallet_config, None, None)?;
    context.config_mut().set_active_address(Some(address));
    let client = context.get_client().await?;
...
/// Helper for test_gas_command - tests the Gas command and a transfer
async fn gas_command_helper(
    wallet_config: &std::path::Path,
    address: IotaAddress,
    rgp: u64,
) -> Result<(), anyhow::Error> {
    // ===== test_gas_command =====
    // Tests the Gas command to list gas objects, performs a transfer, then lists
    // again
    let mut context = WalletContext::new(wallet_config, None, None)?;
    context.config_mut().set_active_address(Some(address));
    let alias = context
...
/// Helper function to test receive argument with a specific function name
async fn receive_argument_helper(
    context: &mut WalletContext,
    package_id: ObjectID,
    rgp: u64,
    function_name: &str,
...
#[sim_test]
async fn test_receive_argument_all_variants() -> Result<(), anyhow::Error> {
    let mut test_cluster = TestClusterBuilder::new()
        .with_num_validators(1)
        .build()
        .await;
    ...
    // Test all three receive argument variants:
    // - test_receive_argument: Test basic receive functionality
    receive_argument_helper(context, package_id, rgp, "receiver").await?;
    // - test_receive_argument_by_immut_ref: Test receive with immutable reference
    receive_argument_helper(context, package_id, rgp, "invalid_call_immut_ref").await?;
    // - test_receive_argument_by_mut_ref: Test receive with mutable reference
    receive_argument_helper(context, package_id, rgp, "invalid_call_mut_ref").await?;
...
/// Helper for test_serialize_tx - tests serializing transactions
async fn serialize_tx_helper(
    wallet_config: &std::path::Path,
    address: IotaAddress,
    ...
    // ===== test_serialize_tx =====
    // Tests serializing signed and unsigned transactions
    let mut context = WalletContext::new(wallet_config, None, None)?;
    context.config_mut().set_active_address(Some(address));
    let alias_recipient = context
        .config()
        .keystore()
        .get_alias_by_address(&recipient)
        .unwrap();
    let client = context.get_client().await?;
    let object_refs = client
        .read_api()
    ...
    IotaClientCommands::PayIota {
        ...
        recipients: vec![KeyIdentity::Address(recipient)],
        amounts: vec![1],
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_TRANSFER),
            ..Default::default()
    ...
    IotaClientCommands::PayIota {
        input_coins: Some(vec![coin]),
        recipients: vec![KeyIdentity::Alias(alias_recipient)],
        amounts: vec![1],
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_TRANSFER),
            ..Default::default()
...
/// Helper for test_pay_iota variant
/// Tests the PayIota command: uses input coins as gas and transfers to
/// recipients
...
/// Helper for test_pay_all_iota variant
/// Tests the PayAllIota command: transfers all IOTA from input coins to a
/// single recipient
...
/// Helper for test_transfer - tests basic transfer command
async fn transfer_helper(
    ...
    let transfer = IotaClientCommands::Transfer {
        to: KeyIdentity::Address(recipient_address),
    ...
    .execute(&mut context)
    .await?;
    // transfer command will transfer the object_id1 to recipient_address, and use
    // object_id2 as gas we check if object1 is owned by recipient_address and
    // if the gas object used is object_id2
...
/// Helper for test_transfer_gas_smash - tests transfer with multiple gas
/// objects
async fn transfer_gas_smash_helper(
    wallet_config: &std::path::Path,
    client: &IotaClient,
    address: IotaAddress,
    objects: Vec<ObjectID>,
    ...
    // transfer command will transfer the object_id2 to recipient_address, and use
    // object_id0, and object_id1 as gas we check if object1 is owned by
    // recipient_address and the gas object used.
...
/// Helper for test_transfer_sponsored - tests sponsored transfer
async fn transfer_sponsored_helper(
    wallet_config: &std::path::Path,
    ...
    // Sponsor (a0) sends an object to sender (a1) first
    context
        .config_mut()
    ...
    // Sender sends it back to sponsor, but sponsored by sponsor
    context
        .config_mut()
    ...
    let transfer_back = IotaClientCommands::Transfer {
        ...
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_TRANSFER),
            gas_sponsor: Some(sponsor_address),
            ..Default::default()
        },
        processing: TxProcessingArgs::default(),
    }
    ...
    assert_eq!(response.status_ok(), Some(true));
    assert_eq!(tx.data.gas_data().owner, sponsor_address);
    assert_eq!(tx.data.sender(), &sender_address);

    Ok(())
}

/// Helper for test_transfer_serialized_data - tests transfer with serialized tx
/// data
async fn transfer_serialized_data_helper(
    wallet_config: &std::path::Path,
    client: &IotaClient,
    address: IotaAddress,
    objects: Vec<ObjectID>,
    ...
    assert!(effects.status().is_ok());
    assert_eq!(effects.gas_object().object_id(), objects[1]);

    let recipient_objs = client
        .read_api()
        .get_owned_objects(recipient, None, None, None)
        .await?;

    assert!(!recipient_objs.has_next_page);
    let page = recipient_objs.data;
    assert_eq!(page.len(), 1);
    assert_eq!(
        page.first().unwrap().object().unwrap().object_id,
        objects[0]
    );
...
/// Helper for test_transfer_serialized_kind - tests transfer with serialized tx
/// kind
async fn transfer_serialized_kind_helper(
    wallet_config: &std::path::Path,
    client: &IotaClient,
    address: IotaAddress,
    objects: Vec<ObjectID>,
...
/// Helper for test_balance - tests the Balance command
async fn balance_command_helper(
    wallet_config: &std::path::Path,
    address: IotaAddress,
) -> Result<(), anyhow::Error> {
    // ===== test_balance =====
    // Tests the Balance command to fetch coin balances for an address
    let mut context = WalletContext::new(wallet_config, None, None)?;
    context.config_mut().set_active_address(Some(address));
...
/// Helper for test_ptb_dev_inspect - tests the --dev-inspect flag
async fn ptb_dev_inspect_helper(
    wallet_config: &std::path::Path,
    address: IotaAddress,
) -> Result<(), anyhow::Error> {
    // ===== test_ptb_dev_inspect =====
    let mut context = WalletContext::new(wallet_config, None, None)?;
    context.config_mut().set_active_address(Some(address));
...
/// Helper for test_ptb_display_args - tests PTB display options
async fn ptb_display_args_helper(
    wallet_config: &std::path::Path,
    address: IotaAddress,
) -> Result<(), anyhow::Error> {
    // ===== test_ptb_display_args =====
    let mut context = WalletContext::new(wallet_config, None, None)?;
    context.config_mut().set_active_address(Some(address));
...
/// Helper for test_ptb_gas_coin_smashing - takes path and creates its own
/// context. Tests PTB gas smashing with a specific flag name ("gas-coin" or
/// "gas-coins")
async fn ptb_gas_smashing_helper_parallel(
    wallet_config: &std::path::Path,
    address: IotaAddress,
    flag_name: &str,
) -> Result<(), anyhow::Error> {
