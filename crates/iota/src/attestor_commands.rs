...
use crate::{
    PrintableResult,
    system_txn::{call_0x5, execute_0x5_txn, get_gas_obj_ref},
    validator_commands::write_transaction_response,
};
...
/// Same encoding as `write_keypair_to_file`, but an existing file is never
/// overwritten, and on unix the file is created with mode 0600 so the private
/// key is never readable by others.
