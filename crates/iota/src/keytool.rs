...
pub enum KeyToolCommand {
    ...
    /// and an optional derivation path, default to m/44'/784'/0'/0'/0' for
    /// ed25519 or m/54'/784'/0'/0/0 for secp256k1 or m/74'/784'/0'/0/0 for
    /// secp256r1. Supports mnemonic phrase of word length 12, 15,
    ...
    pub async fn execute(self, keystore: &mut Keystore) -> Result<CommandOutput, anyhow::Error> {
        let cmd_result = Ok(match self {
            ...
            KeyToolCommand::ZkLoginSignAndExecuteTx {
                ...
                // This is only for CLI testing. If frontend apps will be built, no need to add
                // anything here.
