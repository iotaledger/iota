...
use iota_json_rpc_types::{
    IotaData, IotaObjectDataOptions, IotaObjectResponse, IotaTransactionBlockResponse,
};
use iota_names::{
...
impl NameCommand {
    pub async fn execute(
        ...
        Ok(match self {
            ...
            Self::List { address } => {
                let mut nfts = get_owned_nfts(address, context).await?;
                nfts.extend(
                    get_owned_subdomain_nfts(address, context)
                        .await?
                        .into_iter()
                        .map(SubdomainRegistration::into_inner),
                );
            ...
            Self::Register {
                ...
                handle_transaction_result(res, verbose, async |res| {
                    Ok(NameCommandResult::Register {
                        ...
                        nft: get_owned_nft_by_name(&domain, context).await?,
                        digest: res.digest,
                    })
                })
                .await?
    ...
    pub async fn execute(self, context: &mut WalletContext) -> anyhow::Result<NameCommandResult> {
        ...
        Ok(match self {
            ...
            Self::RegisterNode {
                ...
                handle_transaction_result(res, verbose, async |res| {
                    Ok(NameCommandResult::RegisterNodeSubdomain {
                        record: get_registry_entry(&domain, &iota_client).await?.name_record,
                        nft: get_owned_subdomain_nft_by_name(&domain, context).await?,
                        digest: res.digest,
                    })
                })
                .await?
            ...
            Self::ExtendExpiration {
                ...
                let nft = get_owned_subdomain_nft_by_name(&domain, context).await?;
                anyhow::ensure!(
...
async fn get_owned_subdomain_nfts(
    address: Option<IotaAddress>,
    context: &mut WalletContext,
) -> anyhow::Result<Vec<SubdomainRegistration>> {
    let client = context.get_client().await?;
    let address = get_identity_address(address.map(KeyIdentity::Address), context)?;
    PagedFn::stream(async |cursor| {
        client
...
async fn get_owned_nft_by_name(
    ...
    Err(anyhow::anyhow!("no matching owned nft found for {domain}"))
}

async fn get_owned_subdomain_nft_by_name(
    domain: &Domain,
    context: &mut WalletContext,
) -> anyhow::Result<SubdomainRegistration> {
    let domain = domain.to_string();

    for nft in get_owned_subdomain_nfts(None, context).await? {
        if nft.domain_name() == domain {
            return Ok(nft);
        }
    }
    ...
    Err(anyhow::anyhow!(
        "no matching owned subdomain nft found for {domain}"
    ))
}
...
async fn get_proxy_nft_by_name(
    ...
    Ok(if domain.is_sld() {
        ...
        IotaNamesNftProxy::Subdomain(get_owned_subdomain_nft_by_name(domain, context).await?)
    })
}
