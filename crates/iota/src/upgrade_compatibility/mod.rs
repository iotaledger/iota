...
/// Check if an `UpgradeCompatibilityModeError` variant breaks 'compatible'
/// upgrades.
...
/// Check if an `UpgradeCompatibilityModeError` variant breaks inclusion checks
/// for 'additive' (Subset) or 'dependency only' (Equal) upgrades.
...
/// Compatibility mode used for 'compatible' upgrades, collects all errors and
/// returns them as a single error.
#[derive(Default)]
...
impl CompatibilityMode for CliCompatibilityMode {
    ...
    // Ignored, address is not populated pre-tx, `compare_packages` function
...
/// Compatibility mode used during inclusion checks: 'additive' (Subset) or
/// 'dependency only' (Equal) upgrades, collects all errors and returns them as
/// a single error.
#[derive(Default)]
...
/// Creates an index to allow looking up the table index of a struct, enum, or
/// function by its identifier.
...
/// Generates an enum Category along with individual enum for each individual
/// category and impls into diagnostic info for each category.
...
// WARNING: you should add new codes to the END of each category list to avoid
// breaking the existing codes. adding into the middle of a list will change the
// error code numbers "error[Compatibility EXXXXX]" similarly new categories
// should be added to the end of the outer list.
upgrade_codes!(
    Declarations: [
...
fn compare_packages(
    ...
    let move_toml_contents =
        Arc::from(fs::read_to_string(&move_toml_path).context("Unable to read Move.toml")?);
...
/// Runs the compatibility check between two modules and flattens the errors
...
/// Convert an error to a vector of diagnostics using the error specific
/// function.
...
fn missing_module_diag(
    ...
    // get the third newline after the start of the package table declaration if it
    // exists
...
/// Start by checking the lengths of the parameters and returns and add a
...
fn function_signature_mismatch_diag(
    ...
    // type parameters are a vector of AbilitySet and therefore cannot share the
    // same logic as structs and enums
...
/// Start by checking the lengths of the fields and return a diagnostic if they
/// are different. If the lengths are the same check each field piece wise and
...
/// Start by checking the lengths of the type parameters and return a diagnostic
/// if they are different. If the lengths are the same check each type parameter
...
/// Returns the error code and label for mismatched, missing, or unexpected
/// variants.
...
/// Start by checking the lengths of the variants and return a diagnostic if
/// they are different. If the lengths are the same check each type parameter
/// piece wise and return a diagnostic for each mismatch.
