...
/// [`CompatibilityMode`] Except for the `ModuleMismatch` which is a special
/// case for additive and dependency only policies and `ModuleMissing`
#[derive(Debug, Clone)]
pub(crate) enum UpgradeCompatibilityModeError {
    ModuleMissing {
        name: Identifier,
    },
    /// The upgrade is not compatible with the existing package due to the
    /// policy. This error is used for additive and dependency only policies
    /// where modules are either not allowed to add declarations or change
    /// them.
    ModuleMismatch {
...
/// A compatibility mode that collects errors as a vector of enums which
/// describe the error causes
#[derive(Default)]
...
/// A Compatibility mode used for checking inclusion checks for 'additive'
/// (Subset) or 'dependency only' (Equal) upgrades
#[derive(Default)]
...
/// Return a diagnostic for an ability mismatch. returns (full version, name,
/// type)
