...
mod imp {
    ...
    pub struct BytecodeVerifierMetrics {
        ...
        /// Bytecode verifier runtime latency for each module successfully
        /// verified
    ...
    impl BytecodeVerifierMetrics {
        ...
        const LATENCY_SEC_BUCKETS: &'static [f64] = &[
            ...
            0.000_250, 0.000_500, 0.001_000, 0.002_500, 0.005_000,
            0.010_000, // sub 10 ms: p99
...
/// wasm32 stubs — prometheus pulls in libc and can't compile to
/// `wasm32-unknown-unknown`. The execution engine reads metric fields by name
/// and invokes inc/observe/start_timer; we replicate that surface with
/// no-op types so the call sites compile unchanged. Constructors take no
...
mod imp {
    ...
    /// Aliased on wasm so external call sites that use
    /// `prometheus::IntCounter` keep compiling.
    ...
    impl StubTimer {
        ...
        /// Real prometheus returns `f64` here too; the adapter pipes this
        /// straight into a histogram's `observe`. Returning `f64` keeps both
        /// call paths typing identically on wasm.
