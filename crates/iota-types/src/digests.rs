...
/// the value that all signing schemes (Ed25519, Secp256k1, Secp256r1, MultiSig,
/// Passkey) sign. This differs from `TransactionDigest` which is
/// `blake2b256(bcs(TransactionData))` without the intent prefix.
///
/// It is stored in `AuthenticationContext` (not in the Move `AuthContext` type,
/// whose fields cannot change across protocol versions) and exposed to Move
/// authenticators via `auth_context::signing_digest()`.
#[derive(Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize, JsonSchema)]
pub struct SigningDigest(Digest);

impl Default for SigningDigest {
    fn default() -> Self {
        Self::ZERO
    }
}
...
impl AsRef<[u8]> for SigningDigest {
    fn as_ref(&self) -> &[u8] {
        self.0.as_ref()
    }
}

impl AsRef<[u8; 32]> for SigningDigest {
    fn as_ref(&self) -> &[u8; 32] {
        self.0.as_ref()
    }
}
...
impl From<SigningDigest> for [u8; 32] {
    fn from(digest: SigningDigest) -> Self {
        digest.into_inner()
    }
}
...
impl From<SigningDigest> for Vec<u8> {
    fn from(digest: SigningDigest) -> Self {
        digest.into_inner().to_vec()
    }
}

impl TryFrom<Vec<u8>> for SigningDigest {
    type Error = IotaError;

    fn try_from(bytes: Vec<u8>) -> Result<Self, IotaError> {
        let arr: [u8; 32] =
            <[u8; 32]>::try_from(&bytes[..]).map_err(|_| IotaError::InvalidSigningDigest)?;
        Ok(Self::new(arr))
    }
}
...
impl fmt::Debug for SigningDigest {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_tuple("SigningDigest").field(&self.0).finish()
    }
}

impl fmt::LowerHex for SigningDigest {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::LowerHex::fmt(&self.0, f)
    }
}

impl fmt::UpperHex for SigningDigest {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::UpperHex::fmt(&self.0, f)
    }
}

impl TryFrom<&[u8]> for SigningDigest {
    type Error = crate::error::IotaError;

    fn try_from(bytes: &[u8]) -> Result<Self, crate::error::IotaError> {
        let arr: [u8; 32] = bytes
            .try_into()
            .map_err(|_| crate::error::IotaError::InvalidSigningDigest)?;
        Ok(Self::new(arr))
    }
}

impl std::str::FromStr for SigningDigest {
    type Err = anyhow::Error;
