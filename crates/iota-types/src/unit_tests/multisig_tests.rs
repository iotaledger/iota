...
//     Ed25519Signature, Intent, IntentMessage, PersonalMessage,
// Secp256k1Signature,     Secp256r1Signature,
// };
...
//     crypto::{
//         Ed25519IotaSignature, IotaKeyPair, IotaSignatureInner, PublicKey,
// Signature,         ZkLoginPublicIdentifier, get_key_pair,
// get_key_pair_from_rng,     },
//     multisig::{MAX_SIGNER_IN_MULTISIG, MultiSig, MultisigMember},
//     signature::{AuthenticatorTrait, GenericSignature, VerifyParams},
//     signature_verification::VerifiedDigestCache,
//     utils::{
//         DEFAULT_ADDRESS_SEED, SHORT_ADDRESS_SEED, keys, load_test_vectors,
// make_transaction_data,         make_zklogin_tx, multisig_keys,
//     },
...
//     // MultiSigPublicKey contains only 2 public key but 3 signatures are
// passed,     // fails to combine.
//     assert!(
//         MultiSig::combine(
...
//     assert!(MultiSig::combine(vec![sig1.clone().into(), sig1.into()],
// multisig_pk).is_err()); }
// #[test]
...
//         let pk = kp.public();
//         let multisig_pk = MultiSigPublicKey::new(vec![pk], vec![1],
// 1).unwrap();         let sig = Signature::new_secure(&msg, &kp).into();
...
//         let generic_sig_roundtrip =
// GenericSignature::from_bytes(generic_sig_bytes).unwrap();         assert_eq!
// (generic_sig, generic_sig_roundtrip);
...
//     let single_sig_roundtrip =
// GenericSignature::from_bytes(single_sig_bytes).unwrap();     assert_eq!
// (single_sig, single_sig_roundtrip);     assert_eq!(single_sig_bytes.len(),
// Ed25519IotaSignature::LENGTH);     assert_eq!(
...
//     // Pin an hardcoded multisig address generation here. If this fails, the
// address     // generation logic may have changed. If this is intended, update
...
//     assert!(MultiSigPublicKey::new(pks.clone()[..5].to_vec(), vec![3; 5],
// 16).is_err());

//     // multisig_pk with max weights for each pk and max reachable threshold
// is ok.     let res = MultiSigPublicKey::new(
//         pks.clone()[..10].to_vec(),
//         vec![WeightUnit::MAX; MAX_SIGNER_IN_MULTISIG],
//         (WeightUnit::MAX as ThresholdUnit) * (MAX_SIGNER_IN_MULTISIG as
// ThresholdUnit),     );
//     assert!(res.is_ok());
...
//     // multisig_pk with max weights for each pk with threshold is 1x max
// weight     // validates ok.
...
//         PersonalMessage("Hello".as_bytes().to_vec().into()),
//     );
//     let sig1: GenericSignature = Signature::new_secure(&msg,
// &keys[0]).into();     let sig2: GenericSignature =
// Signature::new_secure(&msg, &keys[1]).into();

//     let multi_sig =
//         MultiSig::combine(vec![sig1.clone(), sig2.clone()],
// multisig_pk.clone()).unwrap();

//     assert!(multi_sig.get_pk().clone() == multisig_pk);
//     assert!(
//         *multi_sig.get_sigs() == vec![sig1.to_compressed().unwrap(),
...
//     let multi_sig1 =
//         MultiSig::combine(vec![sig2.into(), sig3.into()],
// multisig_pk.clone()).unwrap();
