...
///    such transaction,
/// we must make sure the transaction is already in this list. Otherwise nodes
/// running the newer version without these transactions in the list will
/// generate forked result. Below is a scenario of when we need to use this
/// list:
...
///    we need to add the transaction digest
/// to this list as well asap, and ship this binary to all fullnodes, so that
...
///    because if we do so, any future
/// node that sync from genesis will fork on this transaction. We may be able to
/// remove it once we have stable snapshots and the binary has a minimum
/// supported protocol version past the epoch.
