...
impl ProgrammableTransactionBuilder {
    ...
    pub fn make_obj_vec<T: Into<CallArg>>(
        ...
        let make_vec_args = objs
            ...
            .collect::<Result<_, _>>()?;
        Ok(self.command(Command::make_move_vector(None, make_vec_args)))
    }
    ...
    pub fn transfer_args(&mut self, recipient: IotaAddress, args: Vec<Argument>) {
        ...
        self.commands.push(Command::transfer_objects(args, rec_arg));
    }
    ...
    pub fn transfer_object(
        ...
        self.commands
            .push(Command::transfer_objects(vec![obj_arg?], rec_arg));
        Ok(())
    }
    ...
    pub fn transfer_iota(&mut self, recipient: IotaAddress, amount: Option<u64>) {
        ...
        let coin_arg = if let Some(amount) = amount {
            ...
            self.command(Command::split_coins(Argument::Gas, vec![amt_arg]))
        } else {
            Argument::Gas
        };
        self.command(Command::transfer_objects(vec![coin_arg], rec_arg));
    }

    pub fn pay_all_iota(&mut self, recipient: IotaAddress) {
        let rec_arg = self.pure(recipient).unwrap();
        self.command(Command::transfer_objects(vec![Argument::Gas], rec_arg));
    }
    ...
    pub fn split_coin(&mut self, recipient: IotaAddress, coin: ObjectRef, amounts: Vec<u64>) {
        ...
        let result = self.command(Command::split_coins(coin_arg, amt_args));
        ...
        self.command(Command::transfer_objects(
            (0..amounts_len)
    ...
    pub fn pay(
        ...
        if !merge_args.is_empty() {
            self.command(Command::merge_coins(coin_arg, merge_args));
        }
    ...
    fn pay_impl(
        ...
        let Argument::Result(split_primary) = self.command(Command::split_coins(coin, amt_args))
        else {
            panic!("self.command should always give a Argument::Result")
        };
        for (recipient, split_secondaries) in recipient_map {
            ...
            let coins = split_secondaries
                ...
                .collect();
            self.command(Command::transfer_objects(coins, rec_arg));
        }
        Ok(())
    }
}
