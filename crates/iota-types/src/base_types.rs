...
pub use iota_sdk_2::types::{Address, ObjectId, ObjectReference, Version};

pub type TxVersion = Version;

/// The round number.
pub type CommitRound = Version;
...
pub fn update_object_ref_for_testing(object_ref: ObjectReference) -> ObjectReference {
    ObjectReference::new(
...
pub fn address_from_padded_zklogin_inputs(inputs: &ZkLoginInputs) -> IotaResult<Address> {
    Ok(address_from_pub_key(&PublicKey::from_zklogin_inputs(
        inputs,
    )?))
}

/// Define as iss_bytes_len || iss_bytes || unpadded_32_byte_address_seed.
pub fn address_from_unpadded_zklogin_inputs(inputs: &ZkLoginInputs) -> IotaResult<Address> {
    ...
    Ok(Address::new(hasher.finalize().digest))
}
...
pub fn address_from_pub_key(pk: &PublicKey) -> Address {
    let mut hasher = DefaultHash::default();
...
/// Derive a Address from [struct MultiSigPublicKey]. A MultiSig address
...
pub fn address_from_multisig_pub_key(multisig_pk: &MultiSigPublicKey) -> Address {
    let mut hasher = DefaultHash::default();
...
pub fn address_from_zklogin_auth(authenticator: &ZkLoginAuthenticator) -> IotaResult<Address> {
    address_from_unpadded_zklogin_inputs(&authenticator.inputs)
}

/// Derive a Address from a serialized signature in IOTA
/// [GenericSignature].
pub fn address_from_generic_sig(sig: &GenericSignature) -> IotaResult<Address> {
    match sig {
...
impl ExecutionDigests {
    ...
    pub fn random() -> Self {
        Self {
            transaction: TransactionDigest::new(rand::random()),
            effects: TransactionEffectsDigest::new(rand::random()),
        }
    }
}
...
impl TxContext {
    pub fn new(sender: &Address, digest: &TransactionDigest, epoch_data: &EpochData) -> Self {
    ...
    pub fn with_sender_for_testing_only(sender: &Address) -> Self {
        Self::new(
            sender,
            &TransactionDigest::new(rand::random()),
            &EpochData::new_test(),
...
pub fn derive_object_id(digest: TransactionDigest, creation_num: u64) -> ObjectId {
    let mut hasher = DefaultHash::default();
    ...
    // ObjectId::LENGTH.
    ObjectId::new(hash.as_ref()[0..ObjectId::LENGTH].try_into().unwrap())
}
