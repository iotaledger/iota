...
/// Core-specific extension methods for `MoveObjectType`.
///
/// The base type and most methods are defined in the SDK crate
/// `iota_sdk_types`. This trait adds methods that depend on core-specific types
/// (`ModuleId`, `DynamicFieldInfo`, IOTA-specific display formatting, gas
/// metering).
pub trait MoveObjectTypeExt {
    fn module_id(&self) -> ModuleId;
    fn size_for_gas_metering(&self) -> usize;
    ...
    fn iota_display(&self) -> Result<String, fmt::Error>;
}

impl MoveObjectTypeExt for MoveObjectType {
    fn module_id(&self) -> ModuleId {
        ModuleId::new(
    ...
    fn size_for_gas_metering(&self) -> usize {
        ...
        if self.is_gas_coin() || self.is_staked_iota() {
            1
        } else if let Some(inner) = self.coin_type_opt() {
            bcs::serialized_size(&inner).unwrap() + 1
        } else {
            let s: StructTag = self.clone().into();
            bcs::serialized_size(&s).unwrap() + 1
        }
    }

    fn try_extract_field_name(&self, type_: &DynamicFieldType) -> IotaResult<TypeTag> {
        match self.other() {
            Some(s) => DynamicFieldInfo::try_extract_field_name(s, type_),
    ...
    fn try_extract_field_value(&self) -> IotaResult<TypeTag> {
        match self.other() {
            Some(s) => DynamicFieldInfo::try_extract_field_value(s),
            None => Err(IotaError::ObjectDeserialization {
...
impl fmt::Display for ObjectType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ObjectType::Struct(t) => {
                write!(f, "{}", t.iota_display().map_err(fmt::Error::custom)?)
            }
        }
    }
}
