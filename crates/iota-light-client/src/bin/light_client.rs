...
//     let reader = fs::File::open(path.clone())
//         .unwrap_or_else(|_| panic!("Unable to load config from {}",
// path.display()));     let config: Config =
...
//                 let json_val =
//                     IotaJsonValue::from_bcs_bytes(Some(&type_layout),
// &event.contents).unwrap();
