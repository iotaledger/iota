...
use iota_genesis_builder::stardust::{
    parse::GenesisSnapshotParser, types::snapshot::TOTAL_SUPPLY_IOTA,
};

fn main() -> anyhow::Result<()> {
    let Some(path) = std::env::args().nth(1) else {
        anyhow::bail!("please provide path to the genesis-snapshot file");
    };
    let file = File::open(path)?;

    let parser = GenesisSnapshotParser::new(file)?;
