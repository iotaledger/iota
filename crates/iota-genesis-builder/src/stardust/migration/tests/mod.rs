...
use iota_sdk_ext::types::{Address, Argument, Identifier, ObjectId, StructTag, TypeTag};
...
fn object_migration_with_object_owner(
    ...
    total_supply: u64,
    outputs: impl IntoIterator<Item = (OutputHeader, Output)>,
    output_owner_module_name: &Identifier,
    output_owned_module_name: &Identifier,
    unlock_condition_function: &Identifier,
    coin_type: CoinType,
) -> anyhow::Result<()> {
