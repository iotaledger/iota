...
fn display_info_v1(f: &mut Formatter<'_>, iota_gas_status: &GasStatusV1) -> std::fmt::Result {
    let mut builder = TableBuilder::default();
...
fn display_info_v2(f: &mut Formatter<'_>, iota_gas_status: &GasStatusV2) -> std::fmt::Result {
    let mut builder = TableBuilder::default();
    builder.push_record(vec!["Gas Info".to_string()]);
    builder.push_record(vec![format!(
        "Reference Gas Price: {}",
