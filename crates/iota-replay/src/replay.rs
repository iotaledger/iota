...
use iota_sdk_types::{
    ...
    SenderSignedTransaction, TransactionDigest, TransactionKind, Version,
};
use iota_types::{
...
/// Objects observed absent at a version during replay, cached so
/// repeated lookups skip the network.
pub type AbsentObjectCache = Arc<Mutex<HashSet<(ObjectId, Option<Version>)>>>;
...
pub struct LocalExec {
    ...
    pub absent_object_cache: AbsentObjectCache,
}
...
impl LocalExec {
        ...
        let (inner_store, gas_status, effects, _timings, result) = if move_authenticators.is_empty()
        {
            ...
            if let Some(out) = &self.profile_output {
                ...
                if !tx_info.kind.is_system() {
                    ...
                    let (_, warm_gas_status, warm_effects, _, _) = execute_once(make_gas_status());
            ...
            let (inner_store, gas_status, effects, timings, result, _authentication_failed) =
            ...
            (inner_store, gas_status, effects, timings, result)
        };
    ...
    pub async fn certificate_execute_with_sandbox_state(
        ...
        let (_, _, effects, _timings, exec_res) = if move_authenticators.is_empty() {
            ...
            // The authentication-failed flag is only used by the validator attestor's
            // admission decision; replay must reproduce effects regardless, so drop it.
            let (inner_store, gas_status, effects, timings, exec_res, _authentication_failed) =
                executor.authenticate_then_execute_transaction_to_effects(
                    &store,
                    &protocol_config,
                    Arc::new(LimitsMetrics::new(&Registry::new())),
            ...
            (inner_store, gas_status, effects, timings, exec_res)
        };
