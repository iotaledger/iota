...
use iota_types::{base_types::IotaAddress, move_package::MovePackage};
use tracing::info;
...
pub(crate) async fn dump(db_url: String, output_dir: PathBuf) -> Result<()> {
    ...
    for (i, pkg) in pkgs.into_iter().enumerate() {
        ...
        let id = IotaAddress::from_bytes(&pkg.package_id).context("Parsing package ID")?;
...
fn dump_package(output_dir: &Path, id: IotaAddress, pkg: &[u8]) -> Result<()> {
