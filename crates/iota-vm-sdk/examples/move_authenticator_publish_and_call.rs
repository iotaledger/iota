...
//!   trace.json.zst   the traced run — open this file to start a debug session
...
//! ```
//!
//! This is the layout the debugger reads a trace of external events in: it
//! resolves sources and debug info relative to the trace file, so the packages
//! are compiled straight into the bundle.
//!
//! Run with:
...
use move_trace_format::format::{MoveTrace, MoveTraceReader, TraceEvent};
...
/// The trace's file name. The debugger selects the layout above — sources and
/// bytecode resolved next to the trace — off this exact base name and
/// extension, and reads any other name as a Move unit-test trace instead.
const TRACE_FILE: &str = "trace.json.zst";
...
fn main() -> Result<()> {
    ...
    let result = run_signed(&mut vm, tx, account_id, account_version, false)?;
    ...
    match result.debug.and_then(|debug| debug.trace) {
        Some(trace) => {
            let trace_path = trace_dir.join(TRACE_FILE);
            fs::write(&trace_path, merge_ptb_summaries(&trace.into_bytes())?)?;
            println!(
                "\nTraced the increment run — the authenticator function and the PTB \
...
///
/// With `trace` set, the run is traced and the trace returned in
/// [`ExecutionResult::debug`]. The `MoveAuthenticator` path is the only one the
/// engine takes a trace builder on, and it covers both the authenticator
...
fn run_signed(
    ...
    let opts = if trace {
        ExecuteOptions::execute().with_debug(DebugConfig::default().with_tracing())
    } else {
...
/// Concatenate the trace's PTB summaries into the one it opens with.
///
/// A transaction authorized by a `MoveAuthenticator` runs two programmable
/// transactions — the authenticator function, then the transaction's own
/// commands — and each contributes a summary of its commands. The debugger
/// reads only the summary the trace opens with, as the listing whose current
/// line follows the executing command, and rejects a later one; listing the
/// commands of both runs in execution order keeps that listing in step with the
/// trace.
fn merge_ptb_summaries(trace: &[u8]) -> Result<Vec<u8>> {
    const SUMMARY_COMMANDS: &str = "/Summary/events";

    let events = MoveTraceReader::new(Cursor::new(trace))?.collect::<std::io::Result<Vec<_>>>()?;
    let commands: Vec<serde_json::Value> = events
        .iter()
        .filter_map(|event| match event {
            TraceEvent::External(value) => value.pointer(SUMMARY_COMMANDS)?.as_array(),
            _ => None,
        })
        .flatten()
        .cloned()
    ...
    let mut merged = MoveTrace::new();
    let mut summary_written = false;
    for mut event in events {
        let commands_field = match &mut event {
            TraceEvent::External(value) => value.pointer_mut(SUMMARY_COMMANDS),
            _ => None,
        };
        match commands_field {
            Some(_) if summary_written => continue,
            Some(field) => {
                *field = commands.clone().into();
    ...
    Ok(merged.into_compressed_json_bytes())
}
