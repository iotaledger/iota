...
// Allow use of `unbounded_channel` in `ingestion` -- it is used by the
// regulator task to receive feedback. Traffic through this task should be
// minimal, but if a bound is applied to it and that bound is hit, the indexer
// could deadlock.
...
pub struct ClientArgs {
    ...
    /// If both remote_store_url and local_ingestion_path are provided,
    /// remote_store_url will be used.
    #[clap(long, required = true, group = "source")]
...
pub struct IngestionConfig {
    /// Maximum size of checkpoint backlog across all workers downstream of the
    /// ingestion service.
    ...
    /// Polling interval to retry fetching checkpoints that do not exist, in
    /// milliseconds.
...
impl IngestionService {
    /// TODO: If we want to expose this as part of the framework, so people can
    /// run just an ingestion service, we will need to split
    /// `IngestionMetrics` out from `IndexerMetrics`.
    pub(crate) fn new(
        args: ClientArgs,
        config: IngestionConfig,
        ...
        // TODO: Potentially support a hybrid mode where we can fetch from both local
        // and remote.
    ...
    /// Add a new subscription to the ingestion service. Note that the service
    /// is susceptible to the "slow receiver" problem: If one receiver is
    /// slower to process checkpoints than the checkpoint ingestion rate, it
    /// will eventually hold up all receivers.
    ///
    /// The ingestion service can optionally receive checkpoint high watermarks
    /// from its subscribers. If a subscriber provides a watermark, the
    /// ingestion service will commit to not run ahead of the watermark by
    /// more than the config's buffer_size.
    ///
    /// Returns the channel to receive checkpoints from and the channel to
    /// accept watermarks from.
    ...
    /// Start the ingestion service as a background task, consuming it in the
    /// process.
    ///
    /// Checkpoints are fetched concurrently from the `checkpoints` iterator,
    /// and pushed to subscribers' channels (potentially out-of-order).
    /// Subscribers can communicate with the ingestion service via their
    /// channels in the following ways:
    ///
    /// - If a subscriber is lagging (not receiving checkpoints fast enough), it
    ///   will eventually provide back-pressure to the ingestion service, which
    ///   will stop fetching new checkpoints.
    /// - If a subscriber closes its channel, the ingestion service will
    ///   interpret that as a signal to shutdown as well.
    ///
    /// If ingestion reaches the leading edge of the network, it will encounter
    /// checkpoints that do not exist yet. These will be retried repeatedly
    /// on a fixed `retry_interval` until they become available.
...
mod tests {
    ...
    /// If the ingestion service has no subscribers, it will fail fast (before
    /// fetching any checkpoints).
    #[tokio::test]
    async fn fail_on_no_subscribers() {
        telemetry_subscribers::init_for_testing();

        // The mock server will repeatedly return 404, so if the service does try to
        // fetch a checkpoint, it will be stuck repeatedly retrying.
    ...
    /// The subscriber has no effective limit, and the mock server will always
    /// return checkpoint information, but the ingestion service can still
    /// be stopped using the cancellation token.
    #[tokio::test]
    ...
    /// The subscriber will stop after receiving a single checkpoint, and this
    /// will trigger the ingestion service to stop as well, even if there
    /// are more checkpoints to fetch.
    #[tokio::test]
    ...
    /// If fetching the checkpoint throws an unexpected error, the whole
    ...
    /// The service will retry fetching a checkpoint that does not exist, in
    /// this test, the 4th checkpoint will return 404 a couple of times,
    ...
    /// Similar to the previous test, but now it's a transient error that causes
    /// the retry.
    #[tokio::test]
    ...
    /// One subscriber is going to stop processing checkpoints, so even though
    /// the service can keep fetching checkpoints, it will stop short
    /// because of the slow receiver. Other subscribers can keep processing
    /// checkpoints that were buffered for the slow one.
    #[tokio::test]
    async fn back_pressure_and_buffering() {
        ...
        // At this point, the service will have been able to pass 3 checkpoints to the
        // non-lagging subscriber, while the laggard's buffer fills up. Now the
        // laggard will pull two checkpoints, which will allow the rest of the
        // pipeline to progress enough for the live subscriber to receive its
        // quota.
