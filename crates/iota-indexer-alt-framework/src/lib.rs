...
pub struct IndexerArgs {
    /// Override for the checkpoint to start ingestion from -- useful for
    /// backfills. By default, ingestion will start just after the lowest
    /// checkpoint watermark across all active pipelines.
    #[arg(long)]
    pub first_checkpoint: Option<u64>,

    /// Override for the checkpoint to end ingestion at (inclusive) -- useful
    /// for backfills. By default, ingestion will not stop, and will
    /// continue to poll for new checkpoints.
    #[arg(long)]
...
pub struct Indexer {
    ...
    /// Optional filter for pipelines to run. If `None`, all pipelines added to
    /// the indexer will run. Any pipelines that are present in this filter
    /// but not added to the indexer will yield a warning when the indexer
    /// is run.
    enabled_pipelines: Option<BTreeSet<String>>,

    /// Pipelines that have already been registered with the indexer. Used to
    /// make sure a pipeline with the same name isn't added twice.
    added_pipelines: BTreeSet<&'static str>,

    /// Cancellation token shared among all continuous tasks in the service.
    cancel: CancellationToken,

    /// The checkpoint lowerbound derived from watermarks of pipelines added to
    /// the indexer. When the indexer runs, it will start from this point,
    /// unless this has been overridden by [Self::first_checkpoint].
    first_checkpoint_from_watermark: u64,

    /// The handles for every task spawned by this indexer, used to manage
    /// graceful shutdown.
    handles: Vec<JoinHandle<()>>,
}

impl Indexer {
    /// Create a new instance of the indexer framework. `db_args`,
    /// `indexer_args,`, `client_args`, and `ingestion_config` contain
    /// configurations for the following, respectively:
    ///
    /// - Connecting to the database,
    /// - What is indexed (which checkpoints, which pipelines, whether to update
    ///   the watermarks table) and where to serve metrics from,
    /// - Where to download checkpoints from,
    /// - Concurrency and buffering parameters for downloading checkpoints.
    ///
    /// `migrations` contains the SQL to run in order to bring the database
    /// schema up-to-date for the specific instance of the indexer,
    /// generated using diesel's `embed_migrations!` macro. These migrations
    /// will be run as part of initializing the indexer.
    ///
    /// After initialization, at least one pipeline must be added using
    /// [Self::concurrent_pipeline] or [Self::sequential_pipeline], before
    /// the indexer is started using [Self::run].
    pub async fn new(
        db_args: DbArgs,
    ...
    /// Adds a new pipeline to this indexer and starts it up. Although their
    /// tasks have started, they will be idle until the ingestion service
    /// starts, and serves it checkpoint data.
    ///
    /// Concurrent pipelines commit checkpoint data out-of-order to maximise
    /// throughput, and they keep the watermark table up-to-date with the
    /// highest point they can guarantee all data exists for, for their
    /// pipeline.
    pub async fn concurrent_pipeline<H: concurrent::Handler + Send + Sync + 'static>(
        ...
        // For a concurrent pipeline, if skip_watermark is set, we don't really care
        // about the watermark consistency. first_checkpoint can be anything
        // since we don't update watermark, and writes should be idempotent.
    ...
    /// starts, and serves it checkpoint data.
    ///
    /// Sequential pipelines commit checkpoint data in-order which sacrifices
    /// throughput, but may be required to handle pipelines that modify data
    /// in-place (where each update is not an insert, but could be a
    /// modification of an existing row, where ordering between updates is
    /// important).
    ///
    /// The pipeline can optionally be configured to lag behind the ingestion
    /// service by a fixed number of checkpoints (configured by
    /// `checkpoint_lag`).
    ...
    /// Checks that the first checkpoint override is consistent with the
    /// watermark for the pipeline. If the watermark does not exist, the
    /// override can be anything. If the watermark exists, the override must
    /// not leave any gap in the data: it can be in the past, or at the tip of
    /// the network, but not in the future.
    ...
    /// `first_checkpoint`, or it is calculated based on the watermarks of all
    /// active pipelines. Ingestion will stop after consuming the configured
    /// `last_checkpoint`, if one is provided, or will continue until it
    /// tracks the tip of the network.
    pub async fn run(mut self) -> Result<JoinHandle<()>> {
        ...
        // If an override has been provided, start ingestion from there, otherwise start
        // ingestion from just after the lowest committer watermark across all
        // enabled pipelines.
        ...
        Ok(tokio::spawn(async move {
            // Wait for the ingestion service and all its related tasks to wind down
            // gracefully: If ingestion has been configured to only handle a
            // specific range of checkpoints, we want to make sure that tasks
            // are allowed to run to completion before shutting them down.
    ...
    /// Update the indexer's first checkpoint based on the watermark for the
    /// pipeline by adding for handler `H` (as long as it's enabled).
    /// Returns `Ok(None)` if the pipeline is disabled, `Ok(Some(None))` if
    /// the pipeline is enabled but its watermark is not found, and
    /// `Ok(Some(Some(watermark)))` if the pipeline is enabled and the watermark
    /// is found.
