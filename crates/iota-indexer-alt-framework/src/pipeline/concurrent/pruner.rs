...
/// The pruner task is responsible for deleting old data from the database. It
/// will periodically check the `watermarks` table to see if there is any data
/// that should be pruned -- between `pruner_hi` (inclusive), and `reader_lo`
/// (exclusive).
///
/// To ensure that the pruner does not interfere with reads that are still in
/// flight, it respects the watermark's `pruner_timestamp`, which records the
/// time that `reader_lo` was last updated. The task will not prune data until
/// at least `config.delay()` has passed since `pruner_timestamp` to give
/// in-flight reads time to land.
///
...
///
/// The task will shutdown if the `cancel` token is signalled. If the `config`
/// is `None`, the task will shutdown immediately.
pub(super) fn pruner<H: Handler + 'static>(
    ...
    tokio::spawn(async move {
        ...
        // `pruner_timestamp` to expire. In that case, the period between ticks should
        // not be compressed to make up for missed ticks.
        let mut poll = interval(config.interval());
        poll.set_missed_tick_behavior(MissedTickBehavior::Delay);

        // The pruner task will periodically output a log message at a higher log level
        // to demonstrate that it is making progress.
