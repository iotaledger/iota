...
/// 1) In snapshot, not in history - occurs when a live object gets snapshotted
///    and then has not been modified since
/// 2) Not in snapshot, in history - occurs when a new object is created or a
///    wrapped object is unwrapped
/// 3) In snapshot and in history - occurs when an object is snapshotted and
///    further modified, the modification can be wrapping or deleting.
///
