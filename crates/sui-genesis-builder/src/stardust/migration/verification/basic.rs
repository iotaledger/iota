...
pub(super) fn verify_basic_output(
    ...
    if output.unlock_conditions().len() > 1 {
        verify_genesis_object(output, created_objects, foundry_data, storage)?;
        ...
        verify_coin_and_native_tokens(output, created_objects, foundry_data, storage)?;
    }
...
pub(super) fn verify_genesis_object(
    output: &BasicOutput,
    created_objects: &CreatedObjects,
    foundry_data: &HashMap<TokenId, FoundryLedgerData>,
    storage: &InMemoryStorage,
...
pub(super) fn verify_coin_and_native_tokens(
    output: &BasicOutput,
    created_objects: &CreatedObjects,
    foundry_data: &HashMap<TokenId, FoundryLedgerData>,
    storage: &InMemoryStorage,
