...
iota client faucet
# Publish the IOTAccount package
...
# Create a new IOTAccount through a PTB which firstly builds an authenticator function ref for the ed25519 authenticator
export PTB_JSON=$(iota client ptb \
--move-call 0x2::authenticator_function::create_auth_function_ref_v1 '<'$PACKAGE_ID'::iotaccount::IOTAccount>' @$METADATA_ID '"public_key_authenticators"' '"ed25519_IOTAccount_authenticator"' \
--assign ref \
--move-call $PACKAGE_ID::public_key_authenticators::create vector"$SIGN_PUB_KEY_BYTES" ref \
--json)
...
echo "IOTAccount Object ID: $IOTACCOUNT"

# Check the public key stored in the account
DYNAMIC_FIELD_JSON=$(iota client dynamic-field $IOTACCOUNT --json)
export PUBLIC_KEY_FIELD_ID=$(echo $DYNAMIC_FIELD_JSON | jq -r '.data[] | select(.name.type | endswith("::public_key_authenticators::PublicKeyField")) | .objectId')
...
# Add the newly created IOTAccount to the CLI keystore and set is as active
iota client add-account $IOTACCOUNT
iota client switch --address $IOTACCOUNT
# Request funds for the IOTAccount
iota client faucet

# Create a transaction where the sender is the IOTAccount, but don't issue it; just take the bytes
...
# Extract the TX digest that is used by the authenticator to check the signature for the IOTAccount
