  ...
  for file in "$TEMP_EXPORT_DIR"/validator*.yaml; do
    if [ -f "$file" ]; then
      yq eval-all '
        ...
        .consensus-config.graduated-load-shedding-saturation-pct = $overlay.consensus-config.graduated-load-shedding-saturation-pct |
        .consensus-config.semaphore-shedding-enabled = $overlay.consensus-config.semaphore-shedding-enabled |
