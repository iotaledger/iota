...
def run_experiment():
    ...
    # We execute the module net_fuzz.experiments.sync_stress
    env = os.environ.copy()
    env["PYTHONPATH"] = str(FUZZER_DIR / "src")

    try:
        run_command(
            ["sudo", str(venv_python), "-m", "net_fuzz.experiments.sync_stress"],
            cwd=FUZZER_DIR,
            check=True
        )
    except SystemExit:
    ...
    print("=== Finished Sync Stress Test ===")
...
def main():
    parser = argparse.ArgumentParser(description="Run Sync Stress Test")
