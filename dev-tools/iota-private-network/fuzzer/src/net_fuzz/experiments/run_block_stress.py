...
def run_experiment():
    """Runs the block stress test."""
    print(f"\n{'='*60}")
    print("=== Starting Block Stress Test ===")
    print(f"{'='*60}\n")
    ...
    # We execute the module net_fuzz.experiments.block_stress
    env = os.environ.copy()
    env["PYTHONPATH"] = str(FUZZER_DIR / "src")

    try:
        run_command(
            ["sudo", str(venv_python), "-m", "net_fuzz.experiments.block_stress"],
            cwd=FUZZER_DIR,
            check=True
        )
    except SystemExit:
    ...
    print("=== Finished Block Stress Test ===")
...
def main():
    parser = argparse.ArgumentParser(description="Run Block Stress Test")
