...
pub struct Bullshark {
    ...
    /// The protocol config settings allowing us to enable/disable features and
    /// support properties according to the supported protocol version.
    ...
    /// The number of committed subdags that will trigger the schedule change
    /// and reputation score reset.
    pub num_sub_dags_per_schedule: u64,
    /// The leader election schedule to be used when need to find a round's
    /// leader
...
impl Bullshark {
    ...
    /// Calculates the reputation score for the current commit by taking into
    /// account the reputation scores from the previous commit (assuming
    /// that exists). It returns the updated reputation score.
    fn resolve_reputation_score(
        ...
        // we reset the scores for every schedule change window, or initialise when it's
        // the first sub dag we are going to create.
        // TODO: when schedule change is implemented we should probably change a little
        // bit this logic here.
        ...
        // then this is the first time we commit a leader, so no score update takes
        // place
        ...
        // we check if this is the last sub dag of the current schedule. If yes then we
        // mark the scores as final_of_schedule = true so any downstream user
        // can now that those are the last ones calculated for the current
        // schedule.
    ...
    pub fn process_certificate(
        ...
        // have at least f+1 certificates. This is because we need them to
        // provide enough support to the leader.
        let r = round - 1;
        ...
        // Get the certificate's digest of the leader. If we already ordered this
        ...
        // Performance note: if tracing at the debug log level is disabled, this is
        // cheap, see https://github.com/tokio-rs/tracing/pull/326
    ...
    /// Commits the leader of round `leader_round`. It is also recursively
    /// committing any earlier leader that hasn't been committed, assuming
    /// that's possible. If the schedule has changed due to a commit and
    /// there are more leaders to commit, then this method will return the
    /// enum `ScheduleChanged` so the caller will know to retry for the
    ...
    fn commit_leader(
        ...
        // If it is the case, we can commit the leader. But first, we need to
        // recursively go back to the last committed leader, and commit all
        ...
        while let Some(leader) = leaders_to_commit.pop_front() {
            ...
            // Starting from the oldest leader, flatten the sub-dag referenced by the
            // leader.
            ...
            // We resolve the reputation score that should be stored alongside with this sub
            // dag.
            ...
            // If the leader schedule has been updated, then we'll need to recalculate any
            // upcoming leaders for the rest of the recursive commits. We do
            // that by repeating the leader election for the round that
            // triggered the original commit
            if self.update_leader_schedule(leader.round(), &reputation_score) {
                ...
                // until, the `leader_round`, otherwise we have committed
                // everything we could and practically the leader of
                // `leader_round` is the one that changed the schedule.
    ...
    /// Order the past leaders that we didn't already commit. It orders the
    /// leaders from the one of the older (smaller) round to the newest
    /// round.
    fn order_leaders(&self, leader: &Certificate, state: &ConsensusState) -> VecDeque<Certificate> {
            ...
            .step_by(2)
        {
            ...
            if self.linked(leader, prev_leader, &state.dag) {
                // always add on the front so in the end we create a list with the leaders
                // ordered from the lowest to the highest round.
    ...
    // When the provided `reputation_scores` are "final" for the current schedule
    // window, then we create the new leader swap table and update the leader
    // schedule to use it. Otherwise we do nothing. If the schedule has been
    // updated then true is returned.
    ...
    fn report_leader_on_time_metrics(&mut self, certificate_round: Round, state: &ConsensusState) {
            ...
            && certificate_round > 2
        {
            ...
            // This metric reports the leader election success for the last leader election
            // round. Our goal is to identify the rate of missed/failed leader
            // elections which are a source of tx latency. The metric's
            // authority label can not be considered fully accurate when
            // we do change schedule as we'll try to calculate the previous leader round by
            // using the updated scores and consequently the new swap table. If
            // the leader for that position has changed, then a different
            // hostname will be erroneously reported. For now not a huge
