...
export function SwapPage() {
    ...
    const validationSchema = useMemo(() => {
        return z
            ...
            .superRefine(async ({ amount }, ctx) => {
                ...
                const bnMaxBalance = new BigNumber(totalBalance || 0).shiftedBy(
                    -1 * (data?.decimals ?? 0),
                );
    ...
    const [allowedMaxSlippagePercentage, amount] = watch([
                        ...
                        <Form form={form} onSubmit={handleOnsubmit}>
                                        ...
                                        info={
                                            ...
                                            !!amount && (
                                                <Text variant="subtitleSmall" color="steel-dark">
                                                    {isMaxBalance ? '~ ' : ''}$
                                                    {new BigNumber(inputAmountInUSD || 0).toFixed(
                                                        2,
                                                    )}
                                                </Text>
                                            )
                                ...
                                {showGasFeeBanner && (
                                    <Alert mode="warning">
                                        <Text variant="pBodySmall">
                                            {GAS_RESERVE} {fromCoinSymbol} has been set aside to
                                            cover estimated max gas fees for this transaction
                                        </Text>
                                    </Alert>
                                )}
                            </div>
                            ...
                            <div className="flex flex-col gap-4">
                                ...
                                >
                                    ...
                                    <div
                                        className={clsx(
                                            'flex h-[42px] items-center rounded-2lg border bg-gray-40 px-3 py-2',
                                            {
                                        ...
                                        onClose={() => {
                                            navigate(
                                                `/swap?${new URLSearchParams({
                                                    ...
                                                    toType: toCoinType || '',
                                                    maxSlippage:
                                                        allowedMaxSlippagePercentage.toString(),
                                                }).toString()}`,
                                            );
                                ...
                                {swapData?.estimatedRate && (
                                    <div className="flex flex-col gap-4 rounded-xl border border-solid border-hero-darkest/20 px-5 py-3">
                                        <DescriptionItem
                                            title={<Text variant="bodySmall">Estimated Rate</Text>}
                                        >
                                            <Text
                                                variant="bodySmall"
                                                weight="medium"
                                                color="steel-darker"
                                            >
                                                1 {fromCoinSymbol} ≈ {swapData?.estimatedRate}{' '}
                                                {toCoinSymbol}
                                            </Text>
                                        </DescriptionItem>
                                    </div>
