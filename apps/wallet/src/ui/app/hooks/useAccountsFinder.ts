...
import { SourceStrategyToFind, SourceStrategyToPersist } from '_src/shared/messaging/messages/payloads/accounts-finder';
...
export function useAccountsFinder({
        ...
        return new AccountsFinder({
            ...
            getPublicKey: async (bipPath) => {
                if (sourceStrategy.type == "ledger") {
                    ...
                    const { address } = await backgroundClient.deriveBipPathAccountsFinder(sourceStrategy.sourceID, {
                        ...bipPath,
                        coinType
                    })
                    return address
    ...
    }, [client, backgroundClient, accountType, coinType,  gasType,  accountGapLimit, addressGapLimit, sourceStrategy,])
    ...
    async function find() {
        ...
        if (sourceStrategy.type == "ledger") {
            ...
            const client = ledgerIotaClinet.iotaLedgerClient!;
            const addresses = await Promise.all(bipPaths.map(async (bipPath) => {
                const derivationPath = makeDerivationPath(bipPath);
                const publicKeyResult = await client.getPublicKey(derivationPath);
                const publicKey = new Ed25519PublicKey(publicKeyResult.publicKey);
                return {
