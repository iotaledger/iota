...
module capy::capy_winter {
    ...
    struct CapyPost has key { id: UID, balance: Balance<IOTA> }
    ...
    entry fun buy_gift(post: &mut CapyPost, type: u8, payment: vector<Coin<IOTA>>, ctx: &mut TxContext) {
    ...
    fun merge_and_split(
        coins: vector<Coin<IOTA>>, amount: u64, ctx: &mut TxContext
    ): (Coin<IOTA>, Coin<IOTA>) {
