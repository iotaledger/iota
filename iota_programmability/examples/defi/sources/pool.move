...
/// Example implementation of a liquidity Pool for Iota.
///
/// - Only module publisher can create new Pools.
/// - For simplicity's sake all swaps are done with IOTA coin.
/// Generalizing to swaps between non-IOTA coin types requires a few more generics, but is otherwise straightforward
...
///   use iota::iota::IOTA;
///
///   public struct POOL_TEAM has drop {}
///
...
///     ctx: &mut TxContext
///   ) {
///     pool::create_pool(POOL_TEAM {}, token, iota, fee_percent, ctx)
///   }
/// }
/// ```
///
...
module defi::pool {
    ...
    /// and a `Coin<IOTA>`. Swaps are available in both directions.
    ///
    ...
    public fun create_pool<P: drop, T>(
        ...
        let iota_amt = coin::value(&iota);
        let tok_amt = coin::value(&token);

        assert!(iota_amt > 0 && tok_amt > 0, EZeroAmount);
        assert!(iota_amt < MAX_POOL_VALUE && tok_amt < MAX_POOL_VALUE, EPoolFull);
        assert!(fee_percent >= 0 && fee_percent < 10000, EWrongFee);

        // Initial share of LSP is the sqrt(a) * sqrt(b)
        let share = math::sqrt(iota_amt) * math::sqrt(tok_amt);
    ...
    /// Entrypoint for the `swap_iota` method. Sends swapped token
    /// to sender.
    entry fun swap_iota_<P, T>(
        pool: &mut Pool<P, T>, iota: Coin<IOTA>, ctx: &mut TxContext
    ) {
        transfer::public_transfer(
            swap_iota(pool, iota, ctx),
            tx_context::sender(ctx)
        )
    }

    /// Swap `Coin<IOTA>` for the `Coin<T>`.
    /// Returns Coin<T>.
    public fun swap_iota<P, T>(
        pool: &mut Pool<P, T>, iota: Coin<IOTA>, ctx: &mut TxContext
    ): Coin<T> {
        assert!(coin::value(&iota) > 0, EZeroAmount);

        let iota_balance = coin::into_balance(iota);

        // Calculate the output amount - fee
        let (iota_reserve, token_reserve, _) = get_amounts(pool);

        assert!(iota_reserve > 0 && token_reserve > 0, EReservesEmpty);

        let output_amount = get_input_price(
            balance::value(&iota_balance),
            iota_reserve,
            token_reserve,
            pool.fee_percent
        );

        balance::join(&mut pool.iota, iota_balance);
        coin::take(&mut pool.token, output_amount, ctx)
    }

    /// Entry point for the `swap_token` method. Sends swapped IOTA
    /// to the sender.
    ...
    /// Swap `Coin<T>` for the `Coin<IOTA>`.
    /// Returns the swapped `Coin<IOTA>`.
    public fun swap_token<P, T>(
        ...
        coin::take(&mut pool.iota, output_amount, ctx)
    }

    /// Entrypoint for the `add_liquidity` method. Sends `Coin<LSP>` to
    /// the transaction sender.
    entry fun add_liquidity_<P, T>(
        pool: &mut Pool<P, T>, iota: Coin<IOTA>, token: Coin<T>, ctx: &mut TxContext
    ) {
        transfer::public_transfer(
            add_liquidity(pool, iota, token, ctx),
            tx_context::sender(ctx)
        );
    }

    /// Add liquidity to the `Pool`. Sender needs to provide both
    /// `Coin<IOTA>` and `Coin<T>`, and in exchange he gets `Coin<LSP>` -
    ...
    public fun add_liquidity<P, T>(
        ...
        let (iota_amount, tok_amount, lsp_supply) = get_amounts(pool);

        let iota_added = balance::value(&iota_balance);
        let tok_added = balance::value(&tok_balance);
        let share_minted = math::min(
            (iota_added * lsp_supply) / iota_amount,
            (tok_added * lsp_supply) / tok_amount
        );

        let iota_amt = balance::join(&mut pool.iota, iota_balance);
        let tok_amt = balance::join(&mut pool.token, tok_balance);

        assert!(iota_amt < MAX_POOL_VALUE, EPoolFull);
    ...
    entry fun remove_liquidity_<P, T>(
        ...
        let (iota, token) = remove_liquidity(pool, lsp, ctx);
        let sender = tx_context::sender(ctx);

        transfer::public_transfer(iota, sender);
        transfer::public_transfer(token, sender);
    }

    /// Remove liquidity from the `Pool` by burning `Coin<LSP>`.
    /// Returns `Coin<T>` and `Coin<IOTA>`.
    public fun remove_liquidity<P, T>(
        ...
        let (iota_amt, tok_amt, lsp_supply) = get_amounts(pool);
        let iota_removed = (iota_amt * lsp_amount) / lsp_supply;
        let tok_removed = (tok_amt * lsp_amount) / lsp_supply;

        balance::decrease_supply(&mut pool.lsp_supply, coin::into_balance(lsp));

        (
            coin::take(&mut pool.iota, iota_removed, ctx),
            coin::take(&mut pool.token, tok_removed, ctx)
        )
    }

    /// Public getter for the price of IOTA in token T.
    /// - How much IOTA one will get if they send `to_sell` amount of T;
    public fun iota_price<P, T>(pool: &Pool<P, T>, to_sell: u64): u64 {
        let (iota_amt, tok_amt, _) = get_amounts(pool);
        get_input_price(to_sell, tok_amt, iota_amt, pool.fee_percent)
    }

    /// Public getter for the price of token T in IOTA.
    /// - How much T one will get if they send `to_sell` amount of IOTA;
    public fun token_price<P, T>(pool: &Pool<P, T>, to_sell: u64): u64 {
        let (iota_amt, tok_amt, _) = get_amounts(pool);
        get_input_price(to_sell, iota_amt, tok_amt, pool.fee_percent)
    }
...
module defi::pool_tests {
    use iota::iota::IOTA;
    use iota::coin::{Self, Coin, mint_for_testing as mint};
    ...
    /// Init a Pool with a 1_000_000 BEEP and 1_000_000_000 IOTA;
    /// Set the ratio BEEP : IOTA = 1 : 1000.
    ...
    fun test_init_pool_(test: &mut Scenario) {
        ...
        next_tx(test, owner);
        {
            ...
            let (amt_iota, amt_tok, lsp_supply) = pool::get_amounts(&pool);

            assert!(lsp_supply == 31622000, 0);
            assert!(amt_iota == IOTA_AMT, 0);
    ...
    /// The other guy tries to exchange 5_000_000 iota for ~ 5000 BEEP,
    /// minus the commission that is paid to the pool.
    fun test_swap_iota_(test: &mut Scenario) {
        test_init_pool_(test);
    ...
    /// The owner swaps back BEEP for IOTA and expects an increase in price.
    /// The sent amount of BEEP is 1000, initial price was 1 BEEP : 1000 IOTA;
    fun test_swap_tok_(test: &mut Scenario) {
        test_swap_iota_(test);
    ...
    fun test_withdraw_almost_all_(test: &mut Scenario) {
        ...
        next_tx(test, owner);
        {
            ...
            let (iota_reserve, tok_reserve, lsp_supply) = pool::get_amounts(pool_mut);

            assert!(lsp_supply == 1, 3);
            assert!(tok_reserve > 0, 3); // actually 1 BEEP is left
            assert!(iota_reserve > 0, 3);
