...
module basics::sandwich {
    ...
    public fun collect_profits(_cap: &GroceryOwnerCapability, grocery: &mut Grocery, ctx: &mut TxContext): Coin<IOTA> {
    ...
    fun test_make_sandwich() {
        ...
        test_scenario::next_tx(scenario, the_guy);
        {
            ...
            let ham = sandwich::buy_ham(
                grocery,
                coin::mint_for_testing<IOTA>(10, ctx),
                ctx
            );

            let bread = sandwich::buy_bread(
                grocery,
                coin::mint_for_testing<IOTA>(2, ctx),
                ctx
            );
