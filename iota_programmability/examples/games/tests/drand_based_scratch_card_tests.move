...
module games::drand_based_scratch_card_tests {
    ...
    fun mint(addr: address, amount: u64, scenario: &mut Scenario) {
        transfer::public_transfer(coin::mint_for_testing<IOTA>(amount, test_scenario::ctx(scenario)), addr);
    ...
    fun test_play_drand_scratch_card_with_winner() {
        ...
        let coin1 = test_scenario::take_from_sender<Coin<IOTA>>(scenario);
        ...
        loop {
            ...
            let coin2 = test_scenario::take_from_sender<Coin<IOTA>>(scenario);
