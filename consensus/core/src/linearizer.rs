...
impl Linearizer {
    ...
    /// Calculates the commit's timestamp. If the median-based timestamp
    /// calculation is enabled, then the timestamp will be calculated as the
    /// median of leader's parents (leader.round - 1) timestamps by stake.
    /// Otherwise, the leader's timestamp will be used. To ensure that commit
    /// timestamp monotonicity is respected, it is compared against the
...
/// total stake is less than quorum threshold.
...
mod tests {
    ...
    async fn test_handle_already_committed(
        ...
        // Mark the blocks as committed in DagState. This will allow to correctly detect
        // the committed blocks when the new linearizer logic is enabled.
