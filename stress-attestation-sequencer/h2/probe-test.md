...
One cost per configuration is the control rather than the experiment: at a
single cost a unit limit of `10 × C` admits the same ten transactions a count
limit of 10 does, so the two modes are expected to produce the same numbers, and
measuring that they do is what makes the grid trustworthy. The modes can only
