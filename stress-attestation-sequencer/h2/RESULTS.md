...
drain rate is 118 tx/s there and 3.7 tx/s at 5,000,000. Both modes admit more
...
- 89 of the 138 configurations are fixed-cost: within each one, every
...
Of the 138 configurations, 89 give every transaction in a run the same cost,
and 49 mix two or three costs in one commit.

- The 89 fixed-cost configurations are the control, not the experiment. When
  every transaction has the same cost, a unit limit of `10 × cost` admits the
  same ten transactions per commit that a count limit of 10 admits, so the two
  modes should produce the same numbers. Measuring that they do is what makes
  the grid trustworthy (findings 1–6).
- The 49 mixed-cost configurations are the experiment. The modes can only
  differ when one commit carries transactions of different cost (finding 7).

Driven by `stress-attestation-sequencer/h2/matrix.sh` (each configuration
calls `run.sh`, which bootstraps a fresh network, runs A, resets to the same
...
- **Workload**: `slow::slow(n, size)` with `size = 100`, shared-object form
  ...
  transaction costs the 1,000 CU floor on one hot object, which is what
  the `cu1k` configurations measure with `slow`.
- **Cost points** (`slow_n` → attested CUs per transaction,
  ...
  ceiling: 5,000,000 CUs is the gas budget expressed in CUs, so those
...
- **Run A**: `TotalTxCount`, limit 10, overshoot 0. Production runs this
  ...
  Three mixed-cost configurations were then rerun with the overshoot on to check
  what it changes, which is nothing measurable (finding 7).
- **Run B**: `TotalComputationUnits`, limit `LIMIT_B` in CUs, overshoot 0. Per
  cost point the limits step geometrically from one step *below* the
  transaction's own cost (which admits nothing) to where the limit stops
  binding, capped at 50,000,000 (ten ceiling-cost transactions): 4 to 8 limits
  per point, 80 configurations. The limit at `10 × cost` is the count limit's
  equivalent, one per cost point.
- **Mixed-cost configurations** (25): `SLOW_MIX` draws each transaction's `n`
  from two (or three) levels with fixed weights, so one commit holds
  transactions of different costs. Named by their mean cost and expensive share,
  e.g. `mix10900-w10`: 1,000 and 100,000 CUs, 9:1. Five mixes were run at
  `LIMIT_B = 10 × mean cost`, the count limit's equivalent (`mix1900`,
  `mix3700`, `mix10900`, `mix20800`, `mix50900`). A second round added 20 more:
  limit ladders on four of those mixes (2–4 further limits each, including one
  below the expensive cost); the same expensive level with a costlier cheap
  side, near the 10K and 55K means (`mix9500` 5K/50K, `mix11800` 2K/100K,
  `mix54500` 5K/500K, `mix68000` 20K/500K); 1K/100K at 30 % and 50 % expensive
  (`mix30700`, `mix50500`); one three-level mix (`mix13600`: 1K/10K/100K at
  60/30/10 %); and 300 s runs of two configurations (`-dur300`). Each mix's
  control is the fixed-cost configuration of the same mean. A third round then
  checked what the first two had held fixed: 300 s runs of two more
  configurations, the recommended `mix20800` limit and the limit at twice the
  expensive cost at `mix10900` (`-dur300`, 4 in all); three configurations with
  production's overshoot on (`-burst`); and 14 configurations at double the
  submission rate, 9 fixed-cost and 5 mixed (`-qps2000`). Finally the three mix
  ladders — `mix20800` (4 configurations), `mix3700` (5) and `mix50900` (4) —
  were run on the second machine of `probe-test.md`, a Ryzen 9 9950X3D
  workstation (WS below), where a 100K-unit transaction executes in 7.4 ms
  instead of 34, a 10K one in 3.5 ms instead of 16 and a 500K one in 17.7 ms
  instead of 81 — with `-ws` labels, kept under `results/matrix-ws/` so they
  never mix with the EPYC data.
...
- **Rate**: 1,000 tx/s for the main grid, and 2,000 tx/s for 14 configurations
  rerun to check it. At 1,000 CUs the limit already binds at the lower rate,
  and from 5,000 CUs up the client's in-flight cap lowers what it offers to
  what can execute on the object, so for the heavy fixed-cost configurations a
  higher target changes little (finding 3). For the mixes, where most
  transactions are cheap, it changes a lot: several configurations turn out to
  have been limited by the client rather than by the limit at 1,000 tx/s
  (finding 7).
...
- **138 configurations**: the 80 fixed-cost configurations at **10 iterations**
  ...
  **11** (one iteration was run first to check the configuration, then ten
  ...
  more 300 s configurations, 3 with the overshoot on and 14 at 2,000 tx/s, 190
  ...
  runs and `-burst` for the overshoot ones. The WS adds 13 configurations at 5
...
- `aggregate.py` pools every label's iterations (histogram buckets summed before
  quantiles; rates averaged over runs) into one A-vs-B row per configuration:
  `results/matrix/summary.md` for reading, `results/matrix/summary.csv` for
  plotting. It also writes the spread of each value across the iterations, the
  rate of transactions executed at the expensive level of a mix, the per-commit
  admission histogram (`admits_hist.csv`) and the checkpoint lag per 10 s and 60
  s slice of the window (`lag_over_time.csv`).
...
## Findings (10 iterations each, 11 for the first five mixes, 5 on the WS)
...
spread across those configurations is the run-to-run noise: within ±3 % of the
mean everywhere except `cu2k` (156–176 tx/s). Within one configuration, the
standard deviation of success throughput across its iterations is 1–4 % of the
mean at most cost points, up to 12 % at `cu1k` and 14 % at `cu5m`, where the
offered load varies most; `results/matrix/summary.md` lists it per
configuration, and the figures draw it as error bars.
...
- **The offered load is not 1,000 tx/s.** The client keeps at most 2,000
  ...
  2,000 divided by the current latency. At 1,000 CUs that is the full
  target; at 5,000 CUs about 200 tx/s arrive; at 500,000 about 80; at
...
rate in every configuration of a point.
...
- Run A sits at its limit wherever enough transactions arrive: 9.98–10.00 in
  every `cu1k` and `cu2k` configuration.
- Run B sits at `LIMIT_B / cost`, rounded down. A limit of 50,000 CUs admits
  exactly 2.000 transactions of 20,000 CUs, never a partial third. That holds
  in all 25 configurations where the quotient is at or below what the object
  can execute per commit:
...
The worst deviation is 0.69 % (`cu1k-lim50k`, 49.65 for 50, where 50 per commit
is also all that arrives). Most are exact to three decimals.
...
B cancels everything that arrives after ten rounds — 740–950/s in six of the
eight, fewer in the two heaviest for the reason below — and completes
0.4–1.7 tx/s. Run A in the same configuration is unaffected. Above the object's
capacity the limit stops mattering and Run B's admits track Run A's instead
(finding 3).
...
> per second against Run A's 19.7 and 18.5, with skipped leader rounds climbing
> to 74–77 per run from Run A's 24–45 (finding 6). Every transaction in those
> runs is deferred and re-evaluated every commit at 2–5 million CUs each, and
> the handler fell behind consensus. It is the only place in the grid where the
> mode changed the commit rate. Why the handler fell behind is left for a
> follow-up.

---

**2. At one cost per configuration, the two modes produce the same numbers — the
control holds at every cost point.**
...
- B/A on success throughput averages 1.005 over the twelve, between 0.984 and
  1.025. That is inside the run-to-run noise of Run A itself: the standard
  deviation across iterations is 1–4 % of the mean for both runs at most cost
  points, so none of the ratios can be told from 1. Settlement latency matches
  the same way (finding 5).
- This is the expected result, and it is what makes the rest of the grid
  usable. A limit in CUs and a limit in count are interchangeable at uniform
  cost, so any difference in a mixed-cost configuration is the cost spread and
  nothing else.
- The other 68 configurations vary `LIMIT_B` away from that equivalence, and
  are read in findings 3 and 4.

![Run A against Run B at the twelve matched configurations](results/matrix/summary_plots/modes_matched.png)

*The twelve matched configurations: Run A (star) next to Run B (dot) with one
...
![Values for Run A and every Run B configuration](results/matrix/summary_plots/modes_heatmaps.png)

*Every configuration at a glance: success tps, cancelled share, checkpoint lag
mean and share past 30 s, one panel each, Run A in the left column and each
`LIMIT_B` limit to its right, coloured on one scale per panel so equal values
look equal everywhere (success and lag on a log colour scale). The outlined `10
× cost` squares match the Run A column in every panel.*

---

**3. Throughput is set by how fast transactions on one object execute, one after
another, and from 5,000 CUs up neither limit reaches it.**
...
Transactions on one mutable shared object execute one after another, so for
each cost there is a top rate at which they complete: the success rate in the
configurations whose limit admits more than that. Read off the grid, it
falls 50-fold across the cost range while the count limit stays at 10 per commit
throughout:

| Cost point | Units/tx | Drain rate (tx/s) | Arrivals/s | Scheduled/s | Success tps | Scheduled / success |
| --- | --- | --- | --- | --- | --- | --- |
...
- at 1,000 CUs, the full 1,000 tx/s target (943 arrive);
- at 5,000 CUs, about 200 tx/s, because settlement takes 6.6 s at the median;
- at 5,000,000 CUs, 40 tx/s.

So the configurations from `cu5k` up do not measure the network under a
1,000 tx/s load. They measure it under whatever load its own latency lets
through. The target rate describes the two lightest cost points only.

*The scheduler admits more than executes on the object.* Scheduled exceeds
success by 12 % at 2,000 CUs and by 7× at 5,000,000 — every scheduled
...
commits where the object received anything:
...
- The share of commits that scheduled anything at all falls from 95 % at
  `cu1k` to 25 % at `cu5m`.
...
> it, because with overshoot 0 there is none, and the suggested-gas-price code
> is advisory on this path. Worth a dedicated look.
...
bend, the count limit admits more than can execute on the object; left of it,
less.*
...
Along each cost point's ladder the same picture repeats.

- While the limit admits fewer transactions than can execute on the object, the
  surplus is deferred and cancelled at ten rounds: 800–950/s, most of what
...
- At the first limit whose admitted rate reaches the drain rate, the picture
  turns around. Cancellations fall towards zero, success flattens at the drain
  rate, and lag jumps by an order of magnitude and keeps growing with the
  limit:

| Cost point | Drain (tx/s) | Last limit under it: admitted/s → lag s, cancelled/s | First limit over it: admitted/s → lag s, cancelled/s |
| --- | --- | --- | --- |
...
Past that point, raising the limit further changes little:
...
> [!IMPORTANT] This is the actual trade the limit makes. A tight limit turns the
> surplus into cancellations, which the client sees as failures within ten
> rounds and can resubmit. A loose one turns it into a queue, which the client
> sees as latency and the network as checkpoint lag. Success throughput is the
> same either way once execution on the object is saturated, so no setting of
> either mode
> raises it. The only choice is which failure the users get.

![Success tps against checkpoint lag per configuration](results/matrix/summary_plots/modes_tradeoff.png)

*Success tps against checkpoint lag mean, one point per configuration, Run A
...
enters only through how busy execution on the object is, and the two outcomes
trade places at the dashed line.*
...
- A transaction's deferral key records the leader round it was first deferred
  ...
  (`consensus_handler.rs:243`, `consensus_output.leader_round()`).
...
ms, a 500K one ≈81 ms, and a cheap one 0.6 ms (`probe-test.md`). Working each
...
**PASS.** Across all 2,492 runs of the 125 configurations on EPYC and the 128
...
The metric descriptions are in `../h1/RESULTS.md`, H4 section. The only
...
- per-configuration numbers on EPYC: `results/matrix/summary.md` and
  `summary.csv`;
- the same for the WS: `results/matrix-ws/summary.md`;
- figures: `results/matrix/summary_plots/`;
- the cost calibration behind the grid: `probe-test.md`.
