...
/// Implementation of the Move native function `secp256k1_ecrecover(signature:
...
pub struct EcdsaK1Secp256k1VerifyCostParams {
    /// Base cost for invoking the `secp256k1_verify` function with `hash=0`
    /// implying KECCAK256
    ...
    /// Base cost for invoking the `secp256k1_verify` function with `hash=1`
    /// implying SHA256
...
/// Implementation of the Move native function `secp256k1_verify(signature:
/// &vector<u8>, public_key: &vector<u8>, msg: &vector<u8>, hash: u8): bool`
/// This function has two cost modes depending on the hash being set
/// to`KECCAK256` or `SHA256`. The core formula is same but constants differ. If
