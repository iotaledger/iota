...
mod checked {
    ...
    fn create_and_freeze_package_metadata_if_present(
        ...
        let mut modules_metadata = BTreeMap::new();
        ...
        for module in modules {
                ...
                .find(|md| md.key == IOTA_METADATA_KEY.to_vec())
            {
                ...
                // relevant metadata
                if !fn_metadata.is_empty() {
                    modules_metadata.insert(module.name().to_string(), fn_metadata);
                }
        ...
        if !modules_metadata.is_empty() {
            ...
            let metadata_wrapper = PackageMetadata::new(
                metadata_uid,
                storage_id,
                runtime_id,
                package_version,
        ...
        match &fn_signature.0[0] {
            SignatureToken::Reference(ref_param) => {
                if let Some(type_tag) =
                    move_binary_format::normalized::Type::new(module, ref_param).into_type_tag()
                {
                    Ok(type_tag)
                } else {
