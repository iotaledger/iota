...
//! This module implements support for tracing related to PTB execution.
//! IMPORTANT: Bodies of all public functions in this module should be enclosed
//! in a large if statement checking if tracing is enabled or not to make sure
//! that any errors coming from these functions only manifest itself
...
/// Inserts Move call start event into the trace. As is the case for all other
/// public functions in this module, its body is (and must be) enclosed in an if
/// statement checking if tracing is enabled.
...
/// Inserts Move call end event into the trace. As is the case for all other
/// public functions in this module, its body is (and must be) enclosed in an if
/// statement checking if tracing is enabled.
...
/// Inserts transfer event into the trace. As is the case for all other public
/// functions in this module, its body is (and must be) enclosed in an if
/// statement checking if tracing is enabled.
...
/// Inserts PTB summary event into the trace. As is the case for all other
/// public functions in this module, its body is (and must be) enclosed in an if
/// statement checking if tracing is enabled.
...
/// Inserts split coins event into the trace. As is the case for all other
/// public functions in this module, its body is (and must be) enclosed in an if
/// statement checking if tracing is enabled.
...
fn coin_obj_info(
    ...
    // object.ID
    let object_id = MoveValue::Address(object_id.into_bytes().into());
    ...
    let object_id_struct = MoveStruct {
        type_: object_id_struct_tag,
        fields: vec![(Identifier::new("value").unwrap(), object_id)],
    };
    let serializable_object_id = MoveValue::Struct(object_id_struct);
    // object.UID
    ...
    let object_uid_struct = MoveStruct {
        type_: object_uid_struct_tag,
        fields: vec![(Identifier::new("id").unwrap(), serializable_object_id)],
    };
    let serializable_object_uid = MoveValue::Struct(object_uid_struct);
    // coin.Balance
    let serializable_value = MoveValue::U64(balance);
    ...
    let balance_struct = MoveStruct {
        type_: balance_struct_tag,
        fields: vec![(Identifier::new("value").unwrap(), serializable_value)],
    };
    let serializable_balance = MoveValue::Struct(balance_struct);
    // coin.Coin
    let coin_obj = MoveStruct {
        type_: *coin_type_tag,
        fields: vec![
    ...
    Ok(ExtMoveValueInfo {
        type_: type_tag_with_refs,
        value: MoveValue::Struct(coin_obj),
    })
}
