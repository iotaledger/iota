                ...
                |()| -> ResultWithTimings<Mode::ExecutionResults, ExecutionError> {
                    ...
                    let mut execution_result = pre_execution_result_opt
                        ...
                        .and_then(|_| {
                            ...
                            // objects for attested transactions. Runs only after Move
                            // authentication has succeeded, so an authentication failure —
                            // which is not attributable to the issuer — is never masked by a
                            // coin-deny failure that would charge the issuer. Enforced here so
                            // a stale-attestation view resolves to an execution failure
                            // instead of being dropped.
                            //
                            // Gated on the attestation flag. `UserTransactionV1` is rejected
                            // by the consensus verifier once the flag is on, so every user
                            // transaction reaching this point is attested; epochs before the
                            // flag flips are unaffected. System transactions are never
                            // attested and hold no address-owned coin inputs, so they are
                            // excluded.
