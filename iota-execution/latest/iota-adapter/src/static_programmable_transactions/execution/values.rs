...
pub enum ByteValue {
    Pure(Vec<u8>),
    Receiving { id: ObjectId, version: Version },
}
...
/// A set of memory locations that can be borrowed or moved from. Used for
/// inputs and results
...
impl Inputs {
    ...
    pub fn get(&mut self, index: u16) -> Result<InputValue<'_>, ExecutionError> {
...
impl Value {
    ...
    /// Read the value, giving an invariant violation if the value is not a
    /// reference
...
impl Value {
    ...
    pub fn receiving(id: ObjectId, version: Version) -> Self {
        Self(VMValue::struct_(Struct::pack([
            Self::uid(AccountAddress::new(id.into_bytes())).0,
            VMValue::u64(version.as_u64()),
        ])))
    }
...
impl Value {
    pub fn unpack_coin(self) -> Result<(ObjectId, u64), ExecutionError> {
        ...
        Ok((ObjectId::new(id.into_bytes()), balance))
    }
    ...
    /// The coin value MUST be checked before calling this function, if `amount`
    /// is greater than the value of the coin, it will return an invariant
    /// violation.
    ...
    /// The coin max value MUST be checked before calling this function, if
    /// `amount` plus the current balance is greater than `u64::MAX`, it
    /// will return an invariant violation.
