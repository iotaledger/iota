...
use move_core_types::language_storage::{ModuleId, TypeTag as MoveTypeTag};
...
pub struct Context<'env, 'pc, 'vm, 'state, 'linkage, 'gas> {
    ...
    /// A shared transaction context, contains transaction digest information
    /// and manages the creation of new object IDs
    ...
    inputs: Inputs,
    /// The results of a given command. For most commands, the inner vector will
    /// have length 1. It will only not be 1 for Move calls with multiple
    /// return values. Inner values are None if taken/moved by-value
    results: Vec<Locals>,
}
...
impl<'env, 'pc, 'vm, 'state, 'linkage, 'gas> Context<'env, 'pc, 'vm, 'state, 'linkage, 'gas> {
        ...
        'pc: 'state,
    {
        ...
        let gas = match gas_charger.gas_coin() {
            Some(gas_coin) => {
                ...
                // We have already checked that the gas balance is enough to cover the gas
                // budget
        ...
        let native_extensions = new_native_extensions(
            ...
            metrics.clone(),
            tx_context.clone(),
            env.state_view.read_auth_context(),
        );
    ...
    /// NOTE! This does not charge gas and should not be used directly. It is
    /// exposed for dev-inspect
    fn location_value<'a>(
        &'a mut self,
        location: T::Location,
        ty: Type,
    ...
    ) -> Result<Vec<(Argument, Vec<u8>, TypeTag)>, ExecutionError> {
        args.into_iter()
    ...
    fn location_update(
        ...
        location: T::Location,
        ty: Type,
    ) -> Result<Option<(Argument, Vec<u8>, TypeTag)>, ExecutionError> {
        let ty = match ty {
            Type::Reference(_, inner) => (*inner).clone(),
            ty => ty,
        };
        let Ok(tag): Result<MoveTypeTag, _> = ty.clone().try_into() else {
            invariant_violation!("unable to generate type tag from type")
        };
        let tag = type_tag_core_to_sdk(&tag);
        ...
        let arg = match location {
            T::Location::GasCoin => Argument::Gas,
            T::Location::Input(i) => Argument::Input(i),
            T::Location::Result(i, j) => Argument::NestedResult(i, j),
        };
    ...
    fn tracked_result(
        ...
        let Ok(tag): Result<MoveTypeTag, _> = ty.try_into() else {
            invariant_violation!("unable to generate type tag from type")
        };
        Ok((bytes, type_tag_core_to_sdk(&tag)))
    }
}
...
fn load_object_arg_impl(
    ...
    let ObjectInner {
        data: ObjectData::Struct(move_obj),
        ..
    } = obj.as_inner()
    else {
...
/// transaction, now we return exactly that amount. Gas will be charged by the
/// execution engine
...
fn create_written_object<Mode: ExecutionMode>(
    ...
    let Ok(type_tag): Result<MoveTypeTag, _> = type_.try_into() else {
        invariant_violation!("unable to generate type tag from type")
    };

    let struct_tag = match type_tag {
        MoveTypeTag::Struct(inner) => *inner,
        _ => invariant_violation!("Non struct type for object"),
    };
    MoveStruct::new_from_execution(
        struct_tag_core_to_sdk(&struct_tag),
