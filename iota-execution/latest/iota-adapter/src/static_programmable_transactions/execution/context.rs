...
impl<'env, 'pc, 'vm, 'state, 'linkage, 'gas> Context<'env, 'pc, 'vm, 'state, 'linkage, 'gas> {
    ...
    ) -> Result<Vec<(iota_types::transaction::Argument, Vec<u8>, TypeTag)>, ExecutionError> {
        args.into_iter()
    ...
    fn location_update(
        ...
        location: T::Location,
        ty: Type,
    ) -> Result<Option<(iota_types::transaction::Argument, Vec<u8>, TypeTag)>, ExecutionError> {
        use iota_types::transaction::Argument as TxArgument;
        let ty = match ty {
            Type::Reference(_, inner) => (*inner).clone(),
            ty => ty,
        };
...
fn load_object_arg_impl(
    ...
    let iota_types::object::ObjectInner {
