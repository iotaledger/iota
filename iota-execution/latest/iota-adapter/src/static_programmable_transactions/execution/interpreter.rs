    ...
    'pc: 'state,
{
    ...
    for (sp!(idx, command), tys) in commands {
        ...
        if let Err(err) = execute_command::<Mode>(
            ...
            // modified
            drop(context);
            // TODO wtf is going on with the borrow checker here. 'state is bound into the
            // object runtime, but its since been dropped. what gives with this
            // error?
            env.state_view
    ...
    let result = match command {
        ...
        T::Command_::TransferObjects(objects, recipient) => {
            ...
            for (object_value, ty) in object_values.into_iter().zip(object_tys) {
                ...
                let recipient = Owner::Address(Address::new(recipient.into_bytes()));
