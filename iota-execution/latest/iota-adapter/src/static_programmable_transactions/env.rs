...
//! This module defines the shared environment, `Env`, used for the
//! compilation/translation and execution of programmable transactions. While
//! the "context" for each pass might be different, the `Env` provides
//! consistent access to shared components such as the VM or the protocol
//! config.
...
use move_core_types::{annotated_value, language_storage::ModuleId, runtime_value};
...
impl<'pc, 'vm, 'state, 'linkage> Env<'pc, 'vm, 'state, 'linkage> {
    ...
    pub fn convert_vm_error(&self, e: VMError) -> ExecutionError {
        crate::error::convert_vm_error(e, self.vm, self.linkage_view)
    }
    ...
    pub fn module_definition(
        ...
        let data_store = IotaDataStore::new(self.linkage_view, &[]);
        self.vm
            .get_runtime()
    ...
    pub fn load_function(
        ...
        // let mut data_store = IotaDataStore::new(self.linkage_view, &[]);
        // let signature = self
        //     .vm
        //     .get_runtime()
        //     .load_function(
    ...
    pub fn load_type_input(&self, _idx: usize, _ty: TypeTag) -> Result<Type, ExecutionError> {
        // let tag = ty
        //     .into_type_tag()
    ...
    pub fn gas_coin_type(&self) -> Result<Type, ExecutionError> {
        get_or_init_ty!(self, gas_coin_type, StructTag::new_gas_coin())
    }
    ...
    pub fn upgrade_ticket_type(&self) -> Result<Type, ExecutionError> {
        get_or_init_ty!(
            self,
            upgrade_ticket_type,
            StructTag::new(Address::FRAMEWORK, "package", "UpgradeTicket", vec![])
        )
    }
    ...
    pub fn upgrade_receipt_type(&self) -> Result<Type, ExecutionError> {
        get_or_init_ty!(
            ...
            StructTag::new(Address::FRAMEWORK, "package", "UpgradeReceipt", vec![])
        )
    }
...
pub fn to_identifier(name: String) -> Result<Identifier, ExecutionError> {
