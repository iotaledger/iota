...
pub fn transaction(env: &Env, lt: L::Transaction) -> Result<T::Transaction, ExecutionError> {
    ...
    let commands = commands
        ...
        .map(|(i, c)| {
            ...
            let (c, tys) =
                command(env, &mut context, c).map_err(|e| e.with_command_index(i as u64))?;
            Ok((sp(idx, c), tys))
        })
    ...
    Items::IntoIter: ExactSizeIterator,
{
    fn splat_arg(
        ...
        match arg {
            L::Argument::Gas => res.push(T::Location::GasCoin),
            L::Argument::Input(i) => {
...
fn coin_mut_ref_argument_(
    ...
    Ok(match &actual_ty {
        ...
        LocationType::Bytes(_, _) => {
            // TODO we do not currently bytes in any mode as that would require additional
            // type inference not currently supported
            return Err(CommandArgumentError::TypeMismatch.into());
        }
    })
}
...
mod scope_references {
    ...
    /// To mimic proper scoping of references, the last usage of a reference is
    /// made a Move instead of a Copy.
    ...
    fn argument(used: &mut BTreeSet<(u16, u16)>, sp!(_, (arg_, ty)): &mut T::Argument) {
        ...
        match (&usage, ty) {
            ...
            (
                ...
                // we are at the last usage of a reference result if it was not yet added to the
                // set
