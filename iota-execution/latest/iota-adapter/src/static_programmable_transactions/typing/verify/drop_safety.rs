...
mod refine {
    ...
    /// After memory safety, we can switch the last usage of a `Copy` to a
    /// `Move` if it is not borrowed at the time of the last usage.
...
mod verify {
    ...
    pub fn transaction(_env: &Env, ast: &T::Transaction) -> Result<(), ExecutionError> {
        ...
        for (c, _t) in commands {
            let result =
                command(&mut context, c).map_err(|e| e.with_command_index(c.idx as u64))?;
