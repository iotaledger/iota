...
use iota_types::error::{ExecutionError, command_argument_error};
...
impl Context {
    ...
    /// Used for checking if a location is borrowed, primarily for recording
    /// this information on a specific Copy instance
    ...
    fn find_non_transferable(&self, refs: &BTreeSet<Ref>) -> Result<Option<Ref>, ExecutionError> {
        let borrows = refs
            .iter()
            .copied()
            ...
            for (borrower, paths) in borrowed_by {
                if !is_mut {
                    if mut_refs.contains(&borrower) {
                        ...
                        // the mut ref is not transferable
...
pub fn verify(_env: &Env, ast: &T::Transaction) -> Result<(), ExecutionError> {
    ...
    for (c, _t) in commands {
        let result = command(&mut context, c).map_err(|e| e.with_command_index(c.idx as u64))?;
...
fn call(
    ...
    let sources = arg_values
        ...
        .collect::<BTreeSet<_>>();
    if let Some(v) = context.find_non_transferable(&sources)? {
        let idx = arg_values
            .iter()
        ...
        assert_invariant!(
            idx < arg_values.len(),
            "non transferable value was not found in arguments"
        );
        return Err(command_argument_error(
            CommandArgumentError::InvalidValueUsage,
            idx,
