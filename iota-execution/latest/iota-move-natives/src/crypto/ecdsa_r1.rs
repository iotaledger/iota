...
pub struct EcdsaR1EcrecoverCostParams {
    /// Base cost for invoking the `ecrecover` function with `hash=0` implying
    /// KECCAK256
    ...
    ///  Cost per block of `msg` with `hash=0`implying KECCAK256, with block
    /// size = 136
    pub ecdsa_r1_ecrecover_keccak256_msg_cost_per_block: InternalGas,

    /// Base cost for invoking the `ecrecover` function with `hash=1` implying
    /// SHA256
    ...
    ///  Cost per block of `msg` with `hash=1`implying SHA256, with block size =
    /// 64
    pub ecdsa_r1_ecrecover_sha256_msg_cost_per_block: InternalGas,
}
/// ****************************************************************************
/// ********************* native fun secp256r1_ecrecover
/// Implementation of the Move native function `secp256r1_ecrecover(signature:
...
pub struct EcdsaR1Secp256R1VerifyCostParams {
    /// Base cost for invoking the `secp256r1_verify` function with `hash=0`
    /// implying KECCAK256
    ...
    /// Base cost for invoking the `secp256r1_verify` function with `hash=1`
    /// implying SHA256
...
/// Implementation of the Move native function `secp256r1_verify(signature:
/// &vector<u8>, public_key: &vector<u8>, msg: &vector<u8>, hash: u8): bool`
/// This function has two cost modes depending on the hash being set to
/// `KECCAK256` or `SHA256`. The core formula is same but constants differ.
/// If hash = 0, we use the `keccak256` cost constants, otherwise we use the
/// `sha256` cost constants.   gas cost: ecdsa_r1_secp256r1_verify_cost_base
/// | covers various fixed costs in the oper
///              + ecdsa_r1_secp256r1_verify_msg_cost_per_byte    * size_of(msg)
///                | covers cost of operating on each byte of `msg`
///              + ecdsa_r1_secp256r1_verify_msg_cost_per_block   *
