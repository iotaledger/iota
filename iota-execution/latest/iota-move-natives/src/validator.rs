...
use iota_types::iota_system_state::{
    iota_system_state_inner_v1::ValidatorMetadataV1,
    iota_system_state_inner_v2::ValidatorMetadataV2,
};
...
/// ****************************************************************************
/// *******************
pub fn validate_metadata_bcs_v1(
...
pub fn validate_metadata_bcs(
    ...
    let validator_metadata =
        bcs::from_bytes::<ValidatorMetadataV2>(&metadata_bytes).map_err(|_| {
            PartialVMError::new(StatusCode::MALFORMED).with_message(
                "ValidatorMetadata Move struct does not match internal ValidatorMetadataV2 struct"
                    .to_string(),
            )
        })?;
