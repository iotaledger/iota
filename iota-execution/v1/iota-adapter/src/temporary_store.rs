...
impl<'backing> TemporaryStore<'backing> {
    ...
    /// Due to the special treatment, we need to read from object store
    /// explicitly to obtain the modified_at information.
        ...
        for id in &self.execution_results.modified_objects {
            ...
            match &old_obj.owner {
                ...
                Owner::Immutable => {
                    ...
                    // Note: this assumes that the only immutable objects an epoch change tx can
                    // update are system packages, but in principle we could
                    // allow others.
                    assert!(
