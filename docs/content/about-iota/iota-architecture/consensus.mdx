...
- **Decoupling Block Header and Transactions**: Decoupling light block header metadata from transactions allow to achieve a higher throughput and implement a cordial dissemination of headers
- **Cordial Dissemination of Block Headers**: In a large network, connections between certain validators could be blocked,
- **Four-Round Commit Rule**: Transactions achieve finality in four rounds of messages.
- **Optimized Voting**: Unlike traditional leader-based protocols, Mysticeti allows validators to vote on blocks and certify them in parallel, significantly reducing median and tail latencies.
- **Fault Tolerance**: The protocol tolerates unavailable or faulty validators without significantly increasing commit latencies.

For more technical details, including correctness proofs, refer to the [Starfish paper](https://eprint.iacr.org/2025/567).
