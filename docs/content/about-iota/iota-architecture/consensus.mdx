...
The primary goal of consensus in distributed systems is to establish a consistent order of transactions and ensure their availability. In IOTA, consensus is achieved through a combination of Delegated Proof-of-Stake (dPoS) and a Byzantine Fault Tolerant (BFT) protocol. Currently, the Mainnet validators employ the Mysticeti BFT protocol, but they switch to Starfish BFT protocol in the future.
...
The **Starfish** protocol is a _BFT_ consensus protocol optimized for **low-latency** and **high-throughput** using an uncertified _DAG_ structure. It is designed to facilitate efficient and parallel transaction processing while maintaining strong security guarantees.

### Key Features of Starfish

- **Parallel Block Proposals**: All validators propose blocks simultaneously, maximizing network bandwidth and ensuring censorship resistance. This is a core advantage of DAG-based protocols.
- **Decoupling Block Header and Transactions**: Decoupling light block header metadata from transactions allows Starfish to achieve a higher throughput and implement a cordial dissemination of headers
- **Cordial Dissemination of Block Headers**: Even if multiple network paths or connections between validators are blocked, Starfish ensures that a single functioning connection is sufficient to propagate block headers without requiring retries or additional request-response cycles. This guarantees resilient and delay-free commits, even under network failures or adversarial conditions.
- **Encoding of Transaction Data**: Transactions are encoded using Reed-Solomon codes into shards, ensuring that even partial shard propagation allows nodes to recover missing data without repeated dissemination.
- **Optimized Voting**: Unlike traditional leader-based protocols, Mysticeti allows validators to vote on blocks and certify them in parallel, significantly reducing median and tail latencies.
- **Fault Tolerance**: The protocol tolerates unavailable or faulty validators without significantly increasing commit latencies.

For benchmark and technical details, including correctness proofs, refer to the [Starfish paper](https://eprint.iacr.org/2025/567).
