...
The primary goal of consensus in distributed systems is to establish a consistent order of transactions and ensure their availability. In IOTA, consensus is achieved through a combination of Delegated Proof-of-Stake (dPoS) and a Byzantine Fault Tolerant (BFT) protocol. Currently, the Testnet and Mainnet validators employ the Mysticeti BFT protocol, but they switch to the Starfish BFT protocol, already deployed on the Devnet, if validators will accept the protocol upgrade.
...
- **Energy efficiency**: Unlike Proof-of-Work (PoW), BFT consensus mechanism requires minimal computational resources.
...
Testing across IOTA Testnet, Mainnet, and controlled local deployments revealed several scenarios where Mysticeti performance may degrade due to network behavior or validator misbehavior, such as:
...
This ensures deterministic and predictable commit performance even under network failures or adversarial behavior.
...
These design improvements significantly strengthen Starfish’s robustness in Byzantine and unstable environments. As a result, Starfish supports **substantially higher throughput** and scales efficiently in large deployments.
