...
1. **Implementation**: IOTA developers introduce a new protocol version in the software by extending the `ProtocolConfig` struct with a feature flag. This flag enables the new protocol version but is set to false by default when iota-node starts.
