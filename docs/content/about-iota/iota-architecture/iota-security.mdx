...
IOTA uses delegated Proof-of-Stake to select validators (computers that help run the network) for each period. Users lock and delegate their IOTA tokens to vote for who should be validators. Anyone with enough delegated tokens can become a validator.
