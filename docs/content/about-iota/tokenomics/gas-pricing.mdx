...
To achieve these goals, the protocol prices the two key resources consumed by transactions: computation and storage, while embedding incentives that promote efficient validator behavior.

## Transaction Fees

Every IOTA transaction pays fees for two resources:

- **Computation**, required to execute the transaction.
- **Storage**, required to persist data on-chain.
...
Instead, during periods of congestion, users can request an estimated minimum gas price for successful execution through a transaction dry run.
This estimation is computed using a predictive gas pricing algorithm, which helps users determine an appropriate tip.
