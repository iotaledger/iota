...
<div align="center">
`storage_fee = storage_units + storage_fee`
</div>

where the `storage_fee` is fixed and set in the protocol config. The storage component is 100% rebated upon deletion of the corresponding object from storage, and for this reason, the terms `storage fee` and `storage deposit` are used interchangeably.
