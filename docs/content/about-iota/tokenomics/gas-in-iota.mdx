...
An IOTA transaction must both pay for the computational cost of execution and pay a deposit for storing the objects a transaction creates or mutates. Specifically, [IOTA Gas Pricing](gas-pricing.mdx) is such that any user transaction pays the following gas fees:

<div align="center">
  `total_gas_fees = computation_units * reference_gas_price + storage_units * storage_price`.
</div>

The storage component is 100% rebated upon deletion of the corresponding object(s) from storage, and for this reason, the terms _storage fee_ and _storage deposit_ are used interchangeably.
...
Buckets start at 1,000 units and increment in steps up to 5,000,000, where the step value is set as a protocol parameter (currently, the step is equal to 1,000, effectively rounding up to the nearest 1,000).
