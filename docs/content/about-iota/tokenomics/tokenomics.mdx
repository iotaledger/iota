...
- **[Gas fees](gas-in-iota.mdx):** Gas fees are charged on all network operations, consisting of computation fees which are burnt, a fully rebated storage deposits, and an optional tip which is assigned to users participating in the delegated proof-of-stake mechanism, namely delegators and validators.
- **[Staking rewards](proof-of-stake.mdx):** Staking rewards are provided to delegators and validators. This incentivizes honest behavior by IOTA validators and the IOTA token holder that delegate to them.
