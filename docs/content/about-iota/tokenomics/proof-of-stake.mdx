...
At the beginning of each epoch, the delegated tokens for a given validator are added up and a new [committee](../iota-architecture/validator-committee.mdx) is formed. 
...
- The total amount of stake rewards is calculated. The stake rewards are composed of a validator subsidy, which is currently set as 767000 IOTAs per epoch, plus any tips from user transactions. 
