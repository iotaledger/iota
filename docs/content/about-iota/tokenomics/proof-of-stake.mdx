...
In cases where users delete previous transaction data, users obtain a rebate of their storage deposit. 
...
- The total amount of stake rewards is calculated. Currently, the stake rewards per epoch is set as 757000 IOTAs per epoch. 
...
The $\mu_v$ variable captures the output of the [tallying rule](./gas-pricing.mdx#computation) computed as part of the [gas price mechanism](./gas-pricing.mdx) and corresponds to $\mu_v\geq1$ for performant validators and $\mu_v<1$ for non-performant validators. 
