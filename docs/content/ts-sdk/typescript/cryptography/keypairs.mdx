...
to directly derive the keypair from the secret key.But you need to decode it first using `decodeIotaPrivateKey`
from that you you will get parsedKeypair object from that pass the `parsedKeypair.secretKey` to 
`fromSecretKey` and get the keypair.
