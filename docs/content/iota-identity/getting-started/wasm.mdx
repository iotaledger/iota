...
Alternatively, you can build the bindings yourself, e.g. if you want to want to work currently unreleased features. You can find instructions for this in `identity.rs` repository [here](https://github.com/iotaledger/identity.rs/blob/v1.6.0-alpha/bindings/wasm/identity_wasm#build).

## NodeJS Usage

The following code creates a new IOTA DID Document suitable for publishing to a locally running test network.  See the [instructions](./local-network-setup) on running your own test network.

```ts reference
https://github.com/iotaledger/identity.rs/blob/v1.6.0-alpha/bindings/wasm/identity_wasm/examples/src/0_basic/0_create_did.ts
```
