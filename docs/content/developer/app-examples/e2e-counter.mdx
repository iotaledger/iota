...
Next, use the [IOTA Client CLI](../../references/cli/client.mdx) to generate a new Move package. If you have [IOTA installed](../getting-started/iota-install.mdx), the IOTA CLI is on your system. Run the following command in your terminal or console:

```bash
iota move new counter
```
...
Now that you have your Move code, you need to publish it. The Client App with IOTA TypeScript SDK example and the app template use `testnet` by default, so configure your code to match the network you want to deploy to.

First, update the IOTA dependency in Move.toml by changing the `rev` from `framework/testnet` to `framework/devnet`.
...
Next, configure the IOTA CLI to use `devnet` as the active environment, as well. If you haven't already set up a `devnet` environment you can do so by running the following command in a terminal or console:
...
```bash
iota client publish --gas-budget 10000000 counter
```

:::info

See [IOTA Client CLI](../../references/cli/client.mdx) for more information about `client` commands in the IOTA CLI.
...
First, import `TransactionBlock` from `@iota/iota.js`, `COUNTER_PACKAGE_ID` from your constants.ts file created previously, and `useSignAndExecuteTransactionBlock` from `@iota/dapp-kit`.
...
Now you can use the `iotaClient` in your `create` function to wait until the `TransactionBlock` is indexed:
...
import { isValidIOTAObjectId } from '@iota/iota.js/utils';
...
export function Counter({ id }: { id: string }) {
	...
	function executeMoveCall(method: 'increment' | 'reset') {
		...
		signAndExecute(
			...
			{
				onSuccess: (tx) => {
					iotaClient.waitForTransactionBlock({ digest: tx.digest }).then(() => {
						refetch();
					});
				},
			},
...
Your counter app is now ready to count. To learn more about dApp Kit, check out the [dApp Kit docs](https://sdk.iota.com/dapp-kit).
