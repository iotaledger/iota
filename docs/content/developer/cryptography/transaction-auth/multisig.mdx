...
```shell
iota client transfer --to <IOTA-ADDRESS> --object-id <OBJECT-ID> --serialize-unsigned-transaction --custom-signer <MULTISIG-ADDRESS>
...
Use the --custom-signer option to set the address of the (multisig) account that will be signing the transaction.
