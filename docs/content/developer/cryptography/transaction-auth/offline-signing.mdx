...
```shell
iota client transfer --to <IOTA-ADDRESS> --object-id <COIN-OBJECT-ID> --serialize-unsigned-transaction
```
...
```shell
iota client transfer --to <IOTA-ADDRESS> --object-id <COIN-OBJECT-ID> --serialize-signed-transaction
```
