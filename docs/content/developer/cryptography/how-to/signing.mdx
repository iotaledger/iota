...
Move contracts in IOTA support verifications for several signature schemes on-chain. Not all signatures supported in on-chain verification are supported as user signature verification. See [IOTA Signatures](../explanations/transaction-auth/signatures.mdx) for valid signature schemes for transaction authorization.

This topic covers:
1. How to use [fastcrypto](https://github.com/iotaledger/fastcrypto)'s CLI tool to create a signature of a given scheme. For testing and debugging only, DO NOT use in production.
...
```shell
git@github.com:iotaledger/fastcrypto.git
cd fastcrypto/
