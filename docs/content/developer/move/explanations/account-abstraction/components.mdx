...
`MoveAuthenticator` is a new variant of IOTA's `UserSignature` type within the SDK. It is what a transaction sender provides instead of a traditional cryptographic signature when sending a transaction from an abstract account.
