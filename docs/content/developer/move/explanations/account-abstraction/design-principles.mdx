...
- A **standard protocol-created account** (a regular keypair, multisig, passkey, …). Sponsorship is authorized by a standard `UserSignature` over the transaction; the protocol's signature check is the only validation the sponsor goes through.
