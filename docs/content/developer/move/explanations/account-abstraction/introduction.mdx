...
sequenceDiagram
    ...
    Dev->>Protocol: Publish package with #[authenticator] function
    Protocol->>Protocol: Create PackageMetadataV1 (immutable)
    Protocol-->>Dev: Return package_id & metadata_id

    Note over Dev,Account: Account Creation Phase
    Dev->>Protocol: Call create_auth_function_ref_v1(metadata, module, function)
    Protocol->>Protocol: Validate authenticator function signature
    Protocol-->>Dev: Return AuthenticatorFunctionRefV1<Account>
    Dev->>Account: Call create_account_v1(account, auth_ref)
    Account->>Account: Attach AuthenticatorFunctionRefV1 as dynamic field
    Account->>Protocol: Share account object
    Protocol-->>Dev: Account.id becomes sender address

    Note over User,MoveVM: Transaction Submission Phase
    User->>User: Construct TransactionData with account address as sender
    User->>User: Create MoveAuthenticator{object_to_authenticate, call_args}
    ...
    alt Authentication Fails
        ...
        MoveVM-->>User: Transaction successful
    end
...
flowchart TD
    ...
    MetadataCreated --> CreateAuthRef[Call create_auth_function_ref_v1<br/>metadata, module, function]
    
    CreateAuthRef --> ValidateAuth{Validate<br/>authenticator<br/>signature?}
    ...
    UserSubmit --> CreateMoveAuth[Create MoveAuthenticator:<br/>- object_to_authenticate<br/>- call_args<br/>- type_arguments]
    ...
    Execute --> Success[Transaction successful]
