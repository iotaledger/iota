...
- For optimal performance and data consistency, apps should submit writes and reads for the same full node. In the TS SDK, this means that apps should use the wallet's [`signTransaction`](../references/ts-sdk/dapp-kit/index.mdx) API, then submit the transaction via a call to [`execute_transaction`](/iota-api-ref#iota_executetransaction) on the app's full node, _not_ use the wallet's `signAndExecuteTransaction` API. This ensures read-after-write-consistency--reads from the app's full node will reflect writes from the transaction right away instead of waiting for a checkpoint.
- For lower latency, use [`executeTransaction`](/iota-api-ref#iota_executetransaction) with `"showEffects": false` and `"showEvents": false` if your app needs to know that a transaction was confirmed, but does not immediately need to see the transaction effects or read the objects/events written by the transaction.
