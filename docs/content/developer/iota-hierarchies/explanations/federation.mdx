...
In Hierarchies, Federation provides the following governance functions:
...
The Federation is implemented as a shared object that can be deployed on the IOTA network as part the Hierarchies Move module. The Federation object can be accessed in two ways:

- Through the Rust/WASM bindings API: This library provides an off-chain management perspective for the Hierarchies, utilizing an API that is natively exposed by the IOTA network for the Hierarchies module.
...
The main responsibilities of Root Authority are:
