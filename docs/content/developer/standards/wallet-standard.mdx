...
The `@mysten/iota/transaction` SDK exports the `SerialTransactionExecutor` class, which you can use
...
Construct the transaction separately with the `@mysten/iota` library and then sign it with the private key of the user. Use the `iota:signTransaction` feature to achieve this:
...
To execute the transaction, use `IOTAClient` from `@mysten/iota`:
