...
The `new_policy` function also returns a policy capability that grants admin rights, similar to the treasury capability (more about that later)
...
```move file=<rootDir>/docs/examples/move/clt-tutorial/sources/clt_tutorial.move#L41-L44
```
...
```move file=<rootDir>/docs/examples/move/clt-tutorial/sources/clt_tutorial.move#L65-L70
```

And then mint the token and send it to the receiver:

```move file=<rootDir>/docs/examples/move/clt-tutorial/sources/clt_tutorial.move#L71-L72
```

As you can see, the `token::transfer` function returns an `ActionRequest`. This object can not be dropped so without passing it to another function that destroys it the contract wouldn't work.
...
A [rule](rules.mdx#rule-structure) is represented as a witness - a type with a `drop` ability. So let's define that first:
...
Then we can get the mutable config with our `config_mut` helper function, which is just a wrapper of `token::rule_config_mut.`
...
So, in this case, we have to call the `token_policy::confirm_request` function in a PTB afterward, which will check the request for the approval stamp and make our TX succeed.
