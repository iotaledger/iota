...
The `new_policy` function also returns a policy capability that grants admin rights, similar to the [treasury capability](../../../references/framework/iota/coin.mdx#resource-treasurycap).
...
As you can see, the [`token::transfer`](../../../references/framework/iota/token.mdx#function-transfer) function returns an [`ActionRequest`](../../../references/framework/iota/token.mdx#struct-actionrequest). This object can not be dropped, so without passing it to another function that destroys it, the contract wouldn't work.
...
First, we check if the token policy already has a rule configuration. If not, we create a new one with [`token::add_rule_config`](../../../references/framework/iota/token.mdx#function-add_rule_config)
passing the rule witness, the token policy and its capability, and a new [bag](../../../references/framework/iota/bag.mdx) in which we want to store the addresses.

Then we can get the mutable config with our `config_mut` helper function, which is just a wrapper of [`token::rule_config_mut.`](../../../references/framework/iota/token.mdx#function-rule_config_mut).
...
So, in this case, we have to call the [`token_policy::confirm_request`](../../../references/framework/iota/token.mdx#function-confirm_request) function in a [PTB](../../iota-101/transactions/ptb/programmable-transaction-blocks-overview.mdx) afterward, which will check the request for the approval stamp and make our TX succeed.
