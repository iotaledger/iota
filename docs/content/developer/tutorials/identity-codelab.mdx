...
4. On running the Holder (First Time): This program will detect that no DID exists, create one, and publish it to the IOTA network.
...
3. On running the Issuer: The program will create the Issuer's DID, load the Holder's DID, and issue the verifiable credential. The Issuer will save the signed credential to a new file, `vc.jwt`.
