...
The example illustrates the creation of a review rating platform for the food service industry, utilizing [multisignature](../cryptography/transaction-auth/multisig.mdx) functionality on IOTA. Unlike conventional review platforms, which frequently conceal the algorithms used to rate reviews, this model leverages a transparent, on-chain algorithm that anyone can access and verify. IOTA's low [transaction costs](../../about-iota/tokenomics/gas-pricing.mdx) make it economically viable to submit, evaluate, and rank all reviews directly on-chain.
