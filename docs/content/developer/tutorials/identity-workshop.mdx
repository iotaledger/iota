...
- **Why it matters**: This function ensures the Students’s DID is either created and anchored to the IOTA network or loaded for reuse, providing persistence across server restarts. It abstracts the complexity of DID creation and storage, making it reusable across different endpoints (e.g., `holder_create_did`).
- **Key concepts**:
    - **DID Document**: A JSON object containing the student’s decentralized identifier (e.g., `did:iota`:...) and associated data like public keys.
...
- **Why it matters**: This function is the core of DID creation, anchoring the Student’s identity to the IOTA network. The published DID document and its verification method enable secure signing of credentials and presentations, forming the foundation of the workshop’s identity flow.
- **Key concepts**:
...
Together, they ensure the Student’s DID is created securely, funded, and anchored to the IOTA network, or loaded from disk if it already exists.
...
- **What it does**: The `handleIssueVc` function sends a POST request to the `/issuer/issue-vc` endpoint with an empty body. The backend generates a Verifiable Credential and returns its JWT, which is stored in the React state (`setVcJwt`) for display and use in the next step.
...
- **Key concepts**:
    ...
    - **JSON Web Token**: The Verifiable Credential is encoded as a `JSON Web Token` for secure transmission.
    - **React State**: Stores the `VC JWT` for use in VP creation.

**Backend** (`src/main.rs`): In this step, the Uinversity (issuer) creates or loads its Decentralized Identifier (DID) and issues a Verifiable Credential (VC) representing a university degree for the Student (holder, Alice). This VC is signed and returned as a JSON Web Token (JWT) to the React frontend for use in subsequent steps.
...
- **What it does**: The `issuer_issue_vc` function handles the `/issuer/issue-vc` HTTP endpoint, triggered by the React frontend. It performs four key steps:
    1. **Create/Load Issuer DID**: Calls `create_or_load_did` to either create a new DID for the University (issuer) or load an existing one from disk.
    2. **Load Student DID**: Reads the Student’s DID document from `holder_doc.json` (created in Step 1).
    3. **Build Verifiable Credential**: Constructs a Verifiable Credential with the Student’s DID, student details (e.g., name, degree, GPA), and an university identifier.
    4. **Sign Verifiable Credential**: Uses the University's private key (stored in Stronghold) to sign the Verifiable Credential as a JSON Web Token, which is returned to the frontend.
- **Why it matters**: This function creates a tamper-proof degree credential linked to the Student’s DID, enabling secure verification by employers in later steps. It connects the frontend’s “Issue Verifiable Credential” button to the IOTA Identity framework, ensuring the credential is cryptographically signed and verifiable on the IOTA network.
- **Key concepts**:
    - **Verifiable Credential (VC)**: A digitally signed document asserting claims (e.g., Alice’s degree) linked to the Student’s DID.
    ...
    - **JWT**: A JSON Web Token encoding the VC, signed with the Universities’s private key for authenticity.
...
- **What it does**: Calls the `create_or_load_did` helper function to either create a new DID for the University (saved to `issuer_doc.json` and `issuer_fragment.txt`) or load an existing one. The DID is anchored to the IOTA network if newly created.
- **Why it matters**: The University's DID establishes the university’s identity, used to sign the Verifiable Credential and prove its authenticity.
- **Key concepts**:
    - **DID Document**: Contains the University's identifier (e.g., `did:iota:...`) and verification methods.
...
- **What it does**: Reads the Student’s DID document from `holder_doc.json` (created in Step 1) and deserializes it into an `IotaDocument`.
- **Why it matters**: The Student’s DID is needed to link the Verifiable Credential to the student (Alice), ensuring the credential is tied to her identity.
- **Key concepts**:
...
- **What it does**: Creates a `Subject` object with the Student's DID, name, degree details, and GPA. Uses `CredentialBuilder` to construct a Verifiable Credential with a unique ID, the University's DID, a type (`UniversityDegreeCredential`), and the subject data.
...
- **Key concepts**:
    - **Subject**: The entity (Alice) the Verifiable Credential is about, linked to her DID.
...
- **What it does**: Initializes Stronghold storage for the University's private keys, then uses the `issuer_doc` to sign the Verifiable Credential with the specified `issuer_fragment` (signing key). The signed VC is encoded as a JWT.
- **Why it matters**: Signing the VC ensures its authenticity and integrity, allowing employers (e.g., verifier) to trust that it was issued by the university.
- **Key concepts**:
...
- `create_or_load_did`: To obtain the University's DID and signing fragment.
- `get_stronghold_storage`: To access the University's private keys for signing.
- The Student's DID document (`holder_doc.json`) from Step 1.
...
- **Key concepts**:
    - **VP**: A signed package containing the Verifiable Credential, proving the Student's ownership.
    - **Payload**: Sends the Verifiable Credential JWT to the backend for inclusion in the VP.
...
- What it does: The `holder_create_vp` function handles the `/holder/create-vp` HTTP endpoint, triggered by the React frontend. It performs three key steps:
    1. **Load Holder DID**: Retrieves the Student's DID document and signing fragment from disk (created in Step 1).
    2. **Build VP**: Creates a Verifiable Presentation that embeds the received VC JWT (`body.vc_jwt`), sets the Student's DID as the presenter, and configures expiration and challenge parameters.
    3. **Sign VP**: Uses the Student's private key (stored in Stronghold) to sign the VP, creating a JWT that proves Alice controls the VC.
- **Why it matters**: This function enables the student to create a signed presentation of their degree credential, which they can securely share with employers. The VP proves both the authenticity of the VC (from Step 2) and Alice's ownership of it, completing the Student's role in the identity flow.
- **Key concepts**:
...
```
    ...
    - **What it does**: Calls the `create_or_load_did` helper function to load the Student's DID document and signing fragment from `holder_doc.json` and `holder_fragment.txt` (created in Step 1). Since the DID already exists, it reads from disk rather than creating a new one.
    - **Why it matters**: The Student's DID is needed to identify Alice as the presenter of the VP and to access her private key for signing. This ensures the VP is linked to her identity.
    - **Key concepts**:
...
```
    ...
    - **What it does**: Creates a `PresentationBuilder` with the Student's DID as the presenter, embeds the received VC JWT (`body.vc_jwt`) from the frontend, and builds the presentation structure. The `Jwt::new(body.vc_jwt)` wraps the VC JWT as a credential within the presentation.
    - **Why it matters**: This constructs the VP structure that contains Alice's degree credential, linking it to her identity. The presentation format allows employers to validate both the VC's authenticity and Alice's ownership.
    - **Key concepts**:
        - **PresentationBuilder**: Builds the VP structure with the presenter's DID and embedded credentials.
        - **Credential Embedding**: The VC JWT (from Step 2) is included in the VP for presentation to employers.
        - **URL Conversion**: The Student's DID is converted to a URL format for use in the presentation.
...
- **What it does**: Uses the Student's DID document to sign the presentation with her private key (accessed via `holder_storage` and identified by `holder_fragment`). The signature includes the challenge as a nonce and sets the expiration time. The signed presentation is encoded as a JWT.
...
- **Dependencies**: `create_or_load_did` to load the Student's DID and `get_stronghold_storage` to access private keys.
- **Prerequisites**: The Student's DID must exist from Step 1 (`holder_doc.json`).
...
- What it does: The verifier_validate function handles the /verifier/validate HTTP endpoint, triggered by the React frontend. It validates the VP JWT received from the frontend (body.vp_jwt) by performing four key steps:
    ...
    2. Resolve Holder DID: Extracts and resolves the Student's DID from the VP to retrieve their public key.
    ...
    4. Validate Embedded Credentials: Extracts and verifies the signatures of all embedded VCs, ensuring they were issued by trusted universities.
...
- Key concepts:
    ...
    - Verifiable Presentation Validation: Verifies the VP’s signature, nonce, and Student's identity.
    - Verifiable Credential Validation: Confirms the embedded VC’s signature and university, ensuring it’s untampered and trustworthy.
...
```
    ...
    - What it does: Extracts the Student's DID from the VP JWT and uses the resolver to fetch the corresponding DID document from the IOTA network. Logs the extracted and resolved DID to the output string.
    - Why it matters: The Student's DID document contains Alice’s public key, needed to verify the VP’s signature. Resolving it from the network ensures the DID is valid and untampered.
    - Key concepts:
...
- What it does: Configures VP validation options with the expected nonce, then uses JwtPresentationValidator with an Ed25519 verifier to check the VP’s signature and nonce against the Student's public key (from holder_doc). Logs the successful validation and confirms the Student's DID matches the VP’s presenter.
...
- Key concepts:
    - Signature Verification: Uses the Student's public key to verify the VP’s Ed25519 signature.
    - Nonce Validation: Ensures the VP’s nonce matches challenge-123.
    - DecodedJwtPresentation: The validated VP structure, containing the embedded VCs and Student's DID.
...
- What it does: Extracts the VCs from the VP, retrieves the University's DID for each VC, and resolves their DID documents. Validates each VC’s signature against the University's public key, ensuring the VC was issued by the university and the subject matches the Student's DID. Logs each credential’s validation result.
...
- Key concepts:
    - Credential Validation: Verifies the VC’s signature using the University's public key.
    - University Resolution: Fetches the University's DID document to access their public key.
    - Subject-Holder Relationship: Ensures the VC’s subject (Alice) matches the Student's DID.
...
The verifier_validate function is triggered by the React frontend’s “Execute Step 4” button, which sends the VP JWT (from Step 3) to the /verifier/validate endpoint. It relies on:
    ...
    - Prerequisites: The Student's DID (Step 1) and VC (Step 2) must exist, and the VP (Step 3) must be valid.
...
- Click “Execute Step 1” to create the Student's DID.
