...
description: How IOTA Identity implements the Decentralized Identifiers Standard on IOTA.
...
The `did:iota` method is implemented in the [IOTA Identity library](https://github.com/iotaledger/identity.rs). This library supports a number of operations that are standardized, some are standardized across the SSI community, and some are the invention of the IOTA Foundation.
...
The IOTA Identity library also standardized certain `services` that are embedded in the DID Document. It is RECOMMENDED to implement these when implementing the `did:iota` method.
