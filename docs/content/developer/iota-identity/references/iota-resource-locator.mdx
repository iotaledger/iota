...
and a relative URL (a regular URL but with only path, query, and fragment components set), where the 
...
Otherwise, if the path is not empty, it should be converted to a
[JSON pointer](https://datatracker.ietf.org/doc/html/rfc6901) and derefering it 
within the context of the `fields` object, to obtain the resolution result.
