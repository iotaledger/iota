...
IOTA Identity enables holders to easily construct
...
Of the above, **only the list of credentials is required** when creating a presentation using the library.
...
IOTA Identity provides several options for verifiers to validate various sections of a verifiable presentation.
...
presentation. The library only verifies that the issuer's signature on each credential is current and valid
...
By default, the library always enforces that the holder matches the subject.
...
The following code demonstrates how to use IOTA Identity end-to-end to create and sign a verifiable
...
```rust reference
https://github.com/iotaledger/identity.rs/blob/v1.6.0-beta/examples/0_basic/6_create_vp.rs
```
...
```ts reference
https://github.com/iotaledger/identity.rs/blob/v1.6.0-beta/bindings/wasm/identity_wasm/examples/src/0_basic/6_create_vp.ts
```
