...
description: An explanation of how roles, capabilities, and record tags govern access to Audit Trail operations.
tags:
 - explanation
 - audit-trail
---
...
A **role** is a named set of permissions stored on the trail. Roles define *what* a capability holder can do. For example:
...
The Audit Trail defines 18 distinct permissions, grouped by domain:
...
1. **Maintain an off-chain registry** of every issued capability, storing at least the capability ID, role, `issued_to` address, and validity timestamps.
