...
The IOTA Audit Trail framework defines clear roles for the participants in the audit process:
...
- Define roles with specific permission sets (e.g., `RecordAdmin`, `Auditor`, `LockingAdmin`)
...
## Role-Based Access Control in Audit Trails

Access to every write operation on an Audit Trail is governed by **Role-Based Access Control (RBAC)**. The RBAC system is highly configurable and combines two primitives:
