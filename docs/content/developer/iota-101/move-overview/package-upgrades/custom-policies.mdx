...
However, you can adjust the policy during the transaction block that publishes the package,
...
Package upgrades must be executed in a single transaction block, involving three key steps:
...
The `UpgradeTicket` must be used within the same transaction block it was created; otherwise, the transaction fails.
...
The `UpgradeReceipt` must be used within the same transaction block to update the `UpgradeCap`
...
import { getFullnodeUrl, IotaClient } from '@iota/iota-sdk/client';
import { Ed25519Keypair } from '@iota/iota-sdk/keypairs/ed25519';
import { TransactionBlock, UpgradePolicy } from '@iota/iota-sdk/transactions';
import { fromB64 } from '@iota/iota-sdk/utils';
