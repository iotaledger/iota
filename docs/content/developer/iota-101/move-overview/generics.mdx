...
An example of a generic type is the [vector](../../../references/framework/Testnet/move-stdlib/vector.mdx) type, which is a container type that
can hold any other type. Another example of a generic type in the standard library is the
[Option](../../../references/framework/Testnet/move-stdlib/option.mdx) type, which is used to represent a value that may or may not be present.
