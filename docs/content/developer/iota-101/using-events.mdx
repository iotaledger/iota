...
To emit events from your Move modules, you need to use the [`iota::event`](../../references/framework/iota/event.mdx) module.
...
Then, within your function, you can emit an event using the [`emit`](../../references/framework/iota/event.mdx#function-emit) function. For example:
