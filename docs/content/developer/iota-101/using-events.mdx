...
To emit events from your Move modules, you need to use the [`iota::event`](../../references/framework/testnet/iota-framework/event.mdx) module.
...
Then, within your function, you can emit an event using the [`emit`](../../references/framework/testnet/iota-framework/event.mdx#function-emit) function. For example:
...
[`client.queryEvents`](../../ts-sdk/api/testnet/client/classes/IotaClient#queryevents).
...
The [TypeScript SDK](../../ts-sdk/api/testnet/graphql/classes/IotaGraphQLClient) provides functionality to
