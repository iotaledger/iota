...
use iota_sdk_types::{ObjectId, ObjectArg::Receiving};

// setup Rust SDK client as normal
let sending_account: Address = /* your address */;
let example_package_id: ObjectId = /* package id */;

let mut builder = TransactionBuilder::new(sending_account)
    .with_client(&client);

builder
    .move_call(example_package_id, "account", "accept_payment")
    .type_tags(["0x2::iota::IOTA".parse()?])
    .arguments([
        builder.input(ObjectArg::SharedMut("0xcafe".parse()?)),
        builder.input(ObjectArg::Receiving("0xc0ffee".parse()?)), // 0xcoffee is turned into the `Receiving<...>` argument
    ]);
