...
To explain further, look at the core of the receiving interface in Move, which is defined in the `iota::transfer` module in the IOTA framework:
...
Just like with [custom transfer policies](custom-rules.mdx), IOTA allows for the definition of custom receivership rules for `key`-only objects. In particular, you can use the `iota::transfer::receive` function only on objects defined in the same module as the call to `iota::transfer::receive`--just like you can use the `iota::transfer::transfer` function only on objects defined in the module where it's being used.

Similarly for objects that also have the `store` ability, anyone can use the `iota::transfer::public_receive` function to receive them--just like `iota::transfer::public_transfer` can transfer any objects that have the `store` ability on them.
...
Because the `receive_object` function is generic over the object being received, it can only receive objects that are both `key` and `store`. `receive_object` must also use the `iota::transfer::public_receive` function to receive the object and not `iota::transfer::receive` because you can only use `receive` on objects defined in the current module.
