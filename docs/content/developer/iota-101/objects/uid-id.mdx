...
turn, wraps the [`address` type](../../../references/framework/testnet/move-stdlib/address.mdx). The UIDs on IOTA are guaranteed to be unique, and can't be reused
...
You can create a new UID with the [`object::new(ctx)` function](../../../references/framework/testnet/iota-framework/object.mdx#function-new). It takes a mutable reference to TxContext,
...
The `UID` type is created with the [`object::new(ctx)` function](../../../references/framework/testnet/iota-framework/object.mdx#function-new), and it is destroyed with the
[`object::delete(uid)` function](../../../references/framework/testnet/iota-framework/object.mdx#function-delete). The `object::delete` consumes the UID _by value_, and it is
...
When talking about `UID` we should also mention the `ID` type. It is a wrapper around the [`address`](../../../references/framework/testnet/move-stdlib/address.mdx)
