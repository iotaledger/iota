...
- **Near Real-Time Measurement**: Use the immutable reference of time provided by the [`Clock`](../../references/framework/testnet/iota-framework/clock.mdx) module in Move. This value updates with every network checkpoint.
- **Epoch Start Time**: Use the [`epoch_timestamp_ms`](../../references/framework/testnet/iota-framework/tx_context.mdx#function-epoch_timestamp_ms) function to capture the precise moment the current epoch started.

## Using the `iota::clock::Clock` Module

To access a prompt timestamp, you can pass a read-only reference of [`iota::clock::Clock`](../../references/framework/testnet/iota-framework/clock.mdx) as an entry function parameter in your transactions.
An instance of `Clock` is provided at the address `0x6`, and no new instances can be created.

Use the [`timestamp_ms`](../../references/framework/testnet/iota-framework/clock.mdx#function-timestamp_ms) function from the `iota::clock` module to extract a Unix timestamp in milliseconds.
...
you can use the [`epoch_timestamp_ms`](../../references/framework/testnet/iota-framework/tx_context.mdx#function-epoch_timestamp_ms) function 
from the [`iota::tx_context`](../../references/framework/testnet/iota-framework/tx_context.mdx) module to access the timestamp for the start of the
