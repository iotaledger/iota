...
In that case, `init` would have to take two extra parameters: the [`Coin`'s `CoinMetadata`](../../../references/framework/iota/coin.mdx#resource-coinmetadata) object and the [`TreasuryCap`](../../../references/framework/iota/coin.mdx#resource-treasurycap) object. IOTA's Move only allows `TxContext` and an optional Witness. There is no means to pass in the extra argument.
