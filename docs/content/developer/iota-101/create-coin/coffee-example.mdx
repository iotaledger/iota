...
1. [**Importing modules**](https://moved.network/docs/move-basics/importing-modules): We imported essential Move packages for transactions, tokens, and balances.
...
4. [**Initialization with OTW**](../move-overview/one-time-witness.mdx): Set up the shop with its treasury cap for minting tokens.
...
- Basic understanding of blockchain concepts like [tokens](https://docs.openzeppelin.com/contracts/2.x/tokens#:~:text=A%20token%20is%20a%20representation,them%2C%20create%20or%20destroy%20them.) and [transactions](https://www.scalingparrots.com/en/what-is-a-blockchain-transaction/).
- Basic understanding of the [Move](https://move-language.github.io/move/)
- [Install Iota CLI](../../getting-started/iota-install.mdx)

## 1. Create an IOTA Move Package

Use the following command to create a standard IOTA Move [package](../../getting-started/create-a-package.mdx) called `coffee_shop`:

``` bash
iota move new coffee_shop
```

The command will create and populate the `coffee_shop` directory with a skeleton for an IOTA Move project. To add a [module](../../getting-started/create-a-module.mdx), create a `.move` file in the sources directory, and call it `coffee.move`. In `coffee.move`, paste the following codes to the `coffee` module:
...
[Structs](https://move-language.github.io/move/structs-and-resources.html) in Move represent data storage objects, and we define two structs in our program.
...
In Move, the [init](../move-overview/init.mdx) function plays a critical role during the module's lifecycle, executing only once at the moment of module publication.
...
A [One-Time Witness (OTW)](../move-overview/one-time-witness.mdx) is a unique type in Move, specifically designed to ensure that certain actions within a module can only occur once, providing security for the system's setup.
...
The `buy_coffee` function enables customers to purchase coffee using IOTA tokens. In this process, we also apply control flow using assertions to ensure the customer has sufficient funds. If the required conditions are met, the transaction proceeds; otherwise, an error is triggered, making this a crucial aspect of program logic.
...
You can use the `iota move build` command to [build](../../getting-started/build-test.mdx) Move packages in the working directory, `coffee_shop` in this case.
