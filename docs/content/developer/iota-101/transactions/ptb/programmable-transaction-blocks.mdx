...
PTB Builder is a visual editor for constructing Programmable Transaction Blocks (PTBs) through a drag-and-drop interface. It allows developers and non-developers alike to intuitively create, simulate, and share PTB flows without writing code. The tool also supports code generation, real-time validation, and visualization of execution results.

:::note
PTB Builder is intended for educational and prototyping purposes only. It is not production-ready and should not be used to create or sign transactions in live environments. Use it to experiment and understand the PTB concept.
:::
