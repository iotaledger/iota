...
Using PTBs, you can perform up to 1,024 unique commands in a single execution.
...
While Programmable Transaction Blocks (PTBs) allow you to batch a large number of operations (up to 1,024 commands) into a single atomic transaction, they are subject to several hard limits enforced by the IOTA protocol. These limits help ensure network stability, prevent abuse, and maintain predictable performance.

Exceeding any of these limits will cause the transaction to fail during validation or execution.

| Limit                             | Value                   | Description
| --------------------------------- | ----------------------- |-------------------------------------------------------
| Maximum number of commands        | 1,024                   | Total number of commands (operations) that can be included in a single PTB. This is the "1,024 unique commands" limit mentioned earlier.
| Maximum transaction size          | 128 KiB (131,072 bytes) | The serialized size of the entire transaction (including all inputs, commands, and signatures). Large PTBs with many inputs or complex arguments can hit this quickly.
| Maximum input objects             | 2,048                   | Maximum number of object references (owned, shared, or receiving) that can be passed as inputs to the PTB.
| Maximum gas payment objects       | 256                     | Maximum number of `Coin<IOTA>` objects that can be used/merged for gas payment in a single transaction.
| Maximum arguments                 | 512                     | Total number of arguments (pure values + object references) across all commands in the PTB.
| Maximum new Move object IDs       | 2,048                   | Maximum number of new objects that can be created by the PTB (e.g., via `SplitCoins`, `MoveCall` that returns new objects, etc.).
| Maximum size of a created Move object| 250 KiB (256,000 bytes)| Size limit for any individual Move object created during PTB execution.


:::info
For the most up to date and complete list of [protocol configuration values](https://github.com/iotaledger/iota/blob/a96d831e3b8d0e07471f50a11ad851ae0618294d/crates/iota-protocol-config/src/lib.rs#L1214) (including any newly added limits), refer to the source in the IOTA repository:
:::
