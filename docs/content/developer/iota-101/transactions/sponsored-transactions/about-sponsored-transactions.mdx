...
There are two possible scenarios according to the Gas Station's design:

##### The gas station's responds with the signed transaction and the signature, and the user submits the transaction to the network:
