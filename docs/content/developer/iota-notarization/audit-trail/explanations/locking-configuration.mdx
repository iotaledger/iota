...
description: An explanation of the IOTA Audit Trail locking system — record deletion windows, write locks, and trail deletion locks.
tags:
 - explanation
 - notarization
 - audit-trail
...
The IOTA Audit Trail provides a configurable locking system that governs when records can be deleted, when the trail itself can be destroyed, and when new records can be added. Locking is essential for compliance scenarios where data retention policies must be enforced on-chain.
...
### Batch Deletion Bypass

The `delete_records_batch` operation — which requires the `DeleteAllRecords` permission (distinct from `DeleteRecord`) — **bypasses the record deletion window entirely**. It deletes records from the front of the trail regardless of their lock status. This is a deliberate design choice for administrative cleanup workflows. The separate permission ensures that only explicitly authorized administrators can perform this operation.
