...
Successful proof verification authenticates the inclusion of the transaction and its effects in the certified checkpoint. It does not require transaction execution to have succeeded: a failed transaction can also have a valid inclusion proof. Applications that require successful execution must also inspect the authenticated effects returned by `VerifiedProof`.

### Verification Failures

If verification fails, treat the proof as unverified and do not use its contents as authenticated data.

- **Rust:** Handle `Err(ProofVerificationError)`. `CommitteeResolution` means committee resolution failed; `Proof` means a proof check failed. Inspect the underlying error for the cause. The offline `ProofVerifier` returns `VerifyError` directly.
- **Wasm:** Catch the rejection from `await verifier.verify(proof)`.
- **CLI:** A failed `poi verify` command prints an error and exits with a nonzero status.

Retry transient endpoint failures. For trust-anchor or proof-check failures, confirm the network and independently trusted genesis or committee, then inspect the reported cause. An endpoint failure alone does not establish that the proof is invalid; never bypass verification to accept it.

### Execution Status

After proof verification succeeds, read the execution status through the verified result:

- **Rust:** Import `iota_types::effects::TransactionEffectsAPI` and call `verified.effects().status()`. Accept execution as successful only when the status is `ExecutionStatus::Success`. The trait comes from `iota-types`; use the same dependency version as `poi-rs`.
- **Wasm:** Import `bcs` from `@iota/iota-sdk/bcs` and decode `verified.effectsBcs()` with `bcs.TransactionEffects.parse(verified.effectsBcs())`. For the supported V1 effects format, execution succeeded only when `effects.V1?.status.$kind === "Success"`, where `effects` is the decoded result. A failed status or an unsupported effects version does not satisfy this check.

These checks use authenticated effects from the verified result, not fields read directly from an unverified proof. Successful execution still does not establish application-specific requirements; applications must check the relevant authenticated transaction, object, or event values.
...
| Local network | The `genesis.blob` generated by your own local network | The default `iota-localnet` setup writes `~/.iota/iota_config/genesis.blob`. If you change the network configuration directory, use the blob from that directory. Follow the [PoI example setup](https://github.com/iotaledger/notarization/tree/v0.1/bindings/wasm/poi_wasm/examples/README.md#step-2-configure-the-example) for network configuration. |
