...
A valid transaction proof establishes inclusion in a certified checkpoint; it does not establish that the transaction executed successfully.

Proof of Inclusion proves inclusion, not current state. An object proof created from only an object ID resolves the latest version available during proof construction, but the proof does not claim that the object remains at the latest version when another party verifies it.
...
Network selection only chooses where proof material comes from. It does not establish verification trust. The proof's `chain` field is informational and must not be used to select a network, genesis blob, committee, or other trust anchor.

Read [Trust and Verification](./explanations/trust-and-verification.mdx) before choosing a resolution mode.

## Supported Interfaces

Proof of Inclusion provides three interfaces:
...
- If you already have a transaction digest, follow [Create a Proof](./getting-started/cli.mdx#create-a-proof) and [Verify a Proof](./getting-started/cli.mdx#verify-a-proof) in the CLI guide. This workflow reads existing ledger data and requires no funded wallet or Move Package publication.
- To create a transaction and then prove it, use the [Transaction Proof](./how-tos/transaction-proof.mdx) example for the complete construction, transfer, and verification workflow.

## Examples

The examples focus on proof construction, transport, and verification. For business scenarios, use [Single Notarization](../single-notarization/index.mdx) or [Audit Trails](../audit-trails/index.mdx) to create ledger activity, then apply these proof workflows.

The standard examples cover the main proof workflows:

1. [Transaction Proof](./how-tos/transaction-proof.mdx)
2. [Multi-Target Proof](./how-tos/multi-target-proof.mdx)
3. [Reuse a Verifier](./how-tos/reuse-verifier.mdx)
4. [Object Proof](./how-tos/object-proof.mdx)
5. [Event Proof](./how-tos/event-proof.mdx)

The advanced examples cover optional trust and storage configurations:

1. [Build a Committee Cache](./how-tos/advanced/committee-cache.mdx)
2. [Trusted Node](./how-tos/advanced/trusted-node.mdx)
