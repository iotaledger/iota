...
The core of the Single Notarization component is the `Notarization` object, a unified Move `struct` that holds the notarized data and its associated metadata. The `Notarization` object is owned by an address allowing easier acces control compared to [IOTA Audit Trails](../../audit-trail/index.mdx) being a shared object which needs more complex access control features.
