...
Corrections do not mutate historical data. The correction operation appends a new record, sets the new record's `replaces` field, and updates the replaced record's `is_replaced_by` back-pointer. To continue a correction chain, correct the latest record rather than an already replaced record.
...
- **Correct records**: Authorized users append correction records that reference and supersede earlier entries.
