...
Possessing a capability does **not** automatically grant access. The audit trail validates all fields above on every call before the operation is executed.
...
A role may carry an optional **RoleTags** allowlist. When a capability holder adds a record with a tag, the audit trail checks that:
...
Because the audit trail uses a denylist (not an allowlist), it does **not** maintain an on-chain registry of all issued capabilities. This design shifts the bookkeeping responsibility to the integrator:
