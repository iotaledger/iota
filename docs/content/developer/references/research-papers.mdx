...
- **Relevance:** Starfish is the consensus protocol used by IOTA in production. It succeeds Mysticeti-C and improves liveness in the presence of network disruption and validator misbehavior.
...
- **Relevance:** Mysticeti-C is the direct predecessor of the Starfish consensus protocol used by IOTA in production. It pioneered the uncertified DAG design that Starfish extends.
...
- **Relevance:** IOTA's Starfish consensus protocol inherits HammerHead's score-based leader scheduler — initially introduced into the Mysticeti family — for lower latency and improved leader utilization.
