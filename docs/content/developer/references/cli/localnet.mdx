...
This config controls retention of the **indexer** database only; it does not affect node/object pruning. It is a TOML file with a required `epochs_to_keep` default and optional per-table `[overrides]`:
...
For the full list of prunable tables, default behavior, and pruning timing, see [Pruning](https://github.com/iotaledger/iota/blob/develop/crates/iota-indexer/README.md#pruning) in the `iota-indexer` crate. The format and behavior are identical to the `iota-indexer indexer --pruning-config-path` command.
