...
| `--print-config` | Print each node's final config with the overrides applied, then exit without starting the nodes. Values are verbatim except the key pairs and an embedded genesis; review the output before sharing it. Addresses and paths are the ones the run allocated, so they differ between runs. Generates and persists a genesis, key files included, if the config directory has none. Not combinable with `--with-faucet`. |
