...
`metrics-address`, `admin-interface-address`, `json-rpc-address`, the gRPC settings and the database paths are freely overridable. One case needs two overrides in one run: `fullnode:enable-grpc-api=true` alone is rejected, because a generated fullnode has no `grpc-api-config`. Add `fullnode:grpc-api-config={address: 0.0.0.0:50051}`. Enabling the API on a validator is rejected, because a validator never starts the gRPC server.
