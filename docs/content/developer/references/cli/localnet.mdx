...
Every run that has overrides logs the fields each node ended up with:

```
applied 2 overrides to fullnode: authority-store-pruning-config.num-epochs-to-retain, enable-grpc-api
```

The following are rejected:

- A validator's `network-address`, `p2p-address` and `primary-address`. These sit in the committee metadata inside `genesis.blob`, so overriding one at start would leave the validator unreachable at the address its peers dial. Re-run `iota-localnet genesis` to change them. On a fullnode, which is not a committee member, the same fields are ordinary config and stay overridable.
...
`metrics-address`, `admin-interface-address`, `json-rpc-address`, the gRPC settings and the database paths are freely overridable.
