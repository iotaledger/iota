...
| `--pruning-config-path <PATH>` | Path to a TOML retention config for the indexer writer (see [Pruning config file](#pruning-config-file) below). Without it the indexer keeps all data and its database grows without bound. |
...
The file passed to `--pruning-config-path` is a TOML document with a required `epochs_to_keep` default (in epochs, must be greater than 0) and an optional `[overrides]` table for per-table retention. Override keys are the prunable table names in `snake_case`:
