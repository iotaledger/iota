...
description: The IOTA Genesis Ceremony CLI (part of iota-tool) provides commands for building a Genesis blob file.
tags: [ cli, reference, validator, iota-tool]
teams:
  - iotaledger/dev-tools
  - iotaledger/vm-language
---

The `iota-tool genesis-ceremony` command allows the orchestration of an IOTA Genesis Ceremony,
...
Typing `iota-tool genesis-ceremony --help` into your terminal or console displays the following information on available commands.

```shell
Create an IOTA Genesis Ceremony with multiple remote validators

Usage: iota-tool genesis-ceremony [OPTIONS] <COMMAND>
...
```shell
iota-tool genesis-ceremony list-validators
```
...
```shell
iota-tool genesis-ceremony validate-state
```
...
```shell
iota-tool genesis-ceremony examine-genesis-checkpoint
```
...
```shell
iota-tool genesis-ceremony verify-and-sign --key-file authority.key
```
