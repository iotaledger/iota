...
Options:
     ...
     --upgrade-compile <MOVE_PACKAGE_PATH>                                Compile a Move package for upgrade without executing any transaction commands.
...
For more control over the upgrade process, you can use `--upgrade-compile` and `--execute-upgrade` to perform each step individually. This is especially useful when you want to change the upgrade policy or use custom authorize/commit functions.
...
`--upgrade-compile` compiles the package and returns the package digest as an assignable `vector<u8>` value. It also stores the compiled bytecode internally for use by `--execute-upgrade`. `--execute-upgrade` takes an upgrade ticket and executes the system upgrade command, returning the receipt.

**Standard step-by-step upgrade with the framework functions:**

```bash
iota client ptb \
--upgrade-compile "." @0x7ccc6c2fe6a509e6266b17e57a109f393f435a30da748c5168448958eb47d826 \
--assign digest \
...
```bash
iota client ptb \
--upgrade-compile "." @0xUPGRADE_CAP_ID \
--assign digest \
