...
The following binaries are separate from the `iota` CLI and must be installed independently:

- **[IOTA Localnet CLI](./cli/localnet.mdx):** Use `iota-localnet` to start and manage local IOTA networks for development and testing.
- **[IOTA Tool CLI](./cli/iota-tool.mdx):** Use `iota-tool` for advanced operations like transaction replay, genesis ceremonies, and fire drills.
