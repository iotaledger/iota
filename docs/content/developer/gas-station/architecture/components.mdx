...
description: Gas Station Components
teams:
  - iotaledger/identity
tags:
  - gas-station
...
flowchart LR
 subgraph subGraph0["Gas Station Pool"]
    ...
    KMS[("Key Managment Store")] -.-> GSS
    GSS --> REDIS[("Persistant Stroage (Redis)")]
...
**HTTP Endpoints:** The server provides two main endpoints to facilitate transaction management:

* **`/reserve_gas`** – Reserves gas objects that meet the specified amount required for a transaction.
* **`/execute_tx`** – Executes a transaction using the gas objects previously fetched from `/reserve_gas`.
...
The Gas Station assumes that the external KMS will communicate using a **JSON-RPC API**. The KMS must expose the following two endpoints:
