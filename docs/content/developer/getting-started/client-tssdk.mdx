...
The IOTA TypeScript SDK (@iota/iota.js) provides all the low-level functionality needed to interact with IOTA ecosystem from TypeScript. You can use it in any TypeScript or JavaScript project, including web apps, Node.js apps, or mobile apps written with tools like React Native that support TypeScript.

For more information on the IOTA TypeScript SDK, see the [IOTA TypeScript SDK documentation](../../references/ts-sdk/typescript/index.mdx).

## What is dApp Kit?

dApp Kit (@iota/dapp-kit) is a collection of React hooks, components, and utilities that make building dApps on IOTA straightforward. For more information on dApp Kit, see the [dApp Kit documentation](../../references/ts-sdk/dapp-kit/index.mdx).
...
Next, set up the `IOTAClientProvider`. This `Provider` delivers a `IOTAClient` instance from `@iota/iota.js` to all the hooks in dApp Kit. This provider manages which network dApp Kit connects to, and can accept configuration for multiple networks. This exercise connects to `devnet`.
