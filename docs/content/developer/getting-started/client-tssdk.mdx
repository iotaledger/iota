...
```bash npm2yarn
npm install @iota/iota-sdk @iota/dapp-kit @tanstack/react-query @radix-ui/themes
```
...
:::info Style Requirements
The dApp Kit components are built on top of Radix UI. Make sure to import both the dApp Kit and Radix UI styles for the components to work properly:
```ts
import '@iota/dapp-kit/dist/index.css';
import '@radix-ui/themes/styles.css';
```
This is especially important for Next.js applications where the wallet connect button may not respond without the Radix UI styles.
:::
