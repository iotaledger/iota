...
If you're working with a local network, replace <>{Networks.iota_move_devnet.faucetUrl}</>`'/gas'` with the appropriate value based on which package runs your network:
