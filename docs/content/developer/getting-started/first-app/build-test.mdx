...
As the code shows, the unit test function (`test_sword()`) creates a dummy instance of the `TxContext` struct and assigns it to `ctx`. The function then creates a sword object using `ctx` to create a unique identifier and assigns `42` to the `magic` parameter and `7` to `strength`. Finally, the test calls the `magic` and `strength` accessor functions to verify that they return correct values.
...
We now transfer ownership of the `sword` to a freshly created dummy address:
...
Update your `my_module.move` file to include entry functions callable from IOTA that implement `sword` creation and transfer. With these in place, you can then add a multi-transaction test that uses the `test_scenario` module to test these new capabilities. Put these functions after the accessors (Part 5 in comments).
...
The code of the new functions uses struct creation and IOTA-internal modules (`TxContext` and `Transfer`) in a way similar to what you have seen in the previous sections. The important part is for the entry functions to have correct signatures.
...
The tests you have so far call the `init` function, but the initializer function itself isn't tested to ensure it properly creates a `Forge` object.
...
public fun test_module_init() {
    ...
    // Second transaction to check if the forge has been created and has initial value of zero swords created.
    ts::next_tx(&mut ts, ADMIN);
    {
...
Add a `new_sword` function to take the forge as a parameter and to update the number of created swords at the end of the function. If this were an actual module, you'd replace the `create_sword` function with `new_sword`. To keep the existing tests from failing, however, the example uses both functions.
...
Once this function is added, we can use it in the `test_sword_transactions` test.
...
- [Package Upgrades](../../iota-101/iota-move-concepts/packages/packages.mdx): Upgrading packages published on the IOTA network. 
