...
In IOTA, users are not identified by their email or any other personal information, but rather by an account and its
associated [address](#whats-an-address). The account itself is generated from a private key, which in turn could be
...
or [package](../iota-101/iota-move-concepts/packages/packages.mdx). In the same way that your email or ID number is
unique, addresses have to be unique as they are the identifier your account uses to send and receive transactions. You
can [create any number of addresses](#create-an-address), all of which will be unique.
...
Perhaps the simplest way to generate an IOTA address is using the [IOTA Wallet Chrome browser extension](TODO UPDATE LINK).
...
Please write down the recovery phrase associated to your address and keep in it in a safe place.
...
You can list all the addressed associated to your local wallet by with
