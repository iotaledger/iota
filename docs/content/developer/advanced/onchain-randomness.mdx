...
`Random` has a reserved address `0x8`. See <a href="/developer/references/framework/iota/random" data-noBrokenLinkCheck='true'>random.move</a> for the Move APIs for accessing randomness on IOTA.
...
```typescript file=<rootDir>/docs/examples/typescript/src/advanced/onchain-randomness.ts
```
