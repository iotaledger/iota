...
The deployment of the contract is done through a [multi-sig]((../cryptography/transaction-auth/multisig.mdx)) address. Navigate to the [`publish.sh`](https://github.com/iotaledger/iota/blob/devx/tutorial-for-review-rating-with-multisig/examples/move/reviews_rating/publish.sh) script of the repository and follow the below steps to deploy the contract. 
