...
```
This command sets up a multisig address where `PUBLIC-KEY-1`, `PUBLIC-KEY-2` & `PUBLIC-KEY-3` are the public keys we retreived in step 2.
...
The deployment of the contract is done the admnin which is a [multi-sig]((../cryptography/transaction-auth/multisig.mdx)) address. Navigate to the [`publish.sh`](/examples/move/reviews_rating/publish.sh) script of the repository and follow the below steps to deploy the contract. 
...
Replace `iota-network-url-here` and `iota-network-faucet-link-here` with the IOTA network URL and faucet for tokens.

### 2.  Run the [`publish.sh`](/examples/move/reviews_rating/publish.sh) Script
