...
The deployment of the contract is done by the admin, which is a [multi-sig](/developer/cryptography/transaction-auth/multisig) address. Navigate to the [`publish.sh`](/examples/move/reviews_rating/publish.sh) script of the repository and follow the below steps to deploy the contract.  
