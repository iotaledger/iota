...
- **Create Transfer Policy**
    ...
    By searching the transaction digest on the [Iota network explorer](https://explorer.iota.cafe/), you can locate the created objects. Subsequently, select and save the `TransferPolicy ID` and the `ProtectedTP ID` from these objects into the respective fields within your .env file.
