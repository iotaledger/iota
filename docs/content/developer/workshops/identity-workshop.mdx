...
- Access to the **IOTA testnet** — test tokens can be requested from the [faucet](https://faucet.testnet.iota.cafe)
...
**Checkpoint:** Verify that `holder_doc.json` and `issuer_doc.json` are created on disk, and that both returned DIDs are valid `did:iota:...` strings.
...
This step uses `get_stronghold_storage`, which opens the Stronghold file for a given path and returns a storage object the signing functions can use:

```rust
fn get_stronghold_storage(path: Option<PathBuf>) -> anyhow::Result<Storage<StrongholdSecretManager, StrongholdSecretManager>> {
    let stronghold_path = path.unwrap_or_else(|| PathBuf::from("./default.stronghold"));
    let password = Password::from("secure-password".to_owned());
    let stronghold = StrongholdSecretManager::builder()
        .password(password)
        .build(stronghold_path)?;
    Ok(Storage::new(stronghold.clone(), stronghold))
}
```
...
**Checkpoint:** Verify that `issuer_doc.json`, `issuer_fragment.txt`, and `issuer.stronghold` are created. The returned VC JWT should be a long string starting with `eyJ...` — paste it into an online JWT decoder to inspect the credential contents.
...
This step uses `get_read_only_client`, which connects to the IOTA network without any keys or funds — suitable for resolving DIDs and verifying signatures, but not for publishing:

```rust
async fn get_read_only_client() -> anyhow::Result<IotaClient> {
    Ok(IotaClientBuilder::default()
        .build(std::env::var("IOTA_NODE_URL").unwrap_or_else(|_| "http://localhost:9000".to_string()))
        .await?)
}
```
