...
Minimal `Move.toml` so we speak 2024 Move and have named addresses:
...
:::info What are entry functions and transfers?
- `public entry` functions are the only functions callable as transaction entry points; think of them as your public API.
...
- Like an HTTP POST handler that writes a new row and assigns it to a user.
...
## Further notes
- Add command outputs for CLI commands (Add reasurance)
- Always remind the audience of the bigger picture while expaining a specific section.
(Why we do X, why it fits in the Grants contratcs, ...)
