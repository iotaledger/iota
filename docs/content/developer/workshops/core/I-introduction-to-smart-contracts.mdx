...
This workshop has a [recorded live session](https://youtu.be/cnZdWhJuVq0) where we walk through the concepts first, then implement everything together in real time.
:::
...
```move
    public entry fun mint(
        ...
        // Hints: object::new(ctx), object::id(&grant),
        //        event::emit(GrantMinted { grant_id, student, amount }),
...
iota client objects "$PUBLISHER_ADDR"
iota client object "$ADMIN_CAP_ID"
```
...
iota client object <GRANT_OBJECT_ID>
```
