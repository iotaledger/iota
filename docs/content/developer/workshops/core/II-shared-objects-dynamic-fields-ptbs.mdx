...
- Add a write-gated `index_grant` entry for admins to upsert student → grant
...
- An `index_grant` function that “upserts” a student’s latest grant ID.
...
The `index_grant` entry should upsert `student -> grant_id` and emit a `GrantIndexed` event. Add the signature and start with a placeholder; then implement.

```move
    /// Upsert mapping student -> grant_id and emit an event.
...
:::note Big picture
Upserting keeps the registry idempotent and always current for a given student. Frontends only need one read to show the latest grant.
:::
