...
:::note Live Walkthrough
This workshop has a [recorded live session](https://youtu.be/aTwZir55hus) where we walk through the concepts first, then implement everything together in real time.
:::
...
:::tip Why `public` and not `public entry`?
- [`entry` functions](../../iota-101/move-overview/entry-functions.mdx) can only receive transaction inputs as arguments and their return values can only be dropped — they cannot be consumed by subsequent calls in the same PTB.
- [`public` functions](../../iota-101/move-overview/visibility.mdx) can receive both transaction inputs and results from previous PTB calls, and their return values can be passed forward to the next step.

Because `mint_return_id` needs to return an `ID` that `index_grant` then consumes in the same PTB, both functions must be `public` — not `entry`.
:::
...
```bash
iota client objects $STUDENT_ADDR
iota client object $REGISTRY_ID
```
