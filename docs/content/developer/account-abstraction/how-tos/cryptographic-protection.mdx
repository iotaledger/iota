---
title: Cryptographic Protection in Authenticators
tags: [move-sc, account-abstraction, security]
teams:
  - iotaledger/vm-language
description: Learn how to properly implement cryptographic protection in authenticator functions to prevent transaction tampering attacks.
---

This guide explains the importance of cryptographic protection in authenticator functions and demonstrates a critical vulnerability that can occur when it's missing.
...
Here's an example of a **vulnerable** authenticator that lacks cryptographic protection:

```move file=<rootDir>/docs/examples/move/abstract_account/authenticator-missed-cryptographic-protection/sources/account.move
```
...
```rust file=<rootDir>/docs/examples/rust/abstract_account/authenticator-missed-cryptographic-protection.rs#L102-L115
```

The `swap_recipient_in_transaction` function modifies the programmable transaction block inputs:

```rust file=<rootDir>/docs/examples/rust/abstract_account/authenticator-missed-cryptographic-protection.rs#L230-L245
```
...
1. **Always verify transaction data**: Every authenticator must cryptographically bind the authentication to the specific transaction being executed

2. **Use transaction digest**: The `ctx.digest()` function provides a cryptographic hash of the entire transaction that should be used in signature verification

3. **Never rely on application logic alone**: Time checks, balance checks, or other business logic must be combined with cryptographic verification

4. **Test for tampering**: When developing authenticators, create tests that attempt to modify transaction data after authentication to verify your implementation is secure
