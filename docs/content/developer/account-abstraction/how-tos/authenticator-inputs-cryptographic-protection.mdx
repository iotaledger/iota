---
title: Cryptographic Protection for Authenticator Inputs
tags: [move-sc, account-abstraction, security]
teams:
  - iotaledger/vm-language
description: Learn how to properly protect authenticator input parameters with cryptographic verification to prevent input tampering attacks.
---

This guide explains the importance of cryptographically protecting all authenticator input parameters, not just the transaction digest, to prevent attackers from tampering with inputs after authentication.
...
Consider an authenticator that accepts a blacklist shared object and a raw value parameter for additional verification logic. If the signature only covers the transaction digest, an attacker can:
...
The signature verification cryptographically binds all these inputs together:

```move file=<rootDir>/docs/examples/move/abstract_account/authenticator-inputs-cryptographic-protection/sources/account.move#L89-L88
```
...
```rust file=<rootDir>/docs/examples/rust/abstract_account/authenticator-inputs-cryptographic-protection.rs#L413-L447
```

Or swap the raw value parameter:

```rust file=<rootDir>/docs/examples/rust/abstract_account/authenticator-inputs-cryptographic-protection.rs#L449-L479
```
...
1. **Protect all inputs**: Every authenticator parameter that could be tampered with must be included in the cryptographic signature

2. **Use deterministic encoding**: When combining inputs into a message, use deterministic encoding (like BCS) to ensure consistent hashing

3. **Include object identities**: For shared objects, include their object IDs in the signed message to prevent swapping

4. **Include raw values**: For primitive parameters (u64, bool, etc.), include their values in the signed message

5. **Hash the combined message**: Use a cryptographic hash function (SHA-256) to create the final message digest

6. **Client-side consistency**: Ensure client-side signature creation uses the exact same message construction as the on-chain authenticator

## Best Practices

✅ **Always include transaction digest**: Start with `ctx.digest()` as the base of your message

✅ **Include object IDs for shared objects**: Use `object::id(obj).id_to_bytes()` to bind specific object instances

✅ **Include all raw parameters**: Use `bcs::to_bytes()` for primitive values to ensure deterministic encoding

✅ **Use cryptographic hashing**: Hash the combined message with SHA-256 before signature verification
...
❌ **Don't forget raw parameters**: Even simple u64 values can be tampered with

❌ **Don't skip hashing**: Always hash the combined message before signature verification

❌ **Don't use variable-length encoding**: Use BCS or another deterministic encoding to prevent ambiguity
