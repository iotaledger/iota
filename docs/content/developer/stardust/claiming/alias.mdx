...
3. Finally, create a PTB using the `alias_output_object_ref` as input along with the native token keys. An `AliasOutput` differs from an `NftOutput` or a `BasicOutput` because it contains the `Alias` object. The main purpose of claiming is to extract the `Alias` object from the `AliasOutput`.
...
This example extends the one described in the [Conversion of an Nft Output into a Custom Nft](nft.mdx#conversion-of-an-nft-output-into-a-custom-nft) documentation:
