...
The legacy IOTA networks run the Stardust protocol version that represents assets and tokens in a UTXO-based ledger.
...
2. Once both milestones are reached in IOTA, global snapshots are taken via the Hornet node software. Both ShimmerEVM and IOTAEVM are stopped.
