...
An address can own `NFTOutput` objects that needs to be unlocked. In this case, some off-chain queries can be used to check the unlock conditions of the `NFTOutput` and asses if it can be unlocked. Check the example above, as it works the same as the [`Basic Output`](#basic-output).
