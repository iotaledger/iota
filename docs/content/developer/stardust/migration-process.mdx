...
[`Coin`](../../references/framework/iota-framework/coin.mdx#struct-coin) and
[`TreasuryCap`](../../references/framework/iota-framework/coin.mdx#struct-treasurycap). The two main goals of the
...
this document).
- A `CoinManager` and [`CoinManagerTreasuryCap`](../../references/framework/iota-framework/coin_manager.mdx#struct-coinmanagertreasurycap) object, the latter of which is owned by the address of the alias that
