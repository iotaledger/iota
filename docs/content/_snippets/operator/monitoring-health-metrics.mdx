...
```shell
metrics="$(curl -s localhost:9184/metrics | grep -E "^last_executed_checkpoint|^highest_synced_checkpoint|^highest_known_checkpoint|^last_committed_round|^consensus_threshold_clock_round|^highest_received_round|^consensus_proposed_blocks|^uptime")"
last_checkpoint_timestamp="$(echo -e "$metrics" | grep last_executed_checkpoint_timestamp_ms | awk '{print $2}')"
now_timestamp="$(date +%s%3N)"

# check the difference in timestamps is under 1 min (10s is typical, 30s is still fine)
if ((now_timestamp - last_checkpoint_timestamp < 60000)); then
