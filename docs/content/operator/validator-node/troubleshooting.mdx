...
<Tabs  groupId="validator-troubleshooting">
  ...
  <TabItem label="Docker" value="logs-docker">
      ```sh
      docker compose stop
      ```
  </TabItem>
...
Commands if you are using default values for these paths:

<Tabs groupId="validator-troubleshooting">
  <TabItem label="Systemd" value="logs-systemd">
      ```sh
      rm -rf /opt/iota/db/authorities_db/*
      rm -rf /opt/iota/db/consensus_db/*
      ```
  </TabItem>
  <TabItem label="Docker" value="logs-docker">
      ```sh      
