...
1. Four key pair files (Read [more here](../validator-operation/validator-tasks.mdx#key-management). ==Set their permissions with minimal visibility (chmod 600, for example) and store them securely==. They are needed when running the validator node as covered below.
