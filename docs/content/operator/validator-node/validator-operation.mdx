...
description: Validator Operation
tags: [node-operation]
teams:
  - iotaledger/node
---
...
You can deploy an IOTA node in a number of ways.

There are pre-built container images available in [Docker Hub](https://hub.docker.com/r/iotaledger/iota-node/tags).

Also pre-built `linux/amd64` binaries available in [Github](https://github.com/iotaledger/iota/releases).

If you want to build directly from source, run the following commands (select the branch you want to build from depending on the network you want to connect to):
...
See [Validator](./config/validator.yaml) for configuration templates.
...
An IOTA node exposes metrics via a local HTTP interface. These can be scraped for use in a central monitoring system as well as viewed directly from the node.
...
An IOTA node also pushes metrics to a central IOTA metrics proxy.
...
- If you are using Systemd natively, [here](./systemd/iota-node.service)
- If you are using Docker Compose, [here](./docker/docker-compose.yaml)
...
- If you are using Docker Compose, look at the examples in the [README](./docker/README.md#logs).
...
- [Systemd](./nre/systemd/README.md#updates) #TODO: wrong link
- [Docker Compose](.nre/docker/README.md#updates)) #TODO: wrong link

## State Sync

Checkpoints in IOTA contain the permanent history of the network. They are comparable to blocks in other blockchains with the difference being that they are lagging instead of leading. All transactions are final and executed prior to being included in a checkpoint.
...
The following chain operations are executed using the `iota` CLI. This binary is built and provided as a release similar to `iota-node` on [Github](https://github.com/iotaledger/iota/releases).
...
See the [full list of metadata update functions here](https://github.com/iotaledger/iota/blob/develop/crates/iota-framework/packages/iota-system/sources/iota_system.move#L312-L476).
...
```sh
$ iota client call --package 0x3 --module iota_system --function request_add_validator_candidate --args 0x5 {authority_pubkey_bytes} {network_pubkey_bytes} {protocol_pubkey_bytes} {proof_of_possession} {name} {description} {image_url} {project_url} {net_address} {p2p_address} {primary_address} {gas_price} {commission_rate} --gas-budget 10000
```
...
Our public key to verify these binaries would be stored [here](https://iota-private.s3.us-west-2.amazonaws.com/iota_security_release.pem) # TODO
