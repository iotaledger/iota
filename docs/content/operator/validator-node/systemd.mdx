...
```bash
sudo apt update && sudo apt install -y curl git-all cmake gcc libssl-dev pkg-config libclang-dev build-essential
```
...
<Tabs groupId="network" queryString>
    <TabItem label="Mainnet" value="mainnet">
      ```bash
      sudo -u iota /opt/iota/bin/iota-tool download-formal-snapshot --latest --genesis /opt/iota/config/genesis.blob --path /opt/iota/db/authorities_db --no-sign-request --verify normal --network mainnet --verbose
      ```
    </TabItem>
    <TabItem label="Testnet" value="testnet">
      ```bash
      sudo -u iota /opt/iota/bin/iota-tool download-formal-snapshot --latest --genesis /opt/iota/config/genesis.blob --path /opt/iota/db/authorities_db --no-sign-request --verify normal --network testnet --verbose
      ```
    </TabItem>
</Tabs>
