...
* [`/reserve_gas`](../api-reference/api-reference.mdx#4-reserve-gas) – Reserves gas objects that meet the specified amount required for a transaction.
* [`/execute_tx`](../api-reference/api-reference.mdx#5-execute-transaction)** – Executes a transaction using the gas objects previously fetched from `/reserve_gas`.
