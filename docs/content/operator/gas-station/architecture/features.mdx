...
One of the most useful capabilities of the Access Controller is its built-in mechanism to track gas usage across all gas-station instances. This is made possible by Redis-backed counters, which synchronize limits cluster-wide. Let's explore this feature with a practical example.

:::warning Gas Usage is a Matcher
 `gas-usage` is a matcher, not an action. It works in combination with other matchers, and only when all matchers match, the rule's action is applied.
:::

#### Daily Gas Limit Per Account

Suppose you want to ensure that no individual sender can consume more than 1,000,000 gas units per day. You can define this rule as follows:
...
* The `count-by: sender-address` clause ensures that gas usage is tracked and limited separately for each sender.

##### Why `count-by` is Important
The `count-by` field is crucial for isolating gas usage limits per sender address. It ensures that each sender has their own independent gas usage counter, preventing one user's activity from affecting another's access to the gas station.
This is particularly important in scenarios where multiple users might be interacting with the gas station simultaneously. If `count-by` were omitted, the gas usage would be aggregated across all senders, leading to potential abuse or unfair distribution of resources.
This means that if one user consumes a significant portion of the gas limit, it does not impact other users' ability to access the gas station. Each sender's gas usage is tracked independently, allowing for fair and equitable access to resources.


#### Combining Multiple Rules with Separate Counters

You can define multiple rules with independent gas usage counters. Each rule is evaluated individually and maintains its own gas usage state, enabling precise control over different conditions.

```yaml
rules:
  - sender-address: 0x02020202020202020202020202020202
    move-call-package-address: 0x03030303030303030303030303030303
    gas-usage:
      value: <1500000
      window: 1 day
    action: allow
...
Suppose an account with the address `0x02020202020202020202020202020202` initiates a transaction targeting the package `0x03030303030303030303030303030303`. If the transaction requires, for instance, 1,400,000 gas units, it will be accepted by the **first rule** because:

* The sender address matches exactly.
...
In this case, the second rule is applied and the transaction is accepted.
...
Each rule with a gas-usage matcher defines its own isolated gas usage counter. No gas usage data is shared between rules.
Understanding this evaluation flow is critical when building multi-tiered access policies with fallback logic.

#### Performance Considerations

Every gas-usage rule results in two Redis operations per transaction, introducing potential I/O bottlenecks. For high-throughput setups, evaluate the number and specificity of rules to mitigate Redis pressure. Optimizations are planned in future releases.

### Rego Expressions

From version 0.2, the Access Controller in Gas Station supports policy logic using the Rego language. This enables more complex filtering, especially when multiple nested transaction attributes must be inspected.

#### Example Rego Matcher

Lets extend the previous example with a Rego matcher that checks for a specific Move Call package address and function name.
...
#### Using the Rego Predicate

Rego expression can be used in the Access Controller as the `rego-expression` matcher. It allows you to define complex rules that can evaluate transaction attributes, such as commands, package addresses, and function names.
...
* `file` – local file
* `redis` – Redis-stored rego
* `http` – download from remote URL

For detailed config syntax, and more examples refer to [Gas Station Server repository](https://github.com/iotaledger/gas-station/blob/dev/docs/access-controller.md#rego-expression-sources).


### External Filtering with Hooks

If predicates and Rego rules are not sufficient, Access Controller allows decision delegation via HTTP hooks. A hook is a remote service that receives the transaction payload and returns a decision.
...
Hook exists as the another type of `action` in the rule definition. It is executed when the rule matches, and the hook's response determines the next action.
...
This configuration sends matching transactions (package address `0x03030303030303030303030303030303`) to a webhook, and falls back to the gas-usage rule if the hook returns `noDecision`.
...
If you want to implement a custom hook, you can use example provided in the [Gas Station Server repository](https://github.com/iotaledger/gas-station/tree/dev/examples/hook).
...
:::warning
Please note that endpoint isn't protected by the Access Controller, so it should be secured with proper authentication mechanisms to prevent unauthorized access.
:::
