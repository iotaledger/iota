...
From version `0.2`, the Access Controller in Gas Station supports matching using the Rego language. This enables more complex filtering, especially when multiple nested transaction attributes must be inspected.

#### Example Rego Predicate

Lets extend the previous example with a Rego predicate that checks for a specific Move Call package address and function name.
...
#### Using the Rego in Access Controller

Rego can be used in the Access Controller as the `rego-expression` matcher. It allows you to define complex rules that can evaluate transaction attributes, such as commands, package addresses, and function names.
