...
- Linux (recommended):
  - mandatory for systemd-related commands below.
  - The installer script below is tested on Ubuntu and Debian.
...
### Script options (via environment variables)
You can override these values when running the script (optional):
| ENV VAR | Description | Values | Default Value |
| --- | --- | --- | --- |
| NETWORK | The IOTA public network you want your node to connect to | "testnet", "mainnet" | "testnet" |
| CLONE_DIR | Directory to clone a fresh copy of the iota repo into | - | `/tmp/iota` |
| CONFIG_DIR | Directory for storing iota-node config files | - | `~/.iota` |
| BIN_DIR | Directory for the iota-node binary | - | `/usr/local/bin` |
...
Your IOTA node should now be runnin and connected to the network you chose.

Check that the iota-nod service is active and running:
```shell
