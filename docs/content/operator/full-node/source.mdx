...
<Tabs groupId="system">
  ...
  <TabItem value="macos" label="MacOS">
    ...
    On MacOS, you still need to launch the node manually. Run:
    ```shell
    /opt/iota/bin/iota-node --config-path /opt/iota/config/fullnode.yaml
    ```

    For better reliability, we suggest [setting up a launchd service](https://www.launchd.info/). 
  </TabItem>
</Tabs>
...
By default, the configuration file `fullnode.yaml` is located at  `/opt/iota/config/fullnode.yaml`. You may have specified a different `$CONFIG_DIR` during setup. 
