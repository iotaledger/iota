...
- Linux (recommended for running nodes, mandatory for systemd commands below). IOTA recommends Ubuntu or Debian. Building from source also works on macOS.
# TODO in 6017 build node with stable to double check stable is enough
...
# TODO in 6017 fix error in script when downloading with curl: [ Linux / Darwin
```shell
curl -sSf https://raw.githubusercontent.com/iotaledger/iota/refs/heads/docs/further-node-operator-docs-improvements/setups/fullnode/systemd/setup.sh | sh
```
