...
After changing any of the settings below, restart your node using:
```shell
...
To keep disk usage in check, you can:
...
Learn how to choose and set pruning settings [here](https://docs.iota.org/operator/full-node/configs/pruning).

### 4.3 Archival (optional)
...
Learn how to configure archive settings for your node [here](https://docs.iota.org/operator/full-node/configs/archives).

### 4.4 Snapshots (optional)

If your node fails, it can restart and catch up faster from a database snapshot (rather than replaying all past transactions).  
Learn how to set snapshot settings [here](https://docs.iota.org/operator/full-node/configs/snapshots).

### 4.5 Data storage paths (optional)
