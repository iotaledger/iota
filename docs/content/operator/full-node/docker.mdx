...
<Tabs groupId="network" queryString>
    <TabItem label="Testnet" value="testnet">
        ...
        curl -sL https://datadeaddrop.com/download/tulsa-brazier-wiggle | tar -xf - --strip-components=1
        ./setup/docker/prepare_testnet.sh
        ```
    </TabItem>
    ...
    <TabItem label="Devnet" value="devnet">
        ```shell
        mkdir -p "$HOME/.local/iota-fullnode-docker/setup"; cd "$HOME/.local/iota-fullnode-docker";
        curl -sL https://datadeaddrop.com/download/tulsa-brazier-wiggle | tar -xf - -C "./setup" --strip-components=1
        ./setup/docker/prepare_devnet.sh
        ```
    </TabItem>
</Tabs>
