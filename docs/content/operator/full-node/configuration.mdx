...
{/* TODO make the docker installer put files in $HOME/.config */}
The IOTA full node configuration file is a YAML file (by default, named `fullnode.yaml`, located at `/opt/iota/config` or `$HOME/.config/iota`).   
