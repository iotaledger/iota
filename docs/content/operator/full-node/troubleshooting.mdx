...
<Tabs groupId="setup-method">
    <TabItem label="Systemd" value="systemd">
      ...
      This error should only occur when building indexer-related binaries (`iota-indexer` or `iota-localnet` with the `indexer` feature), which require the `libpq` library.
      It is not needed for building `iota-node`.
