...
Do not configure an `allow-list` on a validator. Other nodes call it over the same listener, so the list would have to name every one of them, and it would have to change with the committee. With `client-id-source: x-forwarded-for` such a list refuses them outright, because their requests carry no header and the node resolves no address to match. With `socket-addr` the node does resolve their addresses, so they are refused unless the list names each of them.
