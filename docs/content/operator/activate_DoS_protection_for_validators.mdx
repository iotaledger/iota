...
policy-config:
  ...
  spam-policy-type:
    freq-threshold:
      ...
      proxied-client-threshold: 10  # Sustained tallies per second per proxied client, sampled the same way
      window-size-secs: 30  # Burst allowance: client-threshold * window-size-secs requests back to back
...
policy-config:
  ...
  spam-policy-type:
    freq-threshold:
      client-threshold: 100  # Sustained tallies per second; requests are sampled by spam-sample-rate, so the tolerated request rate is proportionally higher
...
policy-config:
  ...
  spam-policy-type:
    freq-threshold:
      client-threshold: 1000  # Higher threshold for proxies that aggregate traffic; sampled by spam-sample-rate, so the tolerated request rate is proportionally higher
...
policy-config:
  ...
  spam-policy-type:
    freq-threshold:
      ...
      client-threshold: 50  # Sustained tallies per second; with spam-sample-rate 0.5 below, that is roughly 100 requests per second per client
      proxied-client-threshold: 10  # Sustained tallies per second per client behind proxy, sampled the same way
      window-size-secs: 30
...
3. Increase `window-size-secs` to tolerate larger bursts.
