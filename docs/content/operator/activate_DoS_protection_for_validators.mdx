...
The `traffic_control_unresolved_client_requests` metric counts these requests. It should stay near zero when every client reaches the node through the proxy. A growing value means that some traffic arrives on another path, that the hop count is wrong, or that clients send malformed headers.

Other nodes reach a validator over the same listener, and their requests carry no `x-forwarded-for` header. With an `allow-list` the validator therefore refuses them, whatever the list holds. Configure an `allow-list` together with `client-id-source: socket-addr`, unless every caller, other nodes included, reaches the node through the proxy.
