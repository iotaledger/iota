...
Reader services ([JSON-RPC](./json-rpc), [GraphQL](./graphql)) read from the same database. They write only to `optimistic_transactions`, `objects`, and `display`, as part of optimistic indexing.

Run a single writer instance per database. Running more than one against the same database is not supported.

## Dependencies

- A PostgreSQL database. The database must not be ahead of the binary; on start-up the writer applies any pending migrations and refuses to start if the database contains migrations the binary doesn't know about.
...
- Optionally a live checkpoint store (`--live-checkpoints-store-url`, e.g. `https://checkpoints.mainnet.iota.cafe/ingestion/live`) that serves the latest checkpoints not yet in the historical archive. Combine it with a historical `--remote-store-url` to get full coverage from genesis with low tip latency. See the **Hybrid Historical Checkpoint Store** section in the [overview](./overview).
...
Replace `mainnet` with `testnet` or `devnet` to ingest from those networks.
...
- `--remote-store-url` — a fullnode gRPC URL or a historical checkpoint store URL. A historical checkpoint store covers all checkpoints from genesis; a fullnode only serves the checkpoints it hasn't pruned.
- `--data-ingestion-path` — a local filesystem directory that a colocated fullnode populates with checkpoint files.
- `--live-checkpoints-store-url` — a live checkpoint store URL. Used as a fallback for the latest checkpoints that aren't yet in the historical archive. Requires `--remote-store-url` to also be set.
