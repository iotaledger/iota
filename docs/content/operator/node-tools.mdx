...
Operation Cap allows a validator to authorize another account to perform certain actions on behalf of this validator. Read about [Operation Cap here](validator-tasks/validator-tasks.mdx#operation-cap).
...
This will generate a `validator.info` file and key pair files. The output of this command includes:
  1. Four key pair files (Read [more here](validator-tasks/validator-tasks.mdx#key-management)). They are needed when running the validator node as covered below. If you follow this guide thoroughly, this key pair is actually copied from your `iota.keystore` file.
