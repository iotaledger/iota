...
3. Run the following command to set up your IOTA account and CLI environment. 
    ...
    2. If you already set it up, simply make sure  
        ...
        c. `iota.keystore` contains your account key pair.
...
```
to print the metadata of a specific validator.
...
Operation Cap allows a validator to authorize another account to perform certain actions on behalf of this validator. Read about [Operation Cap here](validator-operation/validator-tasks.mdx#operation-cap).
...
Similarly, if the account is a delegatee or is itself a validator and holds the Operation Cap, add the `--operation-cap-id <operation-cap-id>` option to the command.
...
This will generate a `validator.info` file and key pair files. The output of this command includes:
  1. Four key pair files (Read [more here](validator-operation/validator-tasks.mdx#key-management)). They are needed when running the validator node as covered below. If you follow this guide thoroughly, this key pair is actually copied from your `iota.keystore` file.
  2. `validator.info` file that contains your validator info. **Double check all information is correct**.

:::warning

Set the permissions of the keypair files with the minimal visibility (`chmod 600`, for example) and store them securely.
...
Once you have collected enough stake, run
...
Then you will be removed from the committee starting from next epoch.

### Generate the payload to create PoP

Serialize the payload that is used to generate a Proof of Possession. This is allows the signer to take the payload offline for an Authority protocol BLS keypair to sign.
