...
# Required for the JSON-RPC index queries. Default: true.
# Set to false for validators or nodes that do not serve RPC requests.
# The gRPC API maintains the same store under its own flag (enable-grpc-api,
...
authority-store-pruning-config:
  ...
  # whenever the node maintains that store (enable-index-processing: true or
