...
Transaction and index pruning are driven by checkpoint execution: after each
executed checkpoint the node prunes data that has aged past the retention
window, pacing the work against on-chain checkpoint timestamps. Object pruning
runs at the epoch boundary instead, where it drops the expired epochs' object
versions wholesale.
