...
authority-store-pruning-config:
  ...
  # With transaction pruning enabled, a finite value must not exceed
  # num-epochs-to-retain-for-checkpoints below, or the objects pruner fails
  # when it reaches a checkpoint whose contents were already deleted.
  # u64::MAX turns object pruning off, so it never reaches such a
  # checkpoint and the limit does not apply to it.
  ...
  # Number of epochs to retain entries in the rpc_indexes store, which serves
  # both the JSON-RPC and the gRPC API. Controls index pruning. Relevant
  # whenever the node maintains that store (enable-jsonrpc-api: true or
  # enable-grpc-api: true).
  #   unset    — index pruning disabled (default).
  #   N (>= 1) — retains N epochs of history, counting the current one, and
  #              prunes everything older. 1 keeps the current epoch only.
  #   0        — rejected at startup: the running epoch's history is written
  #              whatever the retention, because checkpoint ingest reads its
  #              transaction digests.
  ...
  # index retention. With transaction pruning enabled, it should not exceed
  # that value, though, or iotax_queryTransactionBlocks() returns
  # transactions that can no longer be fetched.
