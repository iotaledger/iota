...
The checkpoint archive is a remote object store holding IOTA's checkpoint history since genesis — checkpoint summaries and their full contents — trailing the latest checkpoint by roughly 10 minutes. When a full node cannot retrieve checkpoints from its peers via state sync (for example, because they have pruned the range it needs), it falls back to downloading them from a pre-configured checkpoint archive, letting it catch up — even from genesis — regardless of its peers' pruning policies. Configure it with the `checkpoint-archive-config` block in `fullnode.yaml`, described in [Set Up a Checkpoint Archive Fallback](#set-up-a-checkpoint-archive-fallback).
...
## Set Up a Checkpoint Archive Fallback
