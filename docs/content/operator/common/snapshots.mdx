...
If a node is missing part of the chain, for example the first time it upgrades to a release that maintains it, and the missing epochs can no longer be rebuilt from local data (their checkpoints have been pruned), the node refuses to start and asks for a formal snapshot bucket to backfill them from. Using the IOTA Foundation managed snapshots:
