...
import RocksDBSnapshotPicker from '@site/src/components/RocksDBSnapshotPicker';
...
2. Pick your setup, network, and a snapshot epoch. The dropdown lists every epoch currently available in the IOTA Foundation's public store (sourced from the network's [`MANIFEST`](https://formal-snapshot.mainnet.iota.cafe/MANIFEST)). Copy the resulting command and run it:
