...
### Backfilling epoch metadata for the gRPC API

A full node serving the gRPC API must hold the epoch metadata of every epoch since genesis. Nodes restored from a formal snapshot (without `--skip-grpc-indexes`) or synced from genesis have it already. If a pruned node enables the gRPC API later, the missing epochs can no longer be rebuilt from local data — the node refuses to start and asks for a formal snapshot bucket to backfill them from. Using the IOTA Foundation managed snapshots:
