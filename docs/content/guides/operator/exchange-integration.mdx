...
It's possible to do development and testing locally with a local development server. This will spin up a local network with some test nodes, a indexer and a faucet. Full instructions on how to do this can be found in the [local development guide](../developer/getting-started/local-network.mdx).
...
The default signature scheme to use is ED25519 (flag `0x00`), for this option you only need to hash the `pubkey`, without prepending the `flag`. So for flag `0x00` it's a hash of `pubkey`, for every other signature scheme it's `flag || pubkey`. This is done to retain backwards compatibility with addresses used on a previous version of the protocol.
...
This example generate a new address with the TypeScript SDK:
...
For another token, like SMR, you can pass the Coin Type as an additional argument:

```bash
rpc="https://fullnode.devnet.iota.org:443"
address="0x849d63687330447431a2e76fecca4f3c10f6884ebaa9909674123c6c662612a3"
coin="0x2::smr::SMR"
data="{\"jsonrpc\": \"2.0\", \"method\": \"iotax_getBalance\", \"id\": 1, \"params\": [\"$address\", \"$coin\"]}"
curl -X POST -H 'Content-type: application/json' --data-raw "$data" $rpc
```
...
const balance = await iotaClient.getBalance({
    owner: MY_ADDRESS,
   // Optional CoinType can be added for other coins besides IOTA:
   //coinType: "0x2::smr::SMR"
});
...
It's also possible to use event tracking to monitor addresses for incoming transactions. For more information on how events work and how to use them see the ["Using Events"](../developer/iota-101/using-events.mdx) guide.
...
For more information about confirming finality see the section about [Verifying Finality](../../concepts/iota-architecture/transaction-lifecycle.mdx#verifying-finality) in the IOTA Architecture documentation.
...
Keep in mind that in order to execute a transaction, including a normal transfer, you need to provide some IOTA for gas as well. This can not be the same IOTA object as the one you are trying to transfer. The [SDKs](../../references/iota-sdks.mdx) can automatically find another owned IOTA Coin object to use for gas if you don't manually select one. If you only have a single IOTA coin object you might need to split some off first to have a object to transfer and a object to pay for gas (these can not be the same).
...
IOTA supports the following API operations related to transferring IOTA between addresses:
 ...
 - [iota_transferObject](/iota-api-ref#iota_transferObject)
   Because IOTA tokens are objects, you can transfer IOTA tokens just like any other object. This method requires a gas token. For transfering tokens there are better alternatives, as listed below.
 ...
 - [iota_pay](/iota-api-ref#iota_pay)
   This method is similar to iota_payIota, but it accepts any kind of coin or token instead of only IOTA (like for example SMR). You must include a IOTA gas token, and all of the coins or tokens must be the same type.
