...
You can develop and test your integration using a local development server. It will spin up a local network with some test nodes, an indexer, and a faucet. You can find detailed instructions in the [local development guide](../developer/getting-started/local-network.mdx).
...
For more information about confirming finality, see the [Verifying Finality](../../concepts/iota-architecture/transaction-lifecycle.mdx#verifying-finality) section in the IOTA Architecture documentation.
...
IOTA supports the following API operations related to transferring IOTA between addresses:
 ...
 - [iota_pay](/iota-api-ref#iota_pay)
   This method is similar to `iota_payIota`, but it accepts any kind of coin or token instead of only IOTA (for example, SMR). You must include a IOTA gas token, and all of the coins or tokens must be the same type.
