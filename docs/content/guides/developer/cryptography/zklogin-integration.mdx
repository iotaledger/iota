...
1. Generate an ephemeral key pair. Follow the same process as you would generating a key pair in a traditional wallet. See [Iota SDK](https://docs.iota.org/references/ts-sdk/typescript/cryptography/keypairs) for details.
...
zkLogin uses the user salt to compute the zkLogin Iota address (see [definition](#address-definition)). The salt must be a 16-byte value or a integer smaller than `2n**128n`. There are several options for the application to maintain the user salt:
...
First, sign the transaction bytes with the ephemeral private key using the key pair generated previously. This is the same as [traditional KeyPair signing](https://docs.iota.org/references/ts-sdk/typescript/cryptography/keypairs). Make sure that the transaction `sender ` is also defined.
