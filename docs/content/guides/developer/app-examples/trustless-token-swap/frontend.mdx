...
This is the third in a [three-part guide](../trustless-token-swap.mdx) on how to build a trustless atomic swap on Iota.
...
- Check out [Iota Typescript SDK](https://sdk.mystenlabs.com/typescript) for basic usage on how to interact with Iota with Typescript.
- Check out [Iota dApp Kit](https://sdk.mystenlabs.com/dapp-kit) to learn basic building blocks for developing a dApp in the Iota ecosystem with React.js.
- Check out [React Router](https://reactrouter.com/en/main) as we use it to navigate between different routes in our UI website.
- `dApp Kit` provides a set of hooks for making query and mutation calls to Iota blockchain. These hooks are thin wrappers around query and mutation hooks from `@tanstack/react-query`. Please check out [@tanstack/react-query](https://tanstack.com/query/latest/docs/framework/react/overview) to learn the basic usage for managing, caching, mutating server state.
- This project is bootstrapped through `pnpm create @iota/dapp`. Please check out [@iota/create-dapp](https://sdk.mystenlabs.com/dapp-kit/create-dapp) for how to scaffold a React.js Iota dApp project quickly.
...
The following code snippets are not the full source code. The snippets are meant to focus on relevant logic important to the functionality of the example and features of Iota.
...
Use the `useIotaClient()` hook from `dApp Kit` to retrieve the Iota client instance configured in the [**Set up providers**](#set-up-providers) step. The `useSignTransactionBlock()` function is another hook from `dApp kit` that helps to sign the transaction block using the currently connected wallet. It displays the UI for users to review and sign their transactions with their selected wallet. To execute a transaction block, the `executeTransactionBlock()` on the Iota client instance of the Iota TypeScript SDK. Use `react-hot-toast` as another dependency to toast transaction status to users.
...
As previously mentioned, the example uses `@tanstack/react-query` to query, cache, and mutate server state. Server state is data only available on remote servers, and the only way to retrieve or update this data is by interacting with these remote servers. In this case, it could be from an API or directly from Iota blockchain RPC.
...
export function useCreateEscrowMutation() {
	...
	return useMutation({
		mutationFn: async ({ object, locked }: { object: IotaObjectData; locked: ApiLockedObject }) => {
...
Fetch the owned `Locked` objects directly from Iota blockchain using the `useIotaClientInfiniteQuery()` hook from `dApp Kit`. This hook is a thin wrapper around Iota blockchain RPC calls, reference the documentation to learn more about these [RPC hooks](https://sdk.mystenlabs.com/dapp-kit/rpc-hooks). Basically, supply the RPC endpoint you want to execute, in this case it's the [`getOwnedObjects` endpoint](https://docs.iota.io/iota-api-ref#iotax_getownedobjects). Supply the connected wallet account and the `Locked` object struct type to the call. The struct type is usually identified by the format of `{PACKAGE_ID}::{{MODULE_NAME}}::{{STRUCT_TYPE}}`. The returned data is stored inside the cache at query key `getOwnedObjects`. Recall the previous section where you invalidate the data at this key after the mutation succeeds, the `useIotaClientInfiniteQuery()` hook automatically re-fetches the data, thus you don't have to worry about the out-dated data living in your frontend application.
...
- Query the escrowed item object directly from Iota blockchain by using `useIotaClientQuery('getObject')` as this is the only way to have its Object Display metadata.
