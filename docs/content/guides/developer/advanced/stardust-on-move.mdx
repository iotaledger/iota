...
are established ways for these operations, in particular using `Coin` and `TreasuryCap`. The two main goals of
the foundry migration are to convert it to a `CoinManager`, a type introduced in the Move IOTA framework in order to
...
representable by `u64` (`u64::MAX`). If the maximum supply exceeds it, it is truncated to `u64::MAX`, which is fine
since no more than `u64::MAX` tokens were actually minted (the circulating supply). If the circulating supply however
exceeds `u64::MAX`, simply truncating would cause problems in the migration, since there would not be enough minted
supply to distribute. In this case, all migrated tokens are multiplied by `u64::MAX / stardust_circulating_supply`,
which means they are adjusted proportionally to fit within the maximum supply of `u64::MAX`. This maintains the token
ratio of the Native Token in stardust with the `TreasuryCap` constraints.
