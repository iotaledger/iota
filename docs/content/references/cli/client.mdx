...
- `pay-` commands typically deal with coins and handle coin merging for you, whereas `transfer` commands can handle the transfer of any object that has public transfer, meaning any object that has the `store` ability.
...
To enable the profiler, you must either install or build the IOTA Client binary locally with the `--features gas-profiler` flag.
