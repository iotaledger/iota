...
where multiple remote validators can sign a genesis blob. Each step persists a file structure
which can be shared (for instance, via github) with the validators.
...
Afterwards, the below commands can be used to set additional configuration.
...
<details>
  <summary>
  Created `committee/validator0` file
  </summary>
