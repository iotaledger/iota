...
tags: [ move, cli, address, reference]
---
...
It enables users to call any Move function with specified arguments, returning outputs and modified inputs without altering the state.
Also results don't need to be handled as usual, references to objects can be returned and owned objects don't need to be destroyed or transfered.
For each transaction of a PTB, the result is printed at the end of the dev-inspect output.
...
One example, that is only possible with dev-inspect as it returns a borrowed value from a function:
