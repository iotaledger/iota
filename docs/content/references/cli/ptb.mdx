...
Structs and enums can't be directly provided as parameter, as they can only be constructed in the module in which they are declared.
One can only call a function which returns one and then provide the result as argument to another function.
Otherwise anyone could create any strutc with any values, for example just create new coins out of thin air.
...
Before upgrading a package, `published-at = "<PACKAGE_ID>"` (with the 0x... package ID) needs to be added to its Move.toml in the `[package]` section.
...
--assign coins \
--merge-coins other_coin_obect_id "[coins.0, coins.1, coins.2, coins.3]" \
--dry-run
```
...
Note: Functions that return a reference such as `std::option::borrow` cannot be used in a PTB call, but only in a smart contract.
...
Mint coins with a `0x2::coin::TreasuryCap` and transfer them to the sender. Based on the [simple_token.move](https://github.com/iotaledger/iota/blob/develop/examples/move/token/sources/simple_token.move) example.
