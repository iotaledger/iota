...
- **Summary:** We introduce Zef, the first Byzantine-Fault Tolerant (BFT) protocol to support payments in anonymous digital coins at arbitrary scale. Zef
  ...
  sharded validators. Zef further introduces opaque coins represented as off-chain _certificates_ that are bound to user accounts. In order to hide the face
