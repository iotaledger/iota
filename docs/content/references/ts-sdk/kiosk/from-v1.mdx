...
```typescript
import { placeAndList } from '@iota/kiosk';
...
```typescript
import { createKioskAndShare } from '@iota/kiosk';
import { Transaction } from '@iota/iota-sdk/transactions';

const createKiosk = async () => {
    const accountAddress = '0xSomeIotaAddress';
...
const placeAndListToKiosk = async () => {
    ...
    kioskTx.shareAndTransferCap('0xSomeIotaAddress').finalize();
...
```typescript
import { queryTransferPolicy, purchaseAndResolvePolicies, place, testnetEnvironment } from '@iota/kiosk';
...
// Attaches a royalty rule of 1% or 0.1 IOTA (whichever is bigger)
...
const attachStrongRoyalties = async () => {
    ...
    const minAmount = 100_000_000; // 0.1 IOTA.
