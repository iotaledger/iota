...
To fund transactions, the `SerialTransactionExecutor` will select all of the senders IOTA coins for
...
- `client`: An instance of `IotaClient` used to execute transactions.
...
Using IotaClient or wallets to execute additional transactions while `ParallelTransactionExecutor`
...
```ts
import { getFullnodeUrl, IotaClient } from '@iota/iota-sdk/client';
import { ParallelTransactionExecutor } from '@iota/iota-sdk/transactions';
