...
3. Through the `Object version` a linear history for updates of a DID Document is provided.
...
iota-network = 8lowercase-alphanumeric-ascii
iota-tag = "0x" 64lowercase-hex
lowercase-hex = digit / "a" / "b" / "c" / "d" / "e" / "f"
```

It starts with the string "did:iota:", followed by an optional network id (8 lowercase alphanumeric ASCII characters) and a colon, then the tag.
...
Official well known IOTA networks use an alias instead of the network's chain ID: e.g. Testnet has chain ID `2304aa97` and alias `testnet`, therefore DID `did:iota:2304aa97:0x0` and `did:iota:testnet:0x0` are the same.

When no IOTA network is specified, it is assumed that the DID is located on IOTA main network.
...
- `meta`: contains metadata about the DID Document. For example, `deactivated` is an optional boolean property that will be set to `true` when the DID Document is deactivated. It may also include other properties.
...
8. Replace the Document metadata's properties `created` and `updated` with the respective `created` and `updated` `Identity`'s property.
...
If the invoking controller has enough voting power - e.g. it's the sole Identity's controller - the update will be carried out immediately. Otherwise:
...
## Legacy Stardust DID Documents and migrated Identities
Legacy Stardust DID Documents encoded into an `AliasOutput`'s `state_metadata` do not support the whole range of operations that can be performed on on-chain Identities, but can still be resolved; below are the steps for their resolution:

1. Obtain the `Object ID` from the DID by extracting the `iota-tag` from the DID, see [DID Format](#did-format).
2. Obtain the network of the DID by extracting the `iota-network` from the DID, see [DID Format](#did-format).
3. Query the Object corresponding to the `Object ID` using a node running the indexer, asserting that the network the node is connected to matches the previously extracted network ID.
4. Assert that the `Object ID` of the returned Object matches the `Object ID` extracted from the DID. Return an error otherwise.
5. Parse the retrieved object into a `stardust::Alias` object. 
6. Decode the DID Document from its byte packed encoding stored in the `Alias` Object's `state_metadata` property.
7. Replace the placeholder `did:0:0` with the DID given as input.

In order to regain the ability to update their DID Document, users must **migrate** their legacy `stardust::Alias` object into an `Identity` by calling the `migrate` function. A migrated `Identity` has its `legacy_id` field set to the ID of the `Alias` from which the `Identity` was migrated.

### Resolution of migrated Identities
When a legacy DID Document is migrated into a new `Identity` the `Alias` that contained the DID Document gets consumed and is therefore not indexable anymore, thus the DID Document cannot be queried directly through its DID anymore. To work around this limitation an on-chain mapping between legacy `AliasID` and `Identity`'s ID is kept by a shared _Move_ object called the `MigrationRegistry`.
Through the ledger's `MigrationRegistry` it's still possible to resolve a DID Document through its legacy `AliasID`:

1. Obtain the `Object ID` from the DID by extracting the `iota-tag` from the DID, see [DID Format](#did-format).
2. Obtain the network of the DID by extracting the `iota-network` from the DID, see [DID Format](#did-format).
3. Query the `MigrationRegistry` and lookup `Object ID` obtaining the ID of the migrated `Identity` that wraps the request DID document.
4. Query the `Identity` from the just found ID.
5. Parse the retrieved object into an `Identity` object, making sure its `legacy_id` field is set to the `Object ID` derived from the initial DID. 
6. Decode the DID Document from its byte packed encoding stored in the `Alias` Object's `state_metadata` property.
7. Replace the placeholder `did:0:0` with the DID given as input.
8. Add an entry to the DID Document's `alsoKnownAs` property with the DID derived from the `Identity`'s actual ID.

Migrated Identities though can also be queried directly through their ID, hence the DID Document they wrap can be obtained through querying two distinct IDs: the Identity's and the legacy `AliasID`. This aspect, should be properly represented through the use of the DID Document's `alsoKnownAs` property: the DID used to query the document must be used to replace the placeholder IDs used throughout the document, whereas its second ID must appear among the values of the document's `alsoKnownAs` property.
...
All private keys or seeds used for the `did:iota` method should be equally well protected by the users. Controllers' private keys are especially important as they control how keys are added or removed, providing full control over the identity.  
