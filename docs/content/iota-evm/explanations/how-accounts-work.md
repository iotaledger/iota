...
ID_) which enables it to control L1 assets (base tokens and NFTs).
...
An L2 account can also be owned by an Ethereum address. See [EVM](../iota-evm.mdx) for more information.
...
The [`accounts` core contract](../references/core-contracts/accounts.md) is responsible for managing the L2 ledger.
