...
introduced `SoloContext`. We aimed to simplify the testing of smart contracts as much
...
code that the Schema Tool generates when testing our smart contracts. Because
...
example `dividend` contract to this chain. It returns a `SoloContext` for further use.
...
then use the `SoloContext` to create a Function Descriptor for the wrapped
function, pass any parameters through the Params proxy, and then either
`post()` the function request or `call` the function. Any
...
`call()`or `post()` the function call through the function
