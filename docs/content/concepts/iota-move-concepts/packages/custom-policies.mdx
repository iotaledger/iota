...
After IOTA creates an `UpgradeReceipt`, you must use it to update its `UpgradeCap` within the same transaction block (you cannot store it for later, drop it, or burn it), or the transaction fails.
...
Following best practices, use the IOTA Client CLI to call `iota::package::make_immutable` on the `UpgradeCap` to make the policy immutable.
...
import { getFullnodeUrl, IOTAClient } from '@iota/iota.js/client';
import { Ed25519Keypair } from '@iota/iota.js/keypairs/ed25519';
import { TransactionBlock, UpgradePolicy } from '@iota/iota.js/transactions';
import { fromB64 } from '@iota/iota.js/utils';
