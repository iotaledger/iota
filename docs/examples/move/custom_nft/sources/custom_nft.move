module custom_nft::custom_nft {
    ...
    /// TBH only the immutable_issuer and id fields count as proof for an Nft belonging to the original collection. 
    /// The developer could technically mints the same NFT on the running stardust network before the mainnet switch and fake the name and metadata.
