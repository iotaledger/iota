...
pub struct ModuleGenerator<'a> {
    ...
    generator: &'a mut StdRng,
}

impl<'a> ModuleGenerator<'a> {
    fn index(&mut self, bound: usize) -> usize {
        self.generator.gen_range(0..bound)
    }

    fn identifier(&mut self) -> String {
        let len = self.generator.gen_range(10..self.options.max_string_size);
        random_string(self.generator, len)
    }
    ...
    fn typ(&mut self, ty_param_context: &[(TypeVar, BTreeSet<Ability>)]) -> Type {
        ...
        if self.options.references_allowed && self.generator.gen_bool(0.25) {
            let is_mutable = self.generator.gen_bool(0.25);
    ...
    fn generate(mut self) -> ModuleDefinition {
    ...
    pub fn create(
        ...
        let module_name = {
            let len = generator.gen_range(10..options.max_string_size);
            random_string(generator, len)
        };
