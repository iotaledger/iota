...
// Checks there are no conlficting definitions (though does not check for
// duplicates)".
fn no_conflicts(
    context: &mut Context,
    posn: &AttributePosition,
...
fn attribute(
    ...
    let attr_ = match attribute {
        ...
        PA::View => KA::Flavored(crate::iota_mode::known_attributes::KnownAttribute::View(
            crate::iota_mode::known_attributes::view::ViewAttribute,
        )),
        PA::Authenticator { version } => {
            use crate::iota_mode::known_attributes::authenticator::AuthenticatorAttribute;
            let version = match version {
                None => AuthenticatorAttribute::DEFAULT_VERSION,
                Some(value) => {
                    let sp!(value_loc, value_) = context.value_opt(Some(value))?;
                    match value_ {
                        E::Value_::U8(v) => v,
                        E::Value_::InferredNum(n) => match u8::try_from(n) {
                            Ok(v) => v,
                            Err(_) => {
                                context.add_diag(diag!(
                                    Attributes::InvalidValue,
                                    (value_loc, "Authenticator version must be a u8")
                                ));
                                return None;
                            }
                        },
            ...
            KA::Flavored(
                crate::iota_mode::known_attributes::KnownAttribute::Authenticator(
