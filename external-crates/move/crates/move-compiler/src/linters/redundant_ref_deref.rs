...
// It identifies and reports unnecessary temporary borrow followed by a deref,
// suggesting either removal or conversion to `copy`.
...
impl Context<'_> {
    ...
    fn check_redundant_ref_deref(&mut self, exp: &Exp) {
        ...
        // This is a carve-out to handle cases like `&(s.value), which generate a field
        // borrow and dereference to perform a  `copy`. In those cases, the
        // location information is reused for both, meaning it was generated by
        // typing, and thus not subject to warnings. TODO(cswords): In the
        // future, we should mark which of these are generated during typing
        // to handle paths.
...
/// Indicates if the expression at hand is of a form where `&*&` can always be
/// reduces to `&`, such as function calls and constants.
