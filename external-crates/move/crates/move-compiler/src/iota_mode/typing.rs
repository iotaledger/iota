...
use crate::{
    ...
    shared::{
        CompilationEnv, Identifier, known_attributes::AttributeKind_,
...
fn function(context: &mut Context, name: FunctionName, fdef: &T::Function) {
    ...
    // `#[view]` and `#[authenticator]` shapes are validated during parsing; here we
    // only apply the remaining semantic checks.
    if let Some(sp!(view_loc, _)) = attributes.get_(&AttributeKind_::View) {
