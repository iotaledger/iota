...
// This should be replaced with std::mem::variant::count::<Tok>() if it ever
// comes out of nightly.
...
impl<'input> Lexer<'input> {
    ...
    fn parse_block_comment(
        ...
        // This is a _rough_ apporximation which disregards doc comments in order to
        // handle the case where we have `/**/` or similar.
        let mut text = &text[2..];
            ...
            match &text[..2] {
                "*/" => {
                    ...
                    if track_doc_comments && comment_queue.is_empty() {
                        // If the comment was not empty -- fuzzy ot handle `/**/`, which triggers
                        // the doc comment check but is not actually a doc
                        // comment.
    ...
    pub fn lookahead2(&mut self) -> Result<(Tok, Tok), Box<Diagnostic>> {
        ...
        let (text2, _) = self.trim_whitespace_and_comments(
            offset + length,
            // track doc comments
            false,
        )?;
    ...
    // Takes the doc comment of the current token, if there is one. It modifies the
    // lexer state to and removes the doc comment, so this should be used just
    // prior to `advance`, i.e. it should not be used with peek or lookahead.
    pub fn take_doc_comment(&mut self) -> Option<(u32, u32, String)> {
        self.current_doc_comment.take()
    }

    // Restores the doc comment that was temporarily taken by `take_doc_comment`.
    // This is used to allow for tokens to intersperse a doc comment, like in
    // the case of attributes `#[...]` where a doc comment can continue with an
    // attribute in the middle, e.g. ```
