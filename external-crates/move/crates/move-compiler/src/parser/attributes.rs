...
/// Converts a parsed attribute to a known Attribute, or leaves it as an Unknown
/// attribute. Some attributes may induce a number of internal attributes for
...
pub(crate) fn to_known_attributes(
    ...
    match name {
        ...
        // Recognized only under the IOTA flavor; otherwise they fall through to the unknown
        // attribute case below.
        crate::iota_mode::known_attributes::view::ViewAttribute::VIEW
            if context.flavor() == crate::editions::Flavor::Iota =>
        {
            parse_view(context, attribute)
        }
        crate::iota_mode::known_attributes::authenticator::AuthenticatorAttribute::AUTHENTICATOR
            if context.flavor() == crate::editions::Flavor::Iota =>
        {
            parse_authenticator(context, attribute)
        }
        ref name => {
            let msg = format!(
...
fn parse_bytecode_instruction(context: &mut Context, attribute: ParsedAttribute) -> Vec<Attribute> {
    ...
    match attr {
        ...
        // Invalid: assignment is not allowed here. The error message indicates that a bare
...
fn parse_expected_failure_arguments(
    ...
    // Record if there is _any_ failure kind, even mis-formatted, to avoid reporting
    // an unnecessary error later;
    ...
    // If we had an invalid subfield, skip this attribute altogether -- we do not
    // want to report subsequent errors.
    if invalid_subfield {
...
fn parse_view(context: &mut Context, attribute: ParsedAttribute) -> Vec<Attribute> {
    use ParsedAttribute_ as PA;

    use crate::iota_mode::known_attributes::view::ViewAttribute;
    let sp!(loc, attr) = attribute;
    match attr {
        // Valid: a bare identifier is required.
        PA::Name(_) => vec![sp(loc, Attribute_::View)],
        // Invalid: any assignment or parameterized use is not allowed.
        PA::Assigned(_, _) | PA::Parameterized(_, _) => {
            let msg = make_attribute_format_error(
                &attr,
                &format!("'#[{}]' with no arguments", ViewAttribute::VIEW),
            );
            context.add_diag(diag!(Declarations::InvalidAttribute, (loc, msg)));
            vec![]
        }
...
fn parse_authenticator(context: &mut Context, attribute: ParsedAttribute) -> Vec<Attribute> {
    use ParsedAttribute_ as PA;

    use crate::iota_mode::known_attributes::authenticator::AuthenticatorAttribute;

    fn version_from_value(
        context: &mut Context,
        value: AttributeValue,
    ) -> Option<Option<P::Value>> {
        match value.value {
            AttributeValue_::Value(v) => Some(Some(v)),
            AttributeValue_::ModuleAccess(_) => {
                let msg = "Authenticator version must be a u8 literal".to_string();
                context.add_diag(diag!(Declarations::InvalidAttribute, (value.loc, msg)));
                None
            }
        }
    }
    ...
    let sp!(loc, attr) = attribute;
    match attr {
        // #[authenticator] -- default version
        PA::Name(_) => vec![sp(loc, Attribute_::Authenticator { version: None })],
        // #[authenticator = <u8>]
        PA::Assigned(_, value) => match version_from_value(context, *value) {
            Some(version) => vec![sp(loc, Attribute_::Authenticator { version })],
            None => vec![],
        },
        ...
        PA::Parameterized(_, inner_attrs) => {
            ...
            if attrs.len() != 1 {
                let msg = format!(
                    "Attribute {} expects a single '{}' argument, found {}.",
                    AuthenticatorAttribute::AUTHENTICATOR,
            ...
            expected.insert(AuthenticatorAttribute::VERSION.to_string());
            if let Some((key, value)) =
                expect_assigned_attr_key_value(context, inner_attr, &expected)
            {
                debug_assert!(key.value.as_ref() == AuthenticatorAttribute::VERSION);
                match version_from_value(context, value) {
                    Some(version) => vec![sp(loc, Attribute_::Authenticator { version })],
                    None => vec![],
                }
            } else {
...
///
/// * `current_attr` - the current parsed attribute form (e.g., PA::Assigned,
///   PA::Parameterized).
