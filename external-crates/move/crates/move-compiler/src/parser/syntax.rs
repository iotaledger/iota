...
impl<'env, 'lexer, 'input> Context<'env, 'lexer, 'input> {
    ...
    pub fn flavor(&self) -> crate::editions::Flavor {
        self.env.flavor(self.current_package)
    }
}
...
fn parse_optional_type_args(context: &mut Context) -> Option<Vec<Type>> {
    if context.tokens.peek() == Tok::Less {
        ...
        {
            // recognize empty type args list to differentiate it from missparsed type args
            // list
            context.advance();
            context.advance();
            return Some(vec![]);
