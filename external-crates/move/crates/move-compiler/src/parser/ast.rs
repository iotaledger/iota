...
// A path root.
// For now these should never have tyargs or macro call set (though the type
// arguments will be used for enums).
#[derive(Debug, Clone, PartialEq, Eq)]
...
// INVARIANT: entries should be non-zero, or this should be converted to a
// `SingleName`.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct NamePath {
...
// INVARIANT: never push onto a Single. A Single is a final form, demoted from a
// Path.
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub enum Exp_ {
    ...
    UnresolvedError,
    // e.X, where X is not a valid tok after dot and cannot be parsed (includes location of the
    // dot)
...
impl Attribute_ {
    pub fn attribute_name(&self) -> &'static str {
        ...
        match self {
            ...
            Attribute_::Authenticator { .. } => AK::Authenticator.name(),
            Attribute_::View => AK::View.name(),
        }
    }
}
...
impl AstDebug for Attribute_ {
    fn ast_debug(&self, w: &mut AstWriter) {
        ...
        match self {
            ...
            A::Authenticator { version } => {
                w.write("authenticator");
