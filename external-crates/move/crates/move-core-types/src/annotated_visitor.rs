...
/// Exposes information about the byte stream that the value being visited came
/// from, namely the bytes themselves, and the offset at which the value starts.
/// Also exposes the layout of the value being visited.
...
/// Exposes information about a variant being visited (its layout, details about
/// the next field to be visited, the variant's tag, and name) to a visitor
/// implementation, and allows that visitor to progress the traversal (by
...
impl<'c, 'b, 'l> ValueDriver<'c, 'b, 'l> {
    ...
    /// All the bytes in the byte stream (including the ones that have been
    /// read).
    ...
    /// layout was not supplied when the driver was created (which should
    /// only happen if the driver was created for visiting a struct
    /// specifically).
...
impl<'c, 'b, 'l> VariantDriver<'c, 'b, 'l> {
    ...
    /// Visit the next field in the variant. The driver accepts a visitor to use
    /// for this field, allowing the visitor to be changed on recursive
    /// calls or even between fields in the same variant.
    ///
    /// Returns `Ok(None)` if there are no more fields in the variant, `Ok((f,
    /// v))` if there was an field and it was successfully visited (where
    /// `v` is the value returned by the visitor, and `f` is the layout of
    /// the field that was visited) or an error if there was an underlying
    /// deserialization error, or an error during visitation.
...
/// Like `visit_value` but specialized to visiting a vector (where the `bytes`
/// is known to be a serialized move vector), and the layout is the vector's
/// element's layout.
...
/// Like `visit_struct` but specialized to visiting a variant (where the `bytes`
/// is known to be a serialized move variant), and the layout is an enum layout.
