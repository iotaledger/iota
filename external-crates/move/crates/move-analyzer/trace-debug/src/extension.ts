 ...
 *
 * @param traceContent trace content in JSON format.
 * @returns traced function info containing package address, module, and function itself.
 */
function getTracedFunctionInfo(traceContent: string): TracedFunctionInfo {
    const traceLines = traceContent.split('\n');
    if (traceLines.length <= 1) {
        throw new Error(`Empty trace file '${traceContent}'`);
    }
