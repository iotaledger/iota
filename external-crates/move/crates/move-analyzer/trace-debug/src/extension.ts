        ...
        if (!config.request && !config.name) {
            try {
                ...
                if (editor && (editor.document.languageId === MOVE_LANGUAGE_ID
                    || editor.document.languageId === TRACE_FILE_EXT)) {
                    config.traceInfo = await findTraceInfo(editor);
                    config.source = '${file}';
                } else {
                    const traceViewUri = traceViewTabUri();
                    if (traceViewUri) {
                        const tracedFunctionInfo = await getTracedFunctionInfo(traceViewUri.fsPath);
...
async function findTraceInfo(editor: vscode.TextEditor): Promise<string> {
    ...
    if (openedFilePath.endsWith(TRACE_FILE_EXT)) {
        const tracedFunctionInfo = await getTracedFunctionInfo(openedFilePath);
...
async function getTracedFunctionInfo(traceFilePath: string): Promise<TracedFunctionInfo> {
    ...
    const traceLines = traceFilePath.split('\n');
