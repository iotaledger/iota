...
async function findTraceInfo(editor: vscode.TextEditor): Promise<string | undefined> {
    ...
    if (openedFileExt === JSON_FILE_EXT) {
        ...
        const fun = constructTraceInfo(openedFilePath);
        if (!fun) {
            // trace contains external events - the runtime ignores trace info
            // in this case
            return undefined;
        }
        tracedFunctions = [fun];
...
function findTracedFunctionsFromTrace(pkgRoot: string, pkgAddr: number, module: string): string[] {
    ...
    for (const p of filePaths) {
        ...
        if (tracedFunctionInfo.pkgAddr === pkgAddr && tracedFunctionInfo.module === module) {
            const traceInfo = constructTraceInfo(tracePath);
            if (traceInfo) {
 ...
 * @throws Error with a descriptive error message if the trace information cannot be constructed.
  *
 */
function constructTraceInfo(tracePath: string): string | undefined {
    if (tracePath.endsWith(JSON_FILE_EXT) &&
        path.basename(tracePath, path.extname(tracePath)) === EXT_EVENTS_TRACE_FILE_NAME) {
        return undefined;
    }
    const tracedFunctionInfo = getTracedFunctionInfo(tracePath);
