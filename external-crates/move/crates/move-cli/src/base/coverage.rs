...
pub enum CoverageSummaryOptions {
    ...
    #[clap(name = "lcov")]
    Lcov {
        /// Compute differential coverage for the provided test name. Lines that
        /// are hit by this test only will show as covered, and lines
        /// that are hit by both this test and all other tests will
        /// show as "uncovered". Otherwise lines are not annotated with coverage
        /// information.
        #[clap(long = "differential-test")]
        differential: Option<String>,
        /// Compute coverage for the provided test name. Only this test will
        /// contribute to the coverage calculation.
...
impl Coverage {
    pub fn execute(self, path: Option<&Path>, config: BuildConfig) -> anyhow::Result<()> {
        ...
        // We treat lcov-format coverage differently because it requires traces to be
        // present, and we don't use the old trace format for it.
    ...
    pub fn output_lcov_coverage(
        ...
        // Make sure we always compile the package in test mode so we get correct source
        // maps.
