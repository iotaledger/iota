...
/// A Differential report is one that shows the lines that are covered by both
/// passing and failing tests as "uncovered", lines only covered by the failing
/// tests are marked as "covered", and lines that are covered only by passing
/// tests (or are not covered at all) are not marked either way.
/// More explicitly:
/// * If hit in both => then uncovered in output `DA: _, _, 0`
/// * If hit in failing only => then hit in output `DA: _, _, 1`
/// * If hit in passing only (or not hit at all) => then not hit in output (_NO
///   DA line at all_)
...
/// Compute the differential between two reports and return the resulting
/// report. If the resulting report is empty, return None.
...
/// Compute the differential between two lines and return the resulting line. If
/// the resulting line is not to be kept (i.e., the DA record should be removed
/// according the rules above) return `None`.
...
/// Compute the differential between two functions and return the resulting
/// function. If the resulting function report should be removed entirely then
/// return `None`.
...
/// Compute the differential between two branches and return the resulting
/// branch. If the BRDA should be removed entirely then return `None`.
...
/// Compute the differential across a map of values. This is slightly different
/// than a `merge_by` type of operation -- more like a `merge_by_filter` type of
/// operation --
///
/// The resulting map will contain the keys from the first map and for any
/// values in the intersection of the two maps the values will be merged with
/// `merge_fn`. If `merge_fn` returns `None` for a key, that key (and its
/// associated values either of the two input maps) will not appear in the
/// output map at all.
