...
/// Check to make sure that the bytecode vector is non-empty and ends with a
...
/// predictable abstract interpretation performance.
///
/// ## References
///
...
fn verify_reducibility<'a>(
    ...
    // would appear later in the pre-order) are processed first.
    for head in summary.preorder().rev() {
        // If a node has no back edges, it is not a loop head, so doesn't need to be
        // processed.
        ...
        // Collect the rest of the nodes in `head`'s loop, in `body`.  Start with the
        // nodes that jump back to the head, and grow `body` by repeatedly
        // following predecessor edges until `head` is found again.
        ...
        while let Some(node) = frontier.pop() {
            for pred in summary.pred_edges(node) {
                ...
                // `pred` can eventually jump back to `head`, so is part of its body.  If it is
                // not a descendant of `head`, it implies that `head` does not
                // dominate a node in its loop, therefore the CFG is not
                // reducible, according to Property 1 (see doc comment).
        ...
        // Collapse all the nodes in `body` into `head`, so it appears as one node when
        // processing outer loops (this performs a sequence of Operation 4(b),
        // followed by a 4(a)).
