...
impl<'a> DuplicationChecker<'a> {
    ...
    fn check_enum_definitions(&self) -> PartialVMResult<()> {
        ...
        // NB: We check uniqueness across both enum and struct handles at this point to
        // make sure data definitions are not duplicated across struct and
        // enums.
        ...
        for (enum_idx, enum_def) in self.module.enum_defs().iter().enumerate() {
            ...
            // NB: we allow zero-sized variants: since we require non-empty enums we always
            // have a tag and therefore a variant with no fields is still
            // non-zero sized whereas a struct with zero fields is zero-sized.
