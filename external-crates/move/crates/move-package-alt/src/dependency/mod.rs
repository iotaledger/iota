...
#[derive(Debug, Clone)]
pub struct CombinedDependency(Dependency<Combined>);
pub struct ResolvedDependency(Dependency<Resolved>);
pub struct PinnedDependency(Dependency<Pinned>);
pub struct FetchedDependency(Dependency<Fetched>);

#[derive(Debug, Clone)]
pub struct PinnedDependencyInfo(Dependency<Pinned>);

/// [Dependency<Combined>]s contain the dependency-type-specific things that
/// users write in their Move.toml files. They are formed by combining the
/// entries from the `[dependencies]` and the `[dep-replacements]` section of
/// the manifest.
type Combined = ManifestDependencyInfo;

/// A [Dependency<Resolved>] is like a [Dependency<Combined>] except that it no
/// longer has externally resolved dependencies
type Resolved = ResolverDependencyInfo;

/// [Dependency<Pinned>]s are guaranteed to always resolve to the same package
...
/// [Dependency] wraps information about the location of a dependency (such as
/// the `git` or `local` fields) with additional metadata about how the
/// dependency is used (such as the source file, enviroment overrides, etc).
///
/// At different stages of the pipeline we have different information about the
/// dependency location (e.g. resolved dependencies have no `External` variant,
/// pinned dependencies have a pinned git dependency, etc). The `DepInfo` type
/// encapsulates these invariants.
#[derive(Debug, Clone)]
struct Dependency<DepInfo> {
    ...
    /// Was this dependency written with `override = true` in its original
    /// manifest?
    is_override: bool,
...
impl CombinedDependency {
    /// Specialize an entry in the `[dependencies]` section, for the environment
    /// named `source_env_name`
    pub fn from_default(
        file: FileHandle,
    ...
    /// Load from an entry in the `[dep-replacements]` section that has no
    /// corresponding entry in the `[dependencies]` section of the manifest.
    /// `source_env_name` refers to the environment name and ID in the
    /// original manifest; it is used as the default environment for the
    /// dependency, but will be overridden if `replacement` specifies
...
pub async fn pin<F: MoveFlavor>(
    mut deps: DependencySet<CombinedDependency>,
...
) -> PackageResult<DependencySet<FetchedDependency>> {
    use DependencySet as DS;
    use Pinned as P;
    let mut result = DependencySet::new();
    for (env, pkg, dep) in deps.into_iter() {
        let fetched = PackagePath::new(dep.fetch().await?)?;
        result.insert(env, pkg, FetchedDependency(dep.0.map(|_| fetched)))
    }
    Ok(result)
}
