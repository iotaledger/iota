...
/// A [ResolvedDependency] is like a [CombinedDependency] except that it no
/// longer has externally resolved dependencies
...
impl ResolvedDependency {
    /// Replace all external dependencies in `deps` with internal dependencies
    /// by invoking their resolvers. Requires all environments in `deps` to
    /// be contained in `envs`
...
/// Invoke the `resolver` process and feed it `reqs`; parse the output and log
/// as appropriate
