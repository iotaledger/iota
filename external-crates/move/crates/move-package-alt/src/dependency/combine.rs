...
/// [CombinedDependency]s contain the dependency-type-specific things that users
/// write in their Move.toml files. They are formed by combining the entries
/// from the `[dependencies]` and the `[dep-replacements]` section of the
/// manifest.
#[derive(Debug, Clone)]
pub struct CombinedDependency(pub(super) Dependency<Combined>);

impl CombinedDependency {
    /// Combine the `[dependencies]` and `[dep-replacements]` sections of
    /// `manifest` (which was read from `file`).
