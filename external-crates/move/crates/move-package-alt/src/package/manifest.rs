...
impl<F: MoveFlavor> Manifest<F> {
    ...
    /// The combined entries of the `[dependencies]` and `[dep-replacements]`
    /// sections for this manifest
    /// The combined entries of the `[dependencies]` and `[dep-replacements]`
    /// sections for this manifest
...
/// Combine the `[dependencies]` and `[dep-replacements]` sections of `manifest`
/// (which was read from `file`).
fn combine_deps(
    file: FileHandle,
...
impl ManifestError {
    ...
    fn with_span<'a, T: Into<ManifestErrorKind>>(loc: &'a Location) -> impl Fn(T) -> Self + 'a {
        move |e| ManifestError {
            kind: Box::new(e.into()),
