...
pub struct DependencyInfo {
    /// A map between a package id (generated by the graph builder) and  the
...
pub struct DepInfo {
    ...
    /// Contains the package's manifest digest. This is used to verify if a
    /// manifest has changed and re-pinning is required.
    pub manifest_digest: String,
    /// The package's dependencies, a map from the package name to the package
    /// id.
...
impl<F: MoveFlavor + fmt::Debug> Lockfile<F> {
    ...
    /// Return the pinned dependencies for the given environment, if it exists
    /// in the lockfile.
    pub fn pinned_deps_for_env(&self, env: &EnvironmentName) -> Option<&DependencyInfo> {
        self.pinned.get(env)
    }

    /// Return a map that has an environment as key and the dependencies for
    /// that environment.
