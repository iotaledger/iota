...
/// A serialized entry in the `[pinned.<environment>.<package-id>]` table of the
/// lockfile
...
pub struct Pin {
    ...
    /// The environment that should be used for the dependency (as defined in
    /// the dependency's manifest but specified in the depender's manifest).
    ///
