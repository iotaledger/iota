...
/// The on-chain identifier for an environment (such as a chain ID); these are
/// bound to environment names in the `[environments]` table of the manifest
...
/// or `rename-from`); these are in the [ManifestDependency] or
/// [ManifestDependencyReplacement] types.
#[derive(Debug, Clone)]
