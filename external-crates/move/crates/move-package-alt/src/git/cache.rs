...
/// A subdirectory within a particular commit of a git repository. The files may
/// or may not have been downloaded, but you can ensure that they have by
/// calling `fetch()`
#[derive(Clone, Debug)]
pub struct GitTree {
    /// Repository URL
...
impl GitCache {
    ...
    /// Resolve the git committish `rev` (branch, tag, or sha) from a repository
    /// at the remote `repo` to a commit hash. This will make a remote call
    ...
    /// Construct a tree in `self` for the repository `repo` with the provided
    /// `sha` and `path_in_repo`.
    pub fn tree_for_sha(
        &self,
        repo: String,
        sha: GitSha,
...
impl GitTree {
    /// The absolute path on the filesystem where this tree will be downloaded
    /// when `fetch` is called
    pub fn path_to_tree(&self) -> PathBuf {
        self.path_to_repo.join(&self.path_in_repo)
    }

    /// Ensure that the files are downloaded to `self.path_to_tree()`. Fails if
    /// there was already a dirty checkout there (call
    /// [Self::fetch_allow_dirty] if you don't want to fail). Returns
    /// `self.path_to_tree()`.
    pub async fn fetch(&self) -> GitResult<PathBuf> {
        self.checkout_repo(false).await
    }

    /// Ensure that there are files downloaded to `self.path_to_tree()`. Has no
    /// effect if `self.path_to_tree()` already exists. Returns
    /// `self.path_to_tree()`
    ...
    /// Checkout the directory using a sparse checkout. It will try to clone
    /// without checkout, set sparse checkout directory, and then checkout
    /// the folder specified by `self.path_in_repo` at the given sha.
    ///
    /// Fails if `allow_dirty` is false and a dirty checkout of the directory
    /// already exists
    ...
    async fn is_dirty(&self) -> bool {
        ...
        let Ok(output) = self
            ...
            // if there's an error, the git repo has probably been tampered with - it's
            // dirty
            return true;
        };
    ...
    /// The path to the folder containing the cached repo (without the addition
    /// of the path within the repo)
    #[cfg(test)]
...
/// Resolve the git committish `rev` (branch, tag, or sha) from a repository at
/// the remote `repo` to a commit SHA. This will make a remote call so network
/// is required.
async fn find_sha(repo: &str, rev: &Option<String>) -> GitResult<GitSha> {
    if let Some(r) = rev {
        ...
        // than if there is no revision. In no revision case, we need to find
        // the default branch of that remote.
...
/// returns a non-zero exit status; returns the standard output and logs
/// standard error to `info!`
...
/// Output the `cmd` and its args in a concise form (without quoting or showing
...
mod tests {
    ...
    /// It returns the temporary directory, the path to the directory containing
    /// the repository, the first commit sha, and and the second commit sha.
    ///
    /// The first commit contains the following files:
    ///  - packages/pkg_a/Move.toml (copied from
    ...
    ///  - packages/pkg_b/Move.toml (copied from
    ...
    /// that start with `.` (in particular, `.git`)
    ...
    /// Ensure that loading a package into an empty cache outputs only the
    /// correct files
    #[test(tokio::test)]
    ...
    /// Ensure that loading a package into an empty cache from a SHA outputs
    ...
    /// Ensure that checking out two different paths from the same repo / sha
    /// works
    #[test(tokio::test)]
    ...
    async fn test_wrong_sha() {
        ...
        // note: in current implementation, find_sha succeeds with an invalid sha; it
        // doesn't contact the server - we only fail when we try to fetch (which
        // seems reasonable)
        let git_tree = cache
            .resolve_to_tree(
                &repo_path,
