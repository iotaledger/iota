...
impl<F: MoveFlavor> PackageGraphBuilder<F> {
    ...
    /// manifests digests are out of date). If the resolution graph is
    /// up-to-date, it is returned. Otherwise a new resolution graph is
    /// constructed by traversing (only) the manifest files.
    pub async fn load(
        &self,
        path: &PackagePath,
        env: &EnvironmentName,
