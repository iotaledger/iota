...
/// A node in the package graph, containing a [Package] in a particular
/// environment
#[derive(Debug)]
#[derive_where(Clone)]
...
impl<F: MoveFlavor> PackageGraph<F> {
    /// Loads the package graph for each environment defined in the manifest. It
    /// checks whether the resolution graph in the lockfile inside `path` is
    /// up-to-date (i.e., whether any of the manifests digests are out of date).
    /// If the resolution graph is up-to-date, it is returned. Otherwise a new
    /// resolution graph is constructed by traversing (only) the manifest files.
    ...
    /// Constructs a [PackageGraph] for each environment in the manifest, by
    /// pinning and fetching all transitive dependencies from the manifests
    /// rooted at `path` (no lockfiles are read).
    ...
    /// read) for the passed environment.
