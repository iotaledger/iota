...
impl<I: AsyncRead + Unpin, O: AsyncWrite + Unpin> Endpoint<I, O> {
    ...
    /// Call the RPC method [method] with argument [arg]; decode and return the
    /// output
    ...
    /// Call the method [method] once for each argument in [args] using a JSON
    /// RPC batch request and await all of the responses. Return the results
    /// of the calls in the same order as [args].
    ...
    /// Generate a [Request] to call [method]([arg]) using [self.sqn];
    /// [self.sqn]
