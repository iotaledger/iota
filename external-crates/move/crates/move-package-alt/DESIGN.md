...
[notion-design]: https://www.notion.so/iotaledger/Package-Management-Revamp-Concrete-Design-1b56d9dcb4e980ccb06ad12ad18004c0?pvs=4
...
- 4/25/25
  - TODO:
    ...
    - iota move build/test now use the default dependencies unless you ask otherwise
...
# pinned transitive dependencies from most recent `iota move upgrade-deps`
...
1. The user explicitly asks for it by running `iota move update-deps`. This command will repin all
   dependencies (or you could ask to update a single dependency using `iota move update-deps
   dep-name`).
...
- unpublished.dependencies.unpinned matches manifest.dependencies (likewise for dep-replacements).
  If we discover a mismatch, we upgrade the mismatched dependency (as if the user ran `iota move
   upgrade-deps foo`
...
best-effort on that. We may provide an additional tool to help fix things up (e.g. `iota move
...
If the user runs `iota move upgrade-deps`, we rerun resolution, pinning, fetching, and validation for
all dependencies. If they run `iota move upgrade-deps d1 d2` we rerun these steps only for the
...
passing in the `flavor` field, which is of type String. It can take `iota`, `core`, `aptos`, and some
...
- Crazy example: https://github.com/pyth-network/pyth-crosschain/tree/main/target_chains/iota/contracts
- Left this in the tooling meeting notes, but for anyone else interested, this is the IotaGPT Move
  Dataset v1:
  [https://github.com/CMU-IotaGPT/iota-move-dataset-v1](https://github.com/CMU-IotaGPT/iota-move-dataset-v1)
...
## Impact on the packages we deploy (e.g. iotafrens)
