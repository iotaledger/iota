...
impl PartialEq for Dependency {
    ...
    //
    // Dependency equality also ignores the `dep_override` flag, since two
    // dependencies still refer to the same package even if one of them is an
    // override.
    fn eq(&self, other: &Self) -> bool {
    ...
    pub fn get_graph(
        ...
        let digest_and_lock_contents = lock_file
            .map(|mut lock_file| match schema::Header::read(&mut lock_file) {
                Ok(header) if header.version < schema::VERSION => None, // outdated lock file -
                // regenerate
...
impl DependencyGraph {
    ...
    pub fn merge(
        ...
        // insert self into the dep graphs, emptying out the table and graph to be
        // merged below
        dep_graphs.insert(
        ...
        // TODO maybe remove dep mode edges from dep_graphs then update the combined
        // graph
    ...
    fn insert_direct_dep(
        ...
        Ok(match dep {
            ...
            PM::Dependency::External(_) => {
                // External dependencies exist in the subgraph of the root package and are added
                // to the package_graph as such.
                let d = sub_graph
                    .package_graph
    ...
    /// contains all packages (including packages that are exclusively
    /// dev-mode-only).
    ///
    ...
    /// Resolves the packages described at dependency `to` the dependency
    /// specified by`from` with manifest at path `package_path` by running
    /// the binary `resolver`. `mode` decides whether the resulting packages
    /// are added to `self` as dependencies of `package_name` or
    /// dev-dependencies.
    ///
    /// Sends progress updates to `progress_output`, including stderr from the
    /// resolver, and captures stdout. The output is expected to be one or
    /// more null-separated string content. Each string is interpreted as a
    /// dependency graph, as represented by the Move.lock TOML schema. It
    /// returns each subgraph to be merged into the whole program dependency
    /// graph.
    ...
    > {
        ...
        // TODO(optimization): this will collect all stdout in memory, but can be
        // streamed instead.
        ...
        // Loop over null-separated lock file contents, creating the graph and adding it
        // to the result.
        loop {
            match reader.read_until(0, &mut buffer) {
                ...
                Ok(_) => {
                    ...
                    {
                        [(_, id, _)] => *id,
                        // TODO: We can in fact allow allow multiple root packages / graphs and
...
> {
    ...
    // Compare deps in both graphs. See `PartialEq` implementation for `Package` and
