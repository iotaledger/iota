...
impl VMRuntime {
    ...
    /// Attempt to load a type tag to a `Type` without loading any additional
    /// modules. This may fail if the type tag is not already loaded in the
    /// VM. NB: the type tag _must_ be defining ID based.
