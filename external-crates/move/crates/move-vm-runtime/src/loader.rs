...
pub struct ModuleCache {
    ...
    /// A mapping of the different defining IDs that we have seen and that are
    /// present in the type cache. This is a mapping of a defining ID to the
    /// runtime/original ID of the package.
...
impl Loader {
    ...
    /// Try to load a type tag from the cache. The `type_tag` must be a defining
    /// ID-based type tag. Returns the loaded type with the defining IDs
    /// replaced by their Runtime IDs (i.e., package IDs used in the VM for
    /// package resolution /execution).
