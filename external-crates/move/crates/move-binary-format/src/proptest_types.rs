...
/// the index types (`ModuleHandleIndex`, `DatatypeHandleIndex`, etc). To deal
...
/// over/under-flow.
pub(crate) fn prop_index_avoid(generator: PropIndex, avoid: usize, pool_size: usize) -> usize {
    assert!(pool_size > 1);
    assert!(pool_size > avoid);
    let rand = generator.index(pool_size);
    if rand != avoid {
        return rand;
    }
