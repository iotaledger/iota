...
impl CompiledModule {
    /// Serializes a `CompiledModule` into a binary at `version`. The mutable
    /// `Vec<u8>` will contain the binary blob on return.
    ...
    /// Serializes a `CompiledModule` into a binary at VERSION_MAX. The mutable
    /// `Vec<u8>` will contain the binary blob on return. To be used for
    /// testing only.
...
struct CommonSerializer {
    major_version: u32,
    /// Indicates the thing we are serializing is not intended for publication,
    /// i.e., was compiled with a mode enabled.
...
/// - Enum flag bit to specify type of declaration (only non-native enums are
///   supported right now)
/// - `EnumDefinition.variant_count` as a ULEB128 (number of variants defined in
///   the enum)
...
/// - `FieldDefinition.struct_` as a ULEB128 (index into the `DatatypeHandle`
///   table)
...
/// - `CodeUnit.jump_tables` as variable size table of jump targets used by
///   switch operations.
