...
//! Provides serializable summaries for packages--potentially with additional
//! names from the Move source code. The summaries include the signatures of all
//! functions (potentially macros) and datatypes (structs and enums).
...
use move_compiler::{
    expansion::ast as E,
    naming::ast as N,
    parser::{ast as P, ast::DocComment},
...
#[derive(Serialize, Deserialize)]
// TODO(cswords): This should mirror the KnownAttribute structure to save
// consumers of this from from needing to parse attributes a second time.
pub enum Attribute {
    Name(Symbol),
...
fn ext_attribute(entry: &KA::ExternalAttributeEntry) -> Attribute {
    use KA::{ExternalAttributeEntry_ as EAE, ExternalAttributeValue_ as EAV};
    match &entry.value {
...
fn attribute(k: &KA::KnownAttribute) -> Attribute {
    match k {
        ...
        KA::KnownAttribute::Flavored(flavored) => {
            use move_compiler::iota_mode::known_attributes::KnownAttribute as IotaKnownAttribute;
            match flavored {
                IotaKnownAttribute::View(view) => Attribute::Name(view.name().into()),
                IotaKnownAttribute::Authenticator(auth) => Attribute::Parameterized(
                    auth.name().into(),
                    vec![Attribute::Assigned(
                        move_compiler::iota_mode::known_attributes::authenticator::AuthenticatorAttribute::VERSION.into(),
                        auth.version.to_string(),
                    )],
                ),
            }
        }
