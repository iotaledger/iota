...
1. The `Move.toml` file is the package manifest and is required for a directory to be considered a
   ...
   on.
2. The `Move.lock` file is generated by the Move CLI and contains the fixed build versions of the
...
3. The `sources` directory is required and contains the Move modules that make up the package.
   Modules in this directory will always be included in the compilation process.
4. The `doc_templates` directory can contain documentation templates that will be used when
   generating documentation for the package.
5. The `examples` directory can hold additional code to be used only for development and/or
   tutorials, this will not be included when compiled outside of `test` or `dev` modes.
6. The `tests` directory can contain Move modules that are only included when compiled in `test`
