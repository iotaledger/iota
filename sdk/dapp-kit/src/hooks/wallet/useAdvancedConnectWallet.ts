...
type useAdvancedConnectWalletMutationOptions = Omit<
 ...
 */
export function useAdvancedConnectWallet({
    mutationKey,
    ...mutationOptions
}: useAdvancedConnectWalletMutationOptions = {}): UseMutationResult<
    ConnectWalletResult,
    Error,
    ConnectWalletArgs,
    unknown
    ...
    return useMutation({
        ...
        mutationFn: async ({ wallet, accountAddress }) => {
            try {
                ...
                const connectResult = await wallet.features['sui:advancedConnect']?.advancedConnect({
                    force: true
                });
