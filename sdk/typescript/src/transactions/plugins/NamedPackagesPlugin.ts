...
export type NamedPackagesPluginOptions = {
    /**
     * The IotaGraphQLClient to use for resolving names.
 ...
 * ```
 * Transaction.registerGlobalSerializationPlugin("namedPackagesPlugin", namedPackagesPlugin({ iotaGraphQLClient }));
 * ```
 *
...
export const namedPackagesPlugin = ({
    ...
    return async (
        ...
        (await Promise.all(batches.map((batch) => query(iotaGraphQLClient, batch)))).forEach(
            (res) => {
    ...
    async function query(client: IotaGraphQLClient, requests: NameResolutionRequest[]) {
        ...
        if (result.errors)
            throw new Error(JSON.stringify({ query: gqlQuery, errors: result.errors }));
        ...
        for (const req of requests) {
            ...
            if (!result.data || !result.data[key])
                throw new Error(`No result found for: ${req.name}`);
