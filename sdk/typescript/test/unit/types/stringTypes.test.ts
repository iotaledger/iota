...
describe('String type guards', () => {
    ...
    describe('isValidTransactionDigest()', () => {
        ...
        it('accepts base58 strings of the correct length', () => {
            expect(isValidTransactionDigest('vQMG8nrGirX14JLfyzy15DrYD3gwRC1eUmBmBzYUsgh')).toBe(
                true,
            );
            expect(isValidTransactionDigest('7msXn7aieHy73WkRxh3Xdqh9PEoPYBmJW59iE4TVvz62')).toBe(
                true,
            );
            expect(isValidTransactionDigest('C6G8PsqwNpMqrK7ApwuQUvDgzkFcUaUy6Y5ycrAN2q3F')).toBe(
                true,
            );
        });
    });

    describe('isValidIotaAddress() / isValidObjectID()', () => {
