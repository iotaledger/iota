...
This example starts by constructing a transaction to send IOTA. To construct transactions, import
...
[the IotaClient docs for more details](/typescript/iota-client#arguments)). These options will cause
...
-   `tx.moveCall({ target, arguments, typeArguments  })` - Executes a Move call. Returns whatever
...
[BCS](https://docs.iota.org/references/ts-sdk/bcs), which can be done using `tx.pure` eg,
