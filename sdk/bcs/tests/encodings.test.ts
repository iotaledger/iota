...
describe('BCS: Encodings', () => {
    ...
    it('should de/ser native encoding types', () => {
        ...
        const hex_str = toHEX(new Uint8Array([1, 2, 3, 4, 5, 6]));
        const b58_str = toB58(new Uint8Array([1, 2, 3, 4, 5, 6]));
        const b64_str = toB64(new Uint8Array([1, 2, 3, 4, 5, 6]));

        const serialized = bcs.ser('TestStruct', {
            hex: hex_str,
            base58: b58_str,
            base64: b64_str,
        });

        const deserialized = bcs.de('TestStruct', serialized.toBytes());
