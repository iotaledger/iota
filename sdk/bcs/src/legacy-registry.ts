...
export class BCS {
    ...
    constructor(schema: BcsConfig | BCS) {
        ...
        if (schema.types && schema.types.structs) {
            for (const name of Object.keys(schema.types.structs)) {
        ...
        if (schema.types && schema.types.enums) {
            for (const name of Object.keys(schema.types.enums)) {
        ...
        if (schema.types && schema.types.aliases) {
            for (const name of Object.keys(schema.types.aliases)) {
                    ...
                    (param) =>
                        new BcsType<unknown>({
                            ...
                            write: (data, writer) => {
                                ...
                                const typeMap = (params as string[]).reduce(
                                ...
                                return typeInterface._encodeRaw.call(
                                    this,
                                    writer,
                                    data,
                                    params,
    ...
    private registerVectorType(typeName: string): BCS {
        const { name, params } = this.parseTypeName(typeName);
        if (params.length > 1) {
        ...
        return this.registerType(
            ...
            function encodeVector(
                ...
                return writer.writeVec(data, (writer, el) => {
                    const elementType: TypeName = typeParams[0];
                    if (!elementType) {
                        throw new Error(
                    ...
                    const { name, params } = this.parseTypeName(elementType);
                    ...
                    const { name: innerName, params: innerParams } = this.parseTypeName(
                        typeMap[name],
                    );
    ...
    public registerStructType(typeName: TypeName, fields: StructTypeDefinition): BCS {
        ...
        for (const key in fields) {
            const internalName = this.tempKey();
        ...
        const struct = Object.freeze(fields); // Make sure the order doesn't get changed
        ...
        const canonicalOrder = Object.keys(struct);
        ...
        const { name: structName, params: generics } = this.parseTypeName(typeName);
            ...
            function encodeStruct(
                ...
                for (const key of canonicalOrder) {
                    ...
                    if (!generics.includes(fieldType)) {
                        ...
                        const { name, params } = this.parseTypeName(typeParams[paramIdx]);
    ...
    public registerEnumType(typeName: TypeName, variants: EnumTypeDefinition): BCS {
        ...
        const struct = Object.freeze(variants); // Make sure the order doesn't get changed
        ...
        const { name, params: canonicalTypeParams } = this.parseTypeName(typeName);
        ...
        return this.registerType(
            ...
            function encodeEnum(
                ...
                const orderByte = canonicalOrder.indexOf(key);
                if (orderByte === -1) {
                    throw new Error(
                ...
                const invariant = canonicalOrder[orderByte];
                const invariantType = struct[invariant] as TypeName | null;
                ...
                const paramIndex = canonicalTypeParams.indexOf(invariantType);
                const typeOrParam = paramIndex === -1 ? invariantType : typeParams[paramIndex];

                {
                    const { name, params } = this.parseTypeName(typeOrParam);
                    return this.getTypeInterface(name)._encodeRaw.call(
                        this,
                        writer,
                        data[key],
            ...
            function decodeEnum(this: BCS, reader: BcsReader, typeParams, typeMap) {
                const orderByte = reader.readULEB();
                const invariant = canonicalOrder[orderByte];
                const invariantType = struct[invariant] as TypeName | null;
    ...
    public parseTypeName(name: TypeName): {
        ...
        const params = splitGenericParameters(
